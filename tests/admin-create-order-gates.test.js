...
describe('a tier is required, and the price comes from it', () => {
  ...
  it('the form cannot submit without one either', () => {
    ...
    // The Create button and the handler both hang off `ready`, which requires a tier.
    expect(page).toMatch(/mode === "single" \? \(selectedTier && validQty/);
    expect(page).toContain('if (!ready) return;');
    expect(page).toMatch(/disabled=\{!ready \|\| !typedOk\}/);
    // And the price it quotes is the tier's, so no tier means no figure — not ₦0.
    expect(page).toContain('const quotePending = mode !== "bulk" && !selectedTier;');
    expect(page).toContain('"Pick a tier"');
  });
});
