...
async function applyReferral(db, depositUser, paidAmountKobo, { welcomeBonusPaid = false } = {}) {
  ...
  // This is the final eligibility fence. It both credits the referrer and
  // acquires the user-row lock, so a concurrent suspension/deletion that won
  // the race prevents every referral effect below from being created.
  ...
  return { paid: true, referrerBonus, inviteeBonus: effectiveInviteeBonus, referrer, withheldReason: null };
}
