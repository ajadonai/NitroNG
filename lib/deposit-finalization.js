...
import { nairaPerUnit, currencyForCountry } from './currency.js';
import { applyTopupBonus } from './topup-bonus.js';
import { cashReferralSettings } from './referral-cash.js';
...
const ELIGIBLE_REFERRER_WHERE = Object.freeze({
  status: 'Active',
  emailVerified: true,
  deletedAt: null,
});
...
async function applyReferral(db, depositUser, paidAmountKobo, { welcomeBonusPaid = false } = {}) {
  const empty = {
    paid: false,
    referrerBonus: 0,
    inviteeBonus: 0,
  ...
  const referrer = await db.user.findFirst({
    where: {
      ...
      ...ELIGIBLE_REFERRER_WHERE,
    },
    select: { id: true, email: true, name: true, signupIp: true },
  });
  if (!referrer) return { ...empty, withheldReason: 'referrer_ineligible' };
  if (referrer.id === depositUser.id) return empty;
  ...
  // Cash mode replaces the referrer's instant wallet credit with a held cash
  // earning (releasing after the refund window). The invitee side and every
  // guard above are identical in both modes.
  const cash = await cashReferralSettings(db);

  // This is the final eligibility fence. It acquires the user-row lock (and in
  // wallet mode credits the referrer), so a concurrent suspension/deletion
  // that won the race prevents every referral effect below from being created.
  const referrerClaim = await db.user.updateMany({
    where: {
      id: referrer.id,
      ...ELIGIBLE_REFERRER_WHERE,
    },
    data: { balance: { increment: cash.enabled ? 0 : referrerBonus } },
  });
  if (referrerClaim.count === 0) {
    return { ...empty, referrer, withheldReason: 'referrer_eligibility_changed' };
  }
  let cashEarning = 0;
  if (cash.enabled) {
    try {
      await db.referralEarning.create({
        data: {
          ...
          referredId: depositUser.id,
          amount: cash.amount,
          releasesAt: new Date(Date.now() + cash.holdDays * 86400000),
        },
      });
      cashEarning = cash.amount;
    } catch (err) {
      // Unique(referredId): this friend already earned someone a referral.
    ...
    await db.transaction.create({
      data: {
        ...
        note: `Referral bonus ${marker}`,
      },
    });
  }

  const effectiveInviteeBonus = welcomeBonusPaid ? 0 : inviteeBonus;
  if (effectiveInviteeBonus > 0) {
    await db.user.update({ where: { id: depositUser.id }, data: { balance: { increment: effectiveInviteeBonus } } });
  }
  await db.transaction.create({
    data: {
      userId: depositUser.id,
      type: 'referral',
      amount: effectiveInviteeBonus,
      status: 'Completed',
      idempotencyKey: inviteeKey,
      note: welcomeBonusPaid ? `Referral tracked (welcome bonus applied) ${marker}` : `Referral welcome bonus ${marker}`,
    },
  });

  return { paid: true, referrerBonus: cash.enabled ? 0 : referrerBonus, cashEarning, inviteeBonus: effectiveInviteeBonus, referrer, withheldReason: null };
}
...
async function finalizeOnce({
  ...
  return prismaClient.$transaction(async db => {
    ...
    // Acquire the wallet row before claiming any payment or bonus effect. The
    // conditional no-op increment is both a row lock and the final eligibility
    // fence: once permanent deletion commits, no late webhook, recovery poll,
    // or manual approval can recreate a wallet balance.
    const walletFence = await db.user.updateMany({
      where: {
        id: deposit.userId,
        status: { not: 'Deleted' },
        anonymizedAt: null,
    ...
    if (walletFence.count === 0) {
      const reviewNote = appendMarker(deposit.note || '', '[account_deleted:credit_blocked]');
      const reviewed = await db.transaction.updateMany({
        where: {
          id: deposit.id,
          type: 'deposit',
      ...
      return {
        finalized: false,
        reason: reviewed.count > 0 || current?.status === 'Review'
          ? 'account_deleted'
          : current?.status === 'Completed' ? 'already_completed' : 'not_claimable',
        transaction: current,
      };
    }
    ...
    await db.user.update({
      ...
      data: { balance: { increment: paidAmountKobo + coupon.amount }, depositCount: { increment: 1 } },
    });

    // Which bonus ladder applies is decided by the customer's country — the
    // market they are in — not by the rail they paid on and not by the currency
    // they had the site set to. The rail was the wrong question: it handed a
    // Nigerian paying USDT the dollar ladder, and would have handed a US
    // customer the naira one the moment another rail existed. Country is set
    // from the phone number at signup and only an admin can change it, so
    // nobody can choose their own bonus. See welcome-bonus.js.
    //
    // The rate is resolved here rather than in welcome-bonus.js because that
    // module is imported by the browser, and only for a foreign account, so a
    // Nigerian deposit — almost all of them — costs no lookup at all.
    const depositor = await db.user.findUnique({
      where: { id: deposit.userId }, select: { country: true },
    });
    const ladderCurrency = currencyForCountry(depositor?.country);
    let bonusRate = null;
    if (ladderCurrency !== 'NGN') {
      try {
        const fx = await resolveDepositRate();
        bonusRate = nairaPerUnit(ladderCurrency, fx);
      } catch (err) {
        log.warn('WelcomeBonus', `No rate for ${ladderCurrency}, using the naira ladder: ${err.message}`);
      }
    }
    const welcome = await applyWelcomeBonusDetailed(db, deposit.userId, paidAmountKobo, {
      ...
      paymentCurrency: ladderCurrency,
      nairaPerUnit: bonusRate,
    });
    ...
    const referral = await applyReferral(db, depositUser, paidAmountKobo, { welcomeBonusPaid: welcome.amount > 0 });
    const topup = await applyTopupBonus(db, deposit.userId, { now });
    ...
    return {
      ...
      topupBonus: topup.amount,
      topupUnlocked: topup.unlocked,
      totalUserCredit: paidAmountKobo + coupon.amount + welcome.amount + referral.inviteeBonus + topup.amount,
    };
