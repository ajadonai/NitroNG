...
import { nairaPerUnit } from './currency.js';
...
async function finalizeOnce({
  ...
  return prismaClient.$transaction(async db => {
    ...
    // Which bonus ladder applies is decided by the rail, not by whatever
    // currency the customer had the site set to (see welcome-bonus.js). The
    // rate is resolved here rather than there because that module is imported
    // by the browser — and only for a foreign rail, so a naira deposit, which
    // is almost all of them, costs no lookup at all.
    let bonusRate = null;
    if (paymentCurrency && paymentCurrency !== 'NGN') {
      try {
        const fx = await resolveDepositRate();
        bonusRate = nairaPerUnit(paymentCurrency, fx);
      } catch (err) {
        log.warn('WelcomeBonus', `No rate for ${paymentCurrency}, using the naira ladder: ${err.message}`);
      }
    }
