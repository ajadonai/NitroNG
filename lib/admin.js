...
const ROLE_PAGES = {
  owner: '*',
  superadmin: '*',
  admin: ['overview', 'orders', 'users', 'leaderboard', 'tickets', 'menu-builder', 'services', 'pricing', 'price-changes', 'blog', 'alerts', 'rewards', 'finance', 'activity', 'promotions', 'acquisition', 'issues', 'crew', 'changelog', 'notifications', 'tasks', 'outreach', 'refills'],
...
const ROLE_WRITE_PAGES = {
  owner: '*',
  superadmin: '*',
  admin: ['orders', 'users', 'tickets', 'menu-builder', 'services', 'pricing', 'blog', 'alerts', 'rewards', 'leaderboard', 'activity', 'promotions', 'issues', 'crew', 'changelog', 'tasks', 'outreach', 'refills'],
...
// Roles not listed are denied by default; customActions can grant exceptions per-admin
const RESTRICTED_ACTIONS = {
  ...
  'internalDashboards.view': ['owner', 'superadmin'],
  'api.sync': ['owner', 'superadmin', 'admin'],
  // Users
  'users.ban': ['owner', 'superadmin', 'admin'],
  'users.edit': ['owner', 'superadmin'],
  'users.adjustBalance': ['owner', 'superadmin'],
  // Wholesale pricing permanently changes what an account pays, so it sits with
  // the other money actions rather than with general user editing.
  'reseller.approve': ['owner', 'superadmin'],
  // Orders
  'orders.dispatch': ['owner', 'superadmin', 'admin'],
  'orders.redispatch': ['owner', 'superadmin', 'admin'],
  'orders.cancel': ['owner', 'superadmin', 'admin'],
  'orders.refund': ['owner', 'superadmin'],
  'orders.retry': ['owner', 'superadmin', 'admin'],
  'orders.check': ['owner', 'superadmin', 'admin', 'finance', 'support'],
  'orders.refill': ['owner', 'superadmin', 'admin', 'support', 'staff'],
  'orders.reset_refill': ['owner', 'superadmin', 'admin', 'support', 'staff'],
  'orders.update_link': ['owner', 'superadmin', 'admin'],
  'orders.reset_drip': ['owner', 'superadmin', 'admin'],
  // Marketing
  'promotions.manage': ['owner', 'superadmin'],
  ...
  'acquisition.manage': ['owner', 'superadmin'],
  'finance.topup': ['owner', 'superadmin'],
};
...
export async function requireAdmin(requiredPage = null, requireWrite = false) {
  let payload;
  try {
    payload = await getCurrentAdmin();
  } catch {
    return { admin: null, error: Response.json({ error: 'Service temporarily unavailable' }, { status: 503 }) };
  }
  if (!payload) {
  ...
  let admin;
  try {
    admin = payload._admin || await prisma.admin.findUnique({ where: { id: payload.id } });
  } catch {
    return { admin: null, error: Response.json({ error: 'Service temporarily unavailable' }, { status: 503 }) };
  }
 ...
 * Custom pages only expand VIEW access — write stays role-based
 */
export function getAdminWritePages(admin) {
  if (admin.role === 'owner' || admin.role === 'superadmin') return '*';
  return ROLE_WRITE_PAGES[admin.role] || [];
}
...
export async function logActivity(adminName, action, type = 'action') {
  try {
    await prisma.activityLog.create({
      data: { adminName, action: redactPII(action), type },
    });
  } catch (e) {
...
export function canSeeSensitive(admin) {
  return admin?.role === 'owner' || admin?.role === 'superadmin';
}
