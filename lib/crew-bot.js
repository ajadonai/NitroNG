...
export async function kickFromGroup(telegramUserId) {
  if (!TOKEN || !GROUP) return false;
  try {
    await fetch(`${API}/banChatMember`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chat_id: GROUP, user_id: Number(telegramUserId) }),
    });
    await fetch(`${API}/unbanChatMember`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chat_id: GROUP, user_id: Number(telegramUserId), only_if_banned: true }),
    });
    return true;
  } catch { return false; }
}
