...
/**
 * Whether an order batches at all — the question asked before any schedule is
 * calculated.
 *
 * The two catalogues answer it differently, because only one of them was
 * curated. A **tier** carries an editorial decision: the `drip` tag is set per
 * group and deliberately not set everywhere (32 of 46 follower groups have it),
 * so the tag is the answer and the type is not consulted.
 *
 * A **full-list row** has no group and so no such decision. Eligibility there is
 * the type itself: if the calculator has rules for it, they apply. Only `plays`
 * opts out, which it already does by returning no config. Nothing can be
 * over-split by this — `calculateIntradayDrip` returns null rather than cut a
 * batch below the provider's own minimum.
 *
 * Until 18 Sep 2026 this read `tier?.group?.tags` for both, so a full-list order
 * was never eligible for anything: 95 orders shipped in one burst, 36 of which
 * had earned a schedule, and a customer who picked five days on the form got
 * all of it at once.
 */
export function isDripEligible({ tier = null, type = '', platform = '' } = {}) {
  if (tier) return !!tier.group?.tags?.includes('drip');
  return !!getDripConfig(type, platform);
}
 ...
 */
export function calculateIntradayDrip(quantity, providerMin, startTime, serviceType, platform, options = {}) {
  ...
  // A day's batches have to fit inside that day. Without this a large day
  // becomes 16 batches two hours apart — a thirty-hour "day" that runs into
  // the next one. Past the budget we send fewer, larger batches instead.
  const maxSpanHours = Number(options.maxSpanHours);
  if (Number.isFinite(maxSpanHours) && maxSpanHours > 0) {
    const maxBatches = Math.max(1, Math.floor(maxSpanHours / config.intervalHours) + 1);
...
export function validateIntradayDuration(dispatches) {
  if (!dispatches || dispatches.length < 2) return null;
  const first = dispatches[0].scheduledAt;
  const last = dispatches[dispatches.length - 1].scheduledAt;
  if (last.getTime() - first.getTime() >= 24 * 60 * 60 * 1000) {
    return 'Schedule spans 24 hours or more. Please select at least 2 days.';
  }
  return null;
}
...
/**
 * How long one day's batches may run. A day owns 24 hours (or its delivery
 * window), less one interval so the last batch of a day lands before the next
 * day's anchor rather than colliding with it.
 */
export function dayBudgetHours(window, serviceType, platform) {
  const intervalHours = getDripConfig(serviceType, platform)?.intervalHours || 2;
  return Math.max(intervalHours, windowHours(window) - intervalHours);
}
...
function calculateMultiDayDripLegacy(quantity, dripDays, providerMin, startTime, serviceType, platform) {
  const daySpan = dayBudgetHours(null, serviceType, platform);
  ...
  for (let day = 1; day <= dripDays; day++) {
    ...
    const intraday = calculateIntradayDrip(dayQty, providerMin, dayStart, serviceType, platform, { maxSpanHours: daySpan });
...
export function calculateMultiDayDrip(quantity, dripDays, providerMin, startTime, serviceType, platform, dripConfig) {
  ...
  const daySpan = dayBudgetHours(window, serviceType, platform);
  ...
  for (let day = 1; day <= clampedDays; day++) {
    ...
    const intraday = calculateIntradayDrip(dayQty, providerMin, dayAnchor, serviceType, platform, { maxSpanHours: daySpan });
 ...
 */
export function rescheduleRemaining(pendingDispatches, dripConfig, serviceType, platform, completionAt) {
  ...
  for (const dispatch of pendingDispatches) {
    ...
    let lowerBound;
    if (prevDay === null) {
      const minByInterval = new Date(anchor.getTime() + intervalMs);
      lowerBound = new Date(Math.max(now.getTime(), original.getTime(), minByInterval.getTime()));
    } else {
...
export function checkDripFeasibility(quantity, dripDays, dripConfig, serviceType, platform, providerMin) {
  ...
  const tz = dripConfig?.timezone || null;
  const startTime = dripConfig?.startAt ? new Date(dripConfig.startAt) : new Date();

  const trial = calculateMultiDayDrip(quantity, dripDays, providerMin, startTime, serviceType, platform, dripConfig);
  if (!trial?.dispatches?.length) {
    return { feasible: false, error: 'Scheduler produced no batches.' };
  }

  const subMin = trial.dispatches.find(d => d.quantity < providerMin);
  if (subMin) {
    return {
      feasible: false,
      error: `Batch on day ${subMin.day} has ${subMin.quantity} units, below provider minimum of ${providerMin}.`,
    };
  }

  const activeDaysUsed = new Set(trial.dispatches.map(d => d.day)).size;
  if (activeDaysUsed < activeDays) {
    return {
      feasible: false,
      error: `Only ${activeDaysUsed} of ${activeDays} active days have enough quantity for the provider minimum (${providerMin}). Reduce days or increase quantity.`,
    };
  }

  // With a delivery window the customer is thinking in calendar days — "between
  // nine and five" — so each day's batches must land on that day's date.
  // Without one, a day is simply the 24 hours from its own anchor: an order
  // placed at 9pm still gets a whole first day instead of an hour of one.
  const window = dripConfig?.window || null;
  const DAY_MS = 24 * 60 * 60 * 1000;
  if (window) {
    for (const d of trial.dispatches) {
      const expectedDateKey = localDateKey(addLocalDays(startTime, d.day - 1, tz), tz);
      if (localDateKey(d.scheduledAt, tz) !== expectedDateKey) {
        return {
          feasible: false,
          error: `Day ${d.day} batches spill outside the delivery window. Widen the window or add more days.`,
        };
      }
    }
  } else {
    const anchor = trial.dispatches[0].scheduledAt.getTime();
    for (const d of trial.dispatches) {
      const offset = d.scheduledAt.getTime() - (anchor + (d.day - 1) * DAY_MS);
      if (offset < -60000 || offset >= DAY_MS) {
        return {
          feasible: false,
          error: `Day ${d.day} cannot fit inside a single day. Try a different number of days.`,
        };
      }
    }
  }

  if (pauseDay > 0 && pauseDay < dripDays) {
    const onPause = trial.dispatches.find(d => d.day === pauseDay + 1);
    if (onPause) {
      return {
        feasible: false,
        error: `A batch is scheduled on the paused day. Delivery window is too narrow.`,
      };
    }
  }
...
/**
 * Slice a newline-separated comments string for a specific drip batch.
 *
 * Each batch occupies a "slot" in the original schedule.  When a batch is
 * reset, the original is marked superseded (lastError = "replaced:#N") and a
 * replacement is created at a new batch number.  The replacement inherits the
 * original's slot so it sends the correct comment range.
 *
 * Comments are never repeated across batches.  When the order has fewer
 * comment lines than total quantity, lines are distributed proportionally.
 *
 * @param {string} allComments  - full newline-separated comment string
 * @param {number} batchQty    - how many comments THIS batch needs
 * @param {Array}  allDispatches - every dispatch for the order, sorted (day, batch).
 *        Each element needs { day, batch, quantity } and optionally { status, lastError, remains }.
 * @param {{ day: number, batch: number }} current - identifies the current dispatch
 * @returns {string} sliced newline-separated comments
 */
export function sliceCommentsForBatch(allComments, batchQty, allDispatches, current) {
  const lines = allComments.split('\n').filter(l => l.trim());
  if (lines.length === 0) return allComments;

  // Build replacement chain: new batch# → superseded dispatch it replaces
  const replacedBy = new Map();
  for (const d of allDispatches) {
    if (d.status === 'superseded' && d.lastError) {
      const m = d.lastError.match(/replaced:#(\d+)/);
      if (m) replacedBy.set(parseInt(m[1], 10), d);
    }
  }

  // Follow chain to find the original slot and sum delivered-in-chain
  let targetDay = current.day;
  let targetBatch = current.batch;
  let deliveredInChain = 0;
  let pred = replacedBy.get(current.batch);
  while (pred) {
    if (pred.remains != null) deliveredInChain += pred.quantity - pred.remains;
    targetDay = pred.day;
    targetBatch = pred.batch;
    const next = replacedBy.get(pred.batch);
    if (!next) break;
    pred = next;
  }

  const isReplacement = new Set(replacedBy.keys());

  // Compute offset and total from slot quantities.
  // Superseded batches count (they define the slot size).
  // Cancelled and replacement batches are skipped.
  let offset = 0;
  let totalQty = 0;
  let foundTarget = false;
  for (const d of allDispatches) {
    if (d.status === 'cancelled' || isReplacement.has(d.batch)) continue;
    if (d.day === targetDay && d.batch === targetBatch) foundTarget = true;
    if (!foundTarget) offset += d.quantity;
    totalQty += d.quantity;
  }

  offset += deliveredInChain;

  if (lines.length >= totalQty) {
    return lines.slice(offset, offset + batchQty).join('\n');
  }

  // Fewer lines than total quantity: distribute proportionally, no repeats
  const start = Math.floor(offset / totalQty * lines.length);
  const end = Math.floor((offset + batchQty) / totalQty * lines.length);
