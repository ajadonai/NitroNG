...
function intradayDispatches(qty) { return Math.floor(qty / BATCH_SIZE); }
...
export function calculateIntradayDrip(quantity, providerMin, startTime) {
  ...
  let dispatches = intradayDispatches(quantity);
  if (dispatches < 2) return null;
...
export { intradayDispatches, INTRADAY_HOURS, INTRADAY_THRESHOLD };
