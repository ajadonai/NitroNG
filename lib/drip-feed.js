...
export function checkDripFeasibility(quantity, dripDays, dripConfig, serviceType, platform, providerMin) {
  ...
  for (const d of trial.dispatches) {
    ...
    if (localDateKey(d.scheduledAt, tz) !== expectedDateKey) {
      return {
        feasible: false,
        error: `Day ${d.day} batches spill into a different calendar date. Widen the delivery window or add more days.`,
      };
    }
  }

  if (pauseDay > 0 && pauseDay < dripDays) {
    const pauseDateKey = localDateKey(addLocalDays(startTime, pauseDay, tz), tz);
    const onPause = trial.dispatches.find(d => localDateKey(d.scheduledAt, tz) === pauseDateKey);
    if (onPause) {
      return {
        feasible: false,
