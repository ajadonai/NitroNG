...
const CONSERVATIVE_THRESHOLDS = [
  { max: 1000, spreadMinutes: 0 },          // instant
  { max: 2000, spreadMinutes: 60 },         // 1 hour
  { max: 5000, spreadMinutes: 240 },        // 4 hours
  { max: 10000, spreadMinutes: 720 },       // 12 hours
 ...
 * @param {number} [minPerRun=0] - provider minimum per run; runs are capped so each meets this
 * @returns {{ runs: number, interval: number } | null} - null means instant delivery
 */
export function calculateDripFeed(platform, quantity, customThresholds = null, minPerRun = 0) {
  ...
  let runs = Math.max(2, Math.min(10, Math.ceil(quantity / targetPerRun)));

  // Reduce runs if per-run quantity would fall below provider minimum
  if (minPerRun > 0) {
    const maxRuns = Math.floor(quantity / minPerRun);
    if (maxRuns < 2) return null;
