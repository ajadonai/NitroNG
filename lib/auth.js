...
export function signAdminToken(admin) {
  const expiry = admin.role === 'superadmin' ? '14d' : '7d';
  return jwt.sign(
...
export async function setAdminCookie(token, role) {
  const cookieStore = await cookies();
  const maxAge = role === 'superadmin' ? 60 * 60 * 24 * 14 : 60 * 60 * 24 * 7;
...
export async function getCurrentUser() {
  ...
  if (!payload) { cookieStore.set('nitro_token', '', { maxAge: 0, path: '/' }); return null; }
  const prisma = (await import('@/lib/prisma')).default;
  const tHash = hashToken(token);
  const session = await prisma.session.findUnique({ where: { tokenHash: tHash } });
  if (!session) { cookieStore.set('nitro_token', '', { maxAge: 0, path: '/' }); return null; }
  const user = await prisma.user.findUnique({ where: { id: payload.id }, select: { status: true } });
  if (!user || user.status === 'Deleted' || user.status === 'Suspended' || user.status === 'PendingDeletion') { cookieStore.set('nitro_token', '', { maxAge: 0, path: '/' }); return null; }
...
export async function getCurrentAdmin() {
  ...
  if (!payload) { cookieStore.set('nitro_admin_token', '', { maxAge: 0, path: '/' }); return null; }
  const prisma = (await import('@/lib/prisma')).default;
  const tHash = hashToken(token);
  const session = await prisma.adminSession.findUnique({ where: { tokenHash: tHash } });
  if (!session) { cookieStore.set('nitro_admin_token', '', { maxAge: 0, path: '/' }); return null; }
