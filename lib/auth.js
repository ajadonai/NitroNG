...
/**
 * Why a sign-out happened, for the notice shown on the landing page.
 *
 * Only four, because only four can be proved. A missing session row is the
 * same row whether it was removed by a sign-out somewhere else, by the
 * five-session cap, or by a password change — nothing distinguishes them
 * after the fact, so `revoked` covers all three and the copy says so rather
 * than picking one and sounding certain.
 */
export const SIGNOUT_REASON = Object.freeze({
  EXPIRED: 'expired',   // the token reached its 24-hour or 7-day life
  AGED_OUT: 'aged_out', // the session reached the 30-day absolute ceiling
  REVOKED: 'revoked',   // the session row is gone
  BLOCKED: 'blocked',   // suspended, deleted, or pending deletion
});

/**
 * verifyUserToken, but it says whether the failure was an expiry.
 * Kept separate so the existing callers keep their null-or-payload contract.
 */
export function verifyUserTokenDetailed(token) {
  try {
    const decoded = jwt.verify(token, SECRET);
    if (decoded.type !== 'user') return { payload: null, expired: false };
...
/**
 * getCurrentUser, plus the reason when there is nobody.
 *
 * getCurrentUser() below is the unchanged one-value form that every other
 * route uses; this is the same walk with the reason kept. The cookie's
 * maxAge is the token's own lifetime, so by the time a token could be
 * rejected as expired the browser has usually dropped the cookie already —
 * which is why a missing token reports EXPIRED rather than a reason of its
 * own. For someone who was on the dashboard a moment ago, that is what it
 * means; nobody else sees this notice.
 */
export async function getCurrentUserWithReason() {
  const cookieStore = await cookies();
  const token = cookieStore.get('nitro_token')?.value;
  if (!token) return { user: null, reason: SIGNOUT_REASON.EXPIRED };
  const { payload, expired } = verifyUserTokenDetailed(token);
  if (!payload) {
    tryClearCookie(cookieStore, 'nitro_token');
    return { user: null, reason: expired ? SIGNOUT_REASON.EXPIRED : SIGNOUT_REASON.REVOKED };
  }
  ...
  if (!session) { tryClearCookie(cookieStore, 'nitro_token'); return { user: null, reason: SIGNOUT_REASON.REVOKED }; }
  if (Date.now() >= new Date(session.createdAt).getTime() + USER_ABSOLUTE_LIFETIME_SECONDS * 1000) {
    tryClearCookie(cookieStore, 'nitro_token');
    return { user: null, reason: SIGNOUT_REASON.AGED_OUT };
  }
  const status = session.user?.status;
  if (!status || status === 'Deleted' || status === 'Suspended' || status === 'PendingDeletion') { tryClearCookie(cookieStore, 'nitro_token'); return { user: null, reason: SIGNOUT_REASON.BLOCKED }; }
...
export async function getCurrentUser() {
  const { user } = await getCurrentUserWithReason();
  return user;
}
