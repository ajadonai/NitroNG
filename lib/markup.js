 ...
 */
/**
 * Which price band a provider cost falls in, by label.
 *
 * Exported because the reseller band caps are keyed by label and the only
 * honest way to find a service's cap is to ask the same brackets that priced
 * it. A second copy of this lookup would drift the first time Trip edits a
 * bracket edge.
 */
export function bandLabelFor(costKobo, settings = {}) {
  const cost = Number(costKobo);
  if (!Number.isFinite(cost) || cost <= 0) return null;
  const naira = cost / 100;
  const b = parseBrackets(settings).find(x => naira >= x.min && naira < (x.max >= 999999999 ? Infinity : x.max));
  return b?.label || null;
}
 ...
 */
export function effectiveResellerPct(retailKobo, costKobo, settings = {}, override = null, bandCap = null) {
  const retail = Number(retailKobo);
  if (!retail || retail <= 0) return 0;
  let asked = usablePct(override)
    ?? usablePct(settings.markup_reseller_discount)
    ?? DEFAULT_RESELLER_DISCOUNT;
  const cap = usablePct(bandCap);
  if (cap !== null) asked = Math.min(asked, cap);
 ...
 */
export function resellerPrice(retailPer1k, settings = {}, override = null, costKobo = null, bandCap = null) {
  ...
  let pct = usablePct(override)
    ?? usablePct(settings.markup_reseller_discount)
    ?? DEFAULT_RESELLER_DISCOUNT;

  // The band cap, where this service's cost band has one. It is the intent —
  // Trip chose 22% on Ultra — and the floor below is the backstop that catches
  // a cap set too high or a bracket edited after the fact. Both apply, the cap
  // first, because a price that satisfies the cap usually never reaches the
  // floor and a reseller should be told the number that was chosen for them.
  const cap = usablePct(bandCap);
