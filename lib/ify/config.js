...
export const IFY = {
  // Master kill switch. Set IFY_ENABLED="false" to instantly mute the bot (in + out).
  enabled: process.env.IFY_ENABLED !== 'false',

  // Soft-launch allowlist: comma-separated WhatsApp numbers (digits, e.g. 2348012345678).
  // Empty = everyone. Use this to test with a handful of numbers before going wide.
...
export function isAllowed(waNumber) {
  if (!IFY.allowlist.length) return true;
