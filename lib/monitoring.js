...
// A client sent a Next router-state header the server cannot use — not JSON,
// doubled, or over the size cap. Crawlers and broken proxies do this to the
// landing page; nothing of ours ran, and Next has already refused it. The
// codes are Next 16's (E10 parse, E142 too large, E418 multiple); the message
// match covers a version that renumbers them.
const MALFORMED_CLIENT_HEADER_CODES = new Set(['E10', 'E142', 'E418']);
export function isMalformedClientRequest(error) {
  return MALFORMED_CLIENT_HEADER_CODES.has(error?.__NEXT_ERROR_CODE)
    || /router state header/i.test(String(error?.message || ''));
}
...
export function forwardServerRequestError(
  ...
  if (isExpectedRequestControlFlow(error) || isMalformedClientRequest(error)) return false;
