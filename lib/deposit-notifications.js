import prisma from './prisma.js';
import { trackDeposit } from './meta-capi.js';
import { STAFF_NAMES, tgBonusWithheld, tgOutreachAlert, tgOutreachConversion, tgPayment } from './telegram.js';
import { attributedTouch } from './outreach-attribution.js';
import { referralBonusEmail, sendEmail, walletCreditEmail } from './email.js';
import { getWhatsAppChannelUrl } from './settings.js';
import { log } from './logger.js';
import { sendOutreach as ifySendOutreach } from './ify/outreach.js';

/** The same two helpers the outreach summary uses, so one person reads the
 *  same way everywhere and an email is never posted in full to a group chat. */
function staffName(tgId) { return STAFF_NAMES[String(tgId)] || `Staff ${String(tgId).slice(-4)}`; }
function mask(email) {
  if (!email || !email.includes('@')) return email || '';
  const [local, domain] = email.split('@');
  return local.length <= 2 ? `${local[0]}***@${domain}` : `${local[0]}***${local.slice(-1)}@${domain}`;
}

function channelFor(result, override) {
  if (override) return override;
  const method = String(result.transaction?.method || '').toLowerCase();
  if (method === 'crypto') return 'Crypto';
  if (method === 'manual') return 'Manual';
  if (method === 'flutterwave') return 'Flutterwave';
  return result.transaction?.method || 'Deposit';
}

export async function notifyDepositFinalized(result, {
  channel,
  approvedBy,
  clientIp,
  userAgent,
  fbp,
  fbc,
  sourceUrl,
} = {}) {
  if (!result?.finalized) return { attempted: 0, failed: [], skipped: true };

  const user = result.user || {};
  const reference = result.transaction?.reference;
  const method = channelFor(result, channel);
  const bonusTotal = (result.couponBonus || 0) + (result.welcomeBonus || 0) + (result.inviteeBonus || 0) + (result.topupBonus || 0);
  const tasks = [];

  tasks.push({
    name: 'meta',
    run: () => trackDeposit({
      email: user.email,
      phone: user.phone,
      userId: user.id || result.transaction?.userId,
      reference,
      amountKobo: result.depositAmount,
      clientIp: clientIp || user.lastIp,
      userAgent: userAgent || user.lastUa,
      fbp: fbp || user.lastFbp,
      fbc: fbc || user.lastFbc,
      sourceUrl,
    }),
  });

  tasks.push({
    name: 'telegram',
    run: async () => tgPayment(
      user.name || user.email || 'Unknown',
      result.depositAmount,
      bonusTotal,
      method,
      approvedBy,
      user.depositCount || null,
    ),
  });

  // Credit the agent who last spoke to this person, if anybody did. Outreach
  // could count touches and reach rates but never the money that followed a
  // call, so an agent could work a list for a week and see nothing attributed
  // to them.
  tasks.push({
    name: 'outreach-conversion',
    run: async () => {
      const userId = user.id || result.transaction?.userId;
      if (!userId) return;
      const touch = await attributedTouch(userId, result.transaction?.createdAt ? new Date(result.transaction.createdAt) : new Date());
      if (!touch) return;
      await tgOutreachConversion({
        name: user.name || mask(user.email) || 'Unknown',
        amountKobo: result.depositAmount,
        bonusKobo: bonusTotal,
        method,
        agent: staffName(touch.contactedBy),
        touch: touch.touchType,
        hoursSince: touch.hoursSince,
        depositNumber: user.depositCount || null,
      });
    },
  });

  tasks.push({
    name: 'first-deposit-alert',
    run: async () => {
      const userId = user.id || result.transaction?.userId;
      if (!userId) return;
      const count = await prisma.transaction.count({
        where: { userId, type: 'deposit', status: 'Completed' },
      });
      if (count === 1 && new Date() >= new Date('2026-08-01T00:00:00Z')) {
        const bonusKobo = (result.welcomeBonus || 0) + (result.inviteeBonus || 0) + (result.couponBonus || 0);
        const amountNaira = result.depositAmount / 100;
        const bonusNaira = bonusKobo / 100;
        await tgOutreachAlert(
          { name: user.name, email: user.email, phone: user.phone, createdAt: user.createdAt },
          'firstDeposit',
          { amount: amountNaira, bonus: bonusNaira },
        );
        ifySendOutreach({
          user: { id: userId, name: user.name, phone: user.phone },
          trigger: 'firstDeposit',
          extra: { amount: amountNaira, bonus: bonusNaira },
        }).catch(() => {});
      }
    },
  });

  if (user.email) {
    tasks.push({
      name: 'deposit-email',
      run: async () => {
        const amountNaira = result.depositAmount / 100;
        const waChannelUrl = await getWhatsAppChannelUrl();
        const html = walletCreditEmail(user.name || 'there', amountNaira, null, {
          kind: 'deposit',
          bonus: (result.welcomeBonus || 0) / 100,
          newBalance: (user.balance || 0) / 100,
          method,
          waChannelUrl,
        });
        return sendEmail(
          user.email,
          `₦${amountNaira.toLocaleString()} is in your wallet`,
          html,
          `Your deposit of ₦${amountNaira.toLocaleString()} landed and is ready to spend. Place an order: https://nitro.ng/dashboard`,
        );
      },
    });
  }

  if (result.referralPaid && result.referrer?.email && result.referrerBonus > 0) {
    tasks.push({
      name: 'referral-email',
      run: () => {
        const amountNaira = result.referrerBonus / 100;
        return sendEmail(
          result.referrer.email,
          `You received ₦${amountNaira.toLocaleString()} on Nitro!`,
          referralBonusEmail(result.referrer.name || 'there', amountNaira),
          `Someone you referred just made their first deposit. ₦${amountNaira.toLocaleString()} landed in your wallet: https://nitro.ng/dashboard`,
        );
      },
    });
  }

  if (result.welcomeWithheld) {
    tasks.push({
      name: 'welcome-withheld',
      run: async () => {
        const withheld = result.welcomeWithheld;
        return tgBonusWithheld(
          withheld.name,
          withheld.email,
          withheld.ip,
          withheld.priorClaims,
          withheld.windowDays,
          withheld.depositAmount,
          withheld.bonus,
        );
      },
    });
  }

  const settled = await Promise.allSettled(tasks.map(task => Promise.resolve().then(task.run)));
  const failed = [];
  settled.forEach((outcome, index) => {
    if (outcome.status !== 'rejected') return;
    const name = tasks[index].name;
    failed.push(name);
    log.warn('Deposit Notification', `${name} failed for ${reference || 'unknown reference'}: ${outcome.reason?.message || outcome.reason}`);
  });

  return { attempted: tasks.length, failed, skipped: false };
}
