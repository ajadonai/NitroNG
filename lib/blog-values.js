...
export async function getLiveValues() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      const [rows, platformRows, serviceCount, userCount, orderCount, orderStats] = result;
      return buildValues(rows, platformRows, serviceCount, userCount, orderCount, orderStats);
    } catch (err) {
      last = err;
...
function buildValues(rows, platformRows, serviceCount, userCount, orderCount, orderStats) {
  const s = {};
