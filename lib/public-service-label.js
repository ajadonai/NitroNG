...
const SERVICE_PHRASES = [
  ...
  // Concurrent viewers on a running stream, not a recorded view count. NTR-11384:
  // "Youtube Live Viewers | With Views | ..." has no "stream" between "live" and
  // "viewers", so it missed the pattern above and fell through to the generic
  // Views catch-all — the customer saw "YouTube Views" for a service they knew,
  // correctly, was live viewers.
  { label: 'Live Viewers', pattern: /\blive\s+viewers?\b/i },
 ...
 */
/**
 * Which platform a service name is about, or null when the name names none or
 * names more than one.
 *
 * The full list needs this because the provider's own category cannot be
 * trusted to say: 2,655 live services sit under "Other", "Vip", "Cheapest",
 * "Private" and "\u{1F535}" across both providers, and every one of them was
 * invisible while the list was keyed on that field. The name is the honest
 * source — it is also the text the customer reads.
 *
 * Deliberately silent when two platforms match. "Instagram" plus "TikTok" in
 * one name is a combo or a mislabel, and guessing which tile owns it is worse
 * than leaving it to the caller's fallback.
 */
export function detectPlatform(rawName) {
  const found = findPlatforms(normalizeSource(rawName));
