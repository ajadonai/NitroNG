...
export async function trackBonusConsumption(tx, userId, orderId, amount) {
  const credits = await tx.bonusCredit.findMany({
    ...
    // Soonest first, and NULLS LAST on ASC in Postgres — so a perishable
    // win-back credit is always spent before a permanent welcome one. Exactly
    // the right order, and it costs nothing to get.
...
export async function restoreBonusForRefund(tx, orderId) {
  ...
  // Account deletion uses the same user-row lock before expiring every unused
  // credit. Whichever transaction gets the lock first therefore wins cleanly:
  // a completed deletion cannot have bonus value recreated, while a restore
  // that wins first is subsequently included in the deletion sweep.
  const userIds = [...new Set(usages.map((usage) => usage.bonusCredit.userId))].sort();
  const restorableUserIds = new Set();
  for (const userId of userIds) {
    const [user] = await tx.$queryRaw`
      SELECT id, status, "anonymizedAt"
      FROM users
      WHERE id = ${userId}
      FOR UPDATE
    `;
    if (user && user.status !== 'Deleted' && !user.anonymizedAt) {
      restorableUserIds.add(userId);
    }
  }

  for (const usage of usages) {
    if (restorableUserIds.has(usage.bonusCredit.userId) && !usage.bonusCredit.expiredAt) {
      await tx.bonusCredit.updateMany({
        where: {
          id: usage.bonusCreditId,
          userId: usage.bonusCredit.userId,
          expiredAt: null,
        },
...
export async function grantWinbackCredit(db, userId, amount, expiryDays) {
  ...
  const credit = await db.$transaction(async (tx) => {
    // The conditional UPDATE both locks the account row and re-checks the
    // eligibility observed by the cron. This prevents a stale read from
    // granting new value after deletion, anonymization, or suspension.
...
export async function expireBonusCredits(db) {
  ...
  const expired = await db.bonusCredit.findMany({
    // `lte` never matches NULL, so a never-expiring credit is skipped here
    // without needing to be excluded — which is correct, and is now on purpose
...
export async function getBonusInfo(db, userId) {
  const credits = await db.bonusCredit.findMany({
    // A null expiry is not "no expiry date to compare against" — it is credit
    // that never expires, so it is always live. The welcome bonus is granted
    // that way: 91% of recipients order within a day, so a deadline would
    // motivate nobody and take something from everybody.
    where: {
      userId, amountRemaining: { gt: 0 }, expiredAt: null,
  ...
  // Postgres sorts NULLS LAST on ASC, so the soonest real expiry leads and a
  // never-expiring credit only surfaces when it is the only one held. The
  // wallet shows a deadline when there is one and just the figure when not.
  const soonestExpiry = credits[0].expiresAt;
  return { amount: totalBonus, expiresAt: soonestExpiry ? soonestExpiry.toISOString() : null };
}
