 ...
 *
 *     tr("Fund your wallet")           → "Alimentez votre portefeuille"
 *     tr("Fund your wallet")  (en)     → "Fund your wallet"
 *     tr("Nothing to translate yet")   → "Nothing to translate yet"
 *
 ...
 *
 * It is `tr`, not the conventional `t`, because `t` is the theme object in
 * every component in this codebase — `t.accent`, `t.cardBg`. Naming the
 * translator `t` would shadow the theme and call an object as a function.
 *
 * Interpolation is deliberately absent. A sentence with a figure in it gets
 * split at the call site — `{tr("You will be charged")} {money(x)}` — because a
 ...
 *
 * tr() is a hook, so it cannot be called at module scope — and plenty of copy
 * lives there: the platform groups in new-order.jsx, the cross-sell prompts,
 * the tier table headings. Those are data, defined once when the module loads,
 * long before any component exists to hold a hook.
 *
 *     const GROUPS = [{ label: msg("Social Platforms"), … }];   // module scope
 *     …
 *     <h3>{tr(group.label)}</h3>                                // in a render
 *
 * msg() returns its argument untouched. Its whole job is to be visible to
 * `npm run i18n:report`, which reads the source for strings that need
 * translating — without it, `tr(group.label)` asks the dictionary for a
 * sentence no tooling ever knew existed, and the first sign of trouble is a
 * customer reading English.
 */
export const msg = (english) => english;
