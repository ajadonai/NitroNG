...
export function classifyFlutterwaveResponse(payload, {
  ...
  // `amount` is what we asked for; `charged_amount` is what actually left the
  // customer — larger when fees are passed on, smaller when a bank transfer
  // arrived short. A short payment must never credit the full quote, whichever
  // field the shortfall shows up in.
  const chargedAmount = payload.data.charged_amount;
  if (typeof chargedAmount === 'number' && Number.isFinite(chargedAmount) && Math.round(chargedAmount * 100) < paidAmountKobo) {
    return failed('amount_mismatch', {
      paidAmountKobo: Math.round(chargedAmount * 100),
      expectedAmountKobo,
      underpaid: true,
    });
  }
...
export async function verifyFlutterwaveTransaction({
  ...
  try {
    ...
    if (!response?.ok) {
      ...
      // 400/404 means the tx_ref does not exist on Flutterwave's side — the
      // user abandoned checkout before Flutterwave recorded the transaction.
      // This is deterministic and will never resolve on retry.
      if (httpStatus === 400 || httpStatus === 404) {
        return failed('provider_not_found', { httpStatus });
      }
      return retryable('provider_http_error', { httpStatus });
    }
