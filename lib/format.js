...
/**
 * The date locale of the page as it stands, read from <html lang>, which the
 * LocaleProvider keeps current. For the ten-odd places that call
 * toLocaleDateString directly with a hardcoded "en-GB" — module-scope helpers
 * with no hook access and no lang prop — so a French or Arabic page stops
 * showing "Sept" in the middle of translated copy.
 *
 * Known limit, accepted on purpose: a component that renders no locale-context
 * value re-renders on nothing when the language switches, so a date it already
 * painted stays in the old locale until any state changes. Every fresh render
 * — and every /fr, /sw, /ar page load, the only path live in production — is
 * correct. Threading lang through every helper would fix the stale frame and
 * cost an argument at thirty call sites; useDates() remains the right tool
 * anywhere a component can use hooks.
 */
export const docDateLocale = () => {
  if (typeof document === "undefined") return DATE_LOCALE.en;
...
/**
 * The Lagos day, month and year boundaries every report buckets by. Lagos is
 * UTC+1 with no DST, so a boundary is a fixed hour off UTC — but four files
 * had written that arithmetic by hand, and one of them had it in SQL. This is
 * the one place it lives. `now` is injectable for tests and for callers that
 * need the bounds of a moment other than the present.
 */
export function watBounds(now = new Date()) {
  const HOUR = 60 * 60 * 1000;
  const watNow = new Date(now.getTime() + HOUR);
  const y = watNow.getUTCFullYear(), m = watNow.getUTCMonth();
  const todayStart = new Date(Date.UTC(y, m, watNow.getUTCDate()) - HOUR);
  const yesterdayStart = new Date(todayStart.getTime() - 24 * HOUR);
  const yesterdaySameTime = new Date(now.getTime() - 24 * HOUR);
  const monthStart = new Date(Date.UTC(y, m, 1) - HOUR);
  const lastMonthStart = new Date(Date.UTC(y, m - 1, 1) - HOUR);
  const nextMonthStart = new Date(Date.UTC(y, m + 1, 1) - HOUR);
  const yearStart = new Date(Date.UTC(y, 0, 1) - HOUR);
  const weekStart = new Date(todayStart.getTime() - 6 * 24 * HOUR);
  return { now, todayStart, yesterdayStart, yesterdaySameTime, monthStart, lastMonthStart, nextMonthStart, yearStart, weekStart };
}
...
/**
 * Month and weekday names in the language the page is in.
 *
 * The date-range picker carried its own hardcoded English arrays — "January",
 * "Su Mo Tu We Th Fr Sa" — so the calendar stayed English under a French or
 * Arabic dashboard while every date around it had already been localised.
 * Intl has these names for every locale; nothing should be typing them out.
 *
 * The reference dates are arbitrary and only pick a month or a weekday:
 * 1 Aug 2021 was a Sunday, which is where the weekday list starts.
 */
export const monthNames = (lang, style = "long") =>
  Array.from({ length: 12 }, (_, m) =>
    new Intl.DateTimeFormat(dateLocale(lang), { month: style }).format(new Date(2021, m, 1)));

export const weekdayNames = (lang, style = "short") =>
  Array.from({ length: 7 }, (_, d) =>
    new Intl.DateTimeFormat(dateLocale(lang), { weekday: style }).format(new Date(2021, 7, 1 + d)));
