...
export async function createCommission(orderId, userId, chargeKobo) {
  try {
    ...
    const totalCommission = Math.round(chargeKobo * (member.commissionRate / 100));
    if (totalCommission <= 0) return null;

    let leadAmount = 0;
    let marketerAmount = totalCommission;
    let leadId = null;

    if (member.role === 'crew' && member.leadId) {
      leadId = member.leadId;
      leadAmount = Math.round(totalCommission * (leadSplit / 100));
      marketerAmount = totalCommission - leadAmount;
    }
