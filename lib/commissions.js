...
const TRANSIENT_DB = /closed the connection|Can't reach database server|Connection terminated|ECONNRESET|\bP100[128]\b|\bP1017\b|\bP2024\b/i;
function isTransientDbError(err) {
  return TRANSIENT_DB.test(err?.code || '') || TRANSIENT_DB.test(err?.message || '');
}
...
export async function voidCommissions(orderId, reason = 'order_cancelled') {
  try {
    ...
    // Neon drops idle connections; the void is idempotent (SELECT FOR UPDATE,
    // a second run finds nothing and returns 0), so a dropped line gets two
    // more tries before it becomes a money alert.
    let count = 0;
    for (let attempt = 1; ; attempt++) {
      try { count = await run(); break; }
      catch (err) {
        if (attempt < 3 && isTransientDbError(err)) { await new Promise(r => setTimeout(r, 300 * attempt)); continue; }
        throw err;
      }
    }
    return count;
