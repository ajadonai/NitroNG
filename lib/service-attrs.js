...
/**
 * The attributes long enough to cost a phone a whole badge row, and the shorter
 * form a phone shows instead. A full-list row on a 360px screen fits about
 * three chips; "Lifetime guarantee" and "Ultra high quality" are each wider
 * than two of them, so on a narrow screen one of them pushes the rest onto a
 * second line.
 *
 * Only these three. An abbreviation the reader has to decode is a cost, and it
 * is worth paying where the alternative is a badge row that wraps — not for
 * "Non-drop" or "Worldwide", which already fit.
 *
 * "Lifetime" is a trim rather than an abbreviation: the noun is the promise and
 * "guarantee" is the word every other refill badge leaves out anyway, so the
 * short form reads as English and needs no decoding.
 *
 * The full words are what the row shows from 768px up, and they are the only
 * form the reseller API ever sends: this is a display decision about a narrow
 * screen, not a change to what a service is called.
 */
export const ATTR_SHORT = {
  'Lifetime guarantee': 'Lifetime',
