  ...
  return `<td style="padding:0 6px;">
    <a href="${href}" style="text-decoration:none;" target="_blank">
      <table cellpadding="0" cellspacing="0" border="0"><tr>
        <td style="width:36px;height:36px;background:#f5f0ec;border-radius:10px;" align="center" valign="middle">
          <img src="${ICON_CDN}/${iconName}.svg" width="16" height="16" alt="${alt}" style="display:block;opacity:.55;" />
        </td>
      </tr></table>
    </a>
  </td>`;
...
export function emailRow(label, value, valueColor = '#333') {
  return `<tr>
    <td style="padding:8px 0;font-size:13px;color:#918b85;border-bottom:1px solid #f0ece6;">${label}</td>
    <td align="right" style="padding:8px 0;font-size:13px;font-weight:600;color:${valueColor};border-bottom:1px solid #f0ece6;">${value}</td>
  </tr>`;
}
...
export function emailDataBox(rows) {
  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:24px;">
    <tr><td style="padding:0;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#faf8f5;border-radius:14px;border-left:3px solid #c47d8e;">
...
<body style="margin:0;padding:0;background:#f6f3ef;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f6f3ef;">
<tr><td align="center" style="padding:40px 16px;">
  <table cellpadding="0" cellspacing="0" border="0" style="max-width:520px;width:100%;background:#ffffff;border-radius:20px;border:1px solid #ebe6df;">
...
export async function sendPromotionBlast(promotionName, discountPercent, bannerCopy) {
  const users = await prisma.user.findMany({
    where: { notifPromo: true, banned: false, deletionScheduledAt: null },
    select: { email: true, name: true },
  });
  if (!users.length) return;

  const subject = bannerCopy || `${promotionName} is live | Save ${discountPercent}% on all orders`;
  const htmlTemplate = await emailWrap({
    label: 'Promotion', labelBg: '#fce7f3', labelColor: '#be185d',
    title: promotionName,
    body: `
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;"><tr><td align="center" style="padding:20px 24px;background:#fce7f3;border-radius:14px;">
        <div style="font-family:'Courier New',monospace;font-size:36px;font-weight:700;color:#be185d;">${discountPercent}% OFF</div>
        <div style="font-size:14px;color:#9d174d;margin-top:6px;">${bannerCopy || 'All services, limited time'}</div>
      </td></tr></table>
      <p style="font-size:15px;line-height:1.65;color:#555;margin:0 0 24px;">Place an order now and the discount is applied automatically at checkout. No codes needed.</p>
      ${emailCTA('https://nitro.ng/dashboard', 'Order now')}
    `,
  });
