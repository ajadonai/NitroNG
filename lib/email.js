...
export function pitRejectionEmail(name) {
  return emailWrap({
    body: `
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">Hi ${name},</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">Thanks for applying to join the Pit, Nitro's referral crew.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">After reviewing your application, we've decided not to move forward at this time. This isn't permanent — you're welcome to reapply in the future if anything changes.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 24px;">In the meantime, you can still use Nitro to grow your content like thousands of other creators.</p>
      ${emailCTA('https://nitro.ng/dashboard', 'Go to Nitro')}
    `,
  });
}
...
export function sendWinback30Email(name, to, amount, days) {
  ...
  const html = preheader('A little fuel is waiting in your wallet. One week only.') + emailWrap({
    body: `
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">It's been about a month.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">Maybe life happened — fair. But the algorithm didn't pause with you. The accounts you were outpacing are still running the play, and the gap you'd closed is quietly reopening.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">Here's the nudge back: we've loaded <strong style="color:#1a1a1a;">₦${amount.toLocaleString()}</strong> in free promo credit into your wallet. It's live now, it spends on anything, and it expires in <strong>${days} days</strong> — after that it's gone and we won't pretend otherwise.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 24px;">You already know how this works. Pick the post that deserves better numbers, point the credit at it, watch it move.</p>
      ${emailCTA('https://nitro.ng/dashboard?page=order', 'Use my ₦' + amount.toLocaleString() + ' before it expires')}
      <p class="em-m" style="font-size:13px;color:#9a948d;margin:18px 0 0;text-align:center;">— The Nitro Team<br/>Your social media deserves better numbers.</p>
    `,
  });
  return sendEmail(to, subject, html, `We loaded ₦${amount.toLocaleString()} in free promo credit into your Nitro wallet. Expires in ${days} days. Use it: https://nitro.ng/dashboard`);
}

// ═══ WIN-BACK PLAY 7: DAY 60 ═══
export function sendWinback60Email(name, to, amount, days) {
  ...
  const html = preheader('One real offer, bigger than last time, then we go quiet.') + emailWrap({
    body: `
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">Straight up: this is the last win-back we'll send for a while. Nobody grows from being nagged.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 16px;">But before we go quiet — one real offer. <strong style="color:#1a1a1a;">₦${amount.toLocaleString()}</strong> in free promo credit, bigger than last time, live in your wallet for <strong>${days} days</strong>. If the timing was the problem, this is the timing.</p>
      <p class="em-t" style="font-size:15px;line-height:1.7;color:#555;margin:0 0 24px;">If you've genuinely moved on, no hard feelings — the work you put in still counts, and the door stays open. But if the plan was always "I'll get back to it," this is the cheapest week you'll ever pick to get back to it.</p>
      ${emailCTA('https://nitro.ng/dashboard?page=order', 'Claim ₦' + amount.toLocaleString() + ' and pick my post')}
      <p class="em-m" style="font-size:13px;color:#9a948d;margin:18px 0 0;text-align:center;">— The Nitro Team<br/>Your social media deserves better numbers.</p>
    `,
  });
  return sendEmail(to, subject, html, `Last one: ₦${amount.toLocaleString()} in free promo credit, bigger than last time, live in your wallet for ${days} days. https://nitro.ng/dashboard`);
}
