...
export async function sendBrandBlast() {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if ((i + 1) % BATCH === 0 && i + 1 < users.length) {
      console.log(`[Email] Brand blast progress: ${sent} sent, ${failed} failed / ${users.length} total`);
      await new Promise(r => setTimeout(r, DELAY));
    }
  }
  console.log(`[Email] Brand blast complete: ${sent}/${users.length} sent, ${failed} failed`);
  return sent;
}
