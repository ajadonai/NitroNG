...
const SENDER_EMAIL = process.env.SENDER_EMAIL || 'adonaijonathancrypto@gmail.com';
const SENDER_NAME = process.env.SENDER_NAME || 'BoostPanel';
...
export async function sendVerificationEmail(to, name, code) {
  const subject = `${code} is your BoostPanel verification code`;
  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
      ...
      <p style="font-size: 13px; color: #999; text-align: center;">This code expires in 30 minutes. If you didn't sign up for BoostPanel, ignore this email.</p>
      <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; text-align: center;">
        <p style="font-size: 12px; color: #bbb;">BoostPanel — Premium SMM Services</p>
      </div>
    </div>
  `;
...
export async function sendPasswordResetEmail(to, name, resetUrl) {
  const subject = 'Reset your BoostPanel password';
  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px;">
      <div style="text-align: center; margin-bottom: 30px;">
        <div style="width: 48px; height: 48px; border-radius: 14px; background: linear-gradient(135deg, #c47d8e, #8b5e6b); display: inline-flex; align-items: center; justify-content: center; font-size: 22px; font-weight: 700; color: #fff;">B</div>
