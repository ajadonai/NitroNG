  ...
  return `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:18px 0 0;"><tr><td class="bb-wa" style="background:#eaf7ef;border-radius:16px;padding:14px 16px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
      ...
      <td align="right" valign="middle" style="padding-left:10px;">
        <a href="https://whatsapp.com/channel/0029Vb8hC6rJ3jv7Ig2m3D3Q" style="display:inline-block;background:#1faa59;color:#ffffff;font-size:12px;font-weight:800;text-decoration:none;padding:9px 15px;border-radius:99px;white-space:nowrap;">Follow</a>
      </td>
    </tr></table>
  </td></tr></table>`;
}
...
// opts: { kind: 'deposit'|'refund'|'credit', bonus, newBalance, method, orderRef, failReason }
  ...
  if (kind === 'refund') {
    ...
    return emailWrap({
      ...
      body: `
        ...
        ${para(`Hi ${name}, one of your orders couldn't complete, so we sent the money straight back to your wallet. No forms, no waiting.`)}
        ${amountBlock('#059669', `+₦${Number(amount).toLocaleString()}`, 'Refunded to your wallet')}
        ${rows ? emailDataBox(rows) : ''}
        ${emailCTA(`${SITE.url}/dashboard?page=order`, 'Reorder in one tap &#8594;', 'green')}
        ${mutedLine('Refunds land instantly as spendable wallet credit.')}
      `,
    });
  }
