...
// ═══ 19. WELCOME BONUS REMINDER (30d+ idle, no deposit — honest, no countdown) ═══
export function sendBonusReminderEmail(name, to) {
  const subject = 'Your welcome bonus is still waiting';
  const html = preheader("Whenever you're ready, your first deposit still comes with free credit.") + emailWrap({
    pill: 'Bonus',
    pillTone: 'amber',
    body: `
      ${headline(`Still waiting,<br/>no ${em('rush')}.`)}
      ${para(`Hi ${name}, you opened a Nitro account a while back and never got to use it. That's fine, life gets busy.`)}
      ${para(`Just so you know what's still on the table: your first deposit comes with ${bold('free credit, added automatically')}.`)}
      ${tierCards()}
      ${para('No codes, nothing to claim. Fund your wallet and the bonus lands with it.')}
      ${para("Whenever you're ready, your account is right where you left it.", 18)}
      ${emailCTA(`${SITE.url}/dashboard?page=add-funds`, 'Fund my wallet &#8594;')}
      ${signOff()}
    `,
  });
  return sendEmail(to, subject, html, `Hi ${name}, your first deposit still comes with free credit, added automatically. Deposit N2,500+ for N500 free, N5,000+ for N1,200, N10,000+ for N3,000. ${SITE.url}/dashboard?page=add-funds`);
}
...
// 24. Nitro Status + Nitro Points launch (Trip fires this manually)
export function sendStatusLaunchEmail(name, to) {
  const firstName = (name || '').split(' ')[0] || 'there';
  const html = preheader('Automatic discounts and points, starting today.') + emailWrap({
    pill: 'New',
    pillTone: 'amber',
    hero: 'hero-rewards.png',
    body: `
      ${headline(`Every order now gives<br/>${em('something back')}.`)}
      ${para(`Hi ${firstName}, we just switched on two new things for every Nitro account.`, 16)}
      ${launchFeaturePanel('crown', 'Nitro Status is your discount', `The more you spend over time, the higher your level, and the bigger your discount on every order. Up to ${bold('4% off')}, applied by itself. No codes.`)}
      <div style="height:0;font-size:0;">&nbsp;</div>
      ${launchFeaturePanel('coin', 'Nitro Points are your cash back', `Every order earns points. ${bold('1 point is worth ₦1')}, and once you reach 5,000 points you can use them to pay for any order.`)}
      ${emailCTA(`${SITE.url}/dashboard`, 'See my rewards &#8594;')}
      ${mutedLine("Nothing to activate. It's on for every account, starting now.")}
      ${signOff()}
    `,
  });
  return sendEmail(to, 'Every order now earns you something back', html,
    `Hi ${firstName}, Nitro Status and Nitro Points are live. Automatic discounts up to 4% off, plus points on every order (1 point = N1, spend from 5,000). Check your dashboard: ${SITE.url}/dashboard`);
}

// 24b. Nitro Points launch — weekend backfill variant (for users who ordered before points went live)
export function sendWeekendBackfillEmail(name, to, pointsDisplay) {
  const firstName = (name || '').split(' ')[0] || 'there';
  const html = preheader('Automatic discounts and points, starting today.') + emailWrap({
    pill: 'New',
    pillTone: 'amber',
    ...
    body: `
      ...
      <div style="height:0;font-size:0;">&nbsp;</div>
      ${launchFeaturePanel('coin', 'Nitro Points are your cash back', `Every order earns points. ${bold('1 point is worth ₦1')}, and once you reach 5,000 points you can use them to pay for any order.`)}
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:7px 0 20px;"><tr><td class="bb-green" align="center" style="background:#e8f8f0;border-radius:14px;padding:13px 16px;">
        <div class="bb-green-b" style="font-size:13px;font-weight:800;color:#0a7d54;">We already credited ${bold(pointsDisplay + ' points')} for your recent orders.</div>
        <div class="bb-green-t" style="font-size:12px;color:#3f9c74;margin-top:3px;">You ordered before Nitro Points went live, so we counted those too. They're in your dashboard now.</div>
      </td></tr></table>
      ${emailCTA(`${SITE.url}/dashboard`, 'See my rewards &#8594;')}
      ${mutedLine("Nothing to activate. It's on for every account, from today.")}
      ${signOff()}
    `,
  });
  return sendEmail(to, 'Every order now earns you something back', html,
    `Hi ${firstName}, Nitro Status and Nitro Points are live. We already credited ${pointsDisplay} points for your recent orders placed before launch. Check your dashboard: ${SITE.url}/dashboard`);
}
...
// ═══ RETIRED (kept so existing imports don't break; superseded by Play 7 — do not wire) ═══
export function sendNudgeComeback(name, to) {
  const subject = 'Your next order is one tap away';
  const html = emailWrap({
    body: `
      ${para(`Hi ${name},`)}
      ${para("Your last Nitro order was delivered successfully. But you haven't been back since.")}
      ${para('One push is good, but steady growth is what actually moves the needle. The creators who get real results on Nitro are the ones who stay consistent.', 18)}
      ${emailCTA(`${SITE.url}/dashboard?page=order`, 'Order again')}
      ${mutedLine('Same speed. Same dashboard. Same results.')}
    `,
  });
  return sendEmail(to, subject, html, `Hi ${name}, ready to keep the momentum going? ${SITE.url}/dashboard`);
}
...
export function sendNudgeLapsed(name, to) {
  ...
  const html = emailWrap({
    body: `
      ${para(`Hi ${name},`)}
      ${para("We noticed you haven't placed an order in a while.")}
      ${para("Nothing has changed on our end. Same fast delivery, same platforms, same prices. Whenever you're ready, your dashboard is right where you left it.", 18)}
      ${emailCTA(`${SITE.url}/dashboard`, 'Back to your dashboard')}
      ${mutedLine('Need help? Reach us at support@nitro.ng')}
    `,
  });
  return sendEmail(to, subject, html, `Hi ${name}, your Nitro dashboard is ready when you are: ${SITE.url}/dashboard`);
}
