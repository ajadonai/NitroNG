...
export function sendAdActivationDay3(name, to) {
  ...
  const html = preheader('Test Nitro small, watch your numbers move, then decide.') + emailWrap({
    pill: 'Try it small',
    body: `
      ${headline(`Start with ${em('₦500')}. Seriously.`)}
      ${para(`Hi ${firstName}, totally fair to be careful. Plenty of "growth" tools out there don't do anything real, so don't take our word for it. Point ₦500 at one post and watch what happens.`)}
      ...
      ${emailCTA(`${SITE.url}/dashboard?page=add-funds`, 'Try it with ₦500 &#8594;')}
    `,
  });
  return sendEmail(to, 'You can start with ₦500 (seriously)', html,
    `Hi ${firstName}, not sure about Nitro? Start with just N500, point it at one post, and watch what happens. Try it: ${SITE.url}/dashboard?page=add-funds`);
}
...
export function sendStatusLaunchEmail(name, to) {
  ...
  const html = preheader('Automatic discounts and points, starting today.') + emailWrap({
    ...
    body: `
      ...
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:7px 0 20px;"><tr><td class="bb-green" align="center" style="background:#e8f8f0;border-radius:14px;padding:13px 16px;">
        <div class="bb-green-b" style="font-size:13px;font-weight:800;color:#0a7d54;">Your past orders already counted.</div>
        <div class="bb-green-t" style="font-size:12px;color:#3f9c74;margin-top:3px;">Your level and your points are on your dashboard right now.</div>
      </td></tr></table>
  ...
  return sendEmail(to, 'Every order now earns you something back', html,
    `Hi ${firstName}, Nitro Status and Nitro Points are live. Automatic discounts up to 4% off, plus points on every order (1 point = N1, spend from 5,000). Your past orders already counted: ${SITE.url}/dashboard`);
}
