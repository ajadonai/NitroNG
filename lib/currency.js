...
// `active` is the selection gate for the picker. All five are on (12 Sep
// 2026): cedis and shillings became real charge currencies with International
// Nitro step 3, and dollars and pounds ride the same rate. Flipping one to
// false puts a "Soon" tag on it in the picker instead of a tick — canDisplay()
// returns false and CurrencyOptions renders the Soon branch — without touching
// the maths, which is what happened between 9 and 12 Sep.
// Note what activation does and does not mean — a currency here is a unit
// prices are READ in. It is not a currency anyone can pay in: Flutterwave is
// still hardcoded to NGN and the only foreign rail is dollar-denominated USDT,
// so a deposit is charged in naira (or dollars) with the figure shown plainly
// beside the box. The bonus ladders are keyed to the customer's country, not to
// this, so nothing here decides what anyone is paid.
...
/**
 * Which currency a customer is CHARGED in at the gateway, by signup country.
 *
 * This is International Nitro step 3. The wallet and the catalogue stay in
 * naira; a Ghanaian is charged in cedis so that mobile money — how people in
 * Ghana and Kenya actually pay — appears on the Flutterwave page at all (each
 * mobile-money method only shows on a charge in its own currency). The naira
 * credited is fixed at initialise from the same padded deposit rate the
 * display prices use, so what they see is what they pay. Unknown or Nigerian
 * country: naira, exactly as before.
 *
 * This names the country's own currency; whether we may actually charge in it
 * is canChargeIn() below, and callers ask both.
 */
export const COUNTRY_CURRENCY = Object.freeze({ NG: "NGN", GH: "GHS", KE: "KES", GB: "GBP", US: "USD" });
export function chargeCurrencyForCountry(country) {
  return COUNTRY_CURRENCY[String(country || "").toUpperCase()] || BASE_CURRENCY;
}

/**
 * The currencies Flutterwave will actually take money in on our account.
 *
 * Cedis and shillings collect (mobile money proved it, 12 Sep 2026); dollars
 * and pounds do not. "Pay with international card", which the dashboard does
 * have switched on, means a card issued abroad can pay one of OUR charges — it
 * does not mean the charge itself may be denominated in dollars. That needs USD
 * collection, a separate Flutterwave approval we do not have, and without it a
 * USD charge resolves to no method at all: the checkout reads "Oops! No Payment
 * method available. Please engage merchant for support."
 *
 * So USD and GBP are display-only. Someone reading prices in dollars is charged
 * the naira their figure converts to, and their foreign card converts it back
 * at the network rate — the international-card path, which works today. When
 * Flutterwave enables USD collection, add the code here and nothing else moves.
 */
const COLLECTIBLE = new Set(["NGN", "GHS", "KES"]);
export function canChargeIn(code) {
  return COLLECTIBLE.has(String(code || "").toUpperCase());
}

/**
 * The foreign amount to charge for a naira credit, ceiled to the cent so the
 * rounding never favours the payer over the wallet. Null when a rate is
 * missing — callers read that as "charge naira instead", never as zero.
 */
export function foreignChargeAmount(nairaKobo, code, rates = {}) {
  if (code === BASE_CURRENCY) return null;
  const perUnit = nairaPerUnit(code, rates);
  if (!perUnit) return null;
  const naira = Number(nairaKobo) / 100;
  if (!Number.isFinite(naira) || naira <= 0) return null;
  // Ceiled to the currency's own minor unit — pesewas for cedis, whole
  // shillings for Kenya. M-Pesa moves integers, and KES prints without
  // decimals, so the page and the gateway must say the same number; a
  // KSh560.06 quote against a KSh560 or KSh561 charge would verify as a
  // mismatch. The tiny epsilon keeps binary noise (56006.000000000004) from
  // ceiling one unit too high.
  const decimals = CURRENCIES[code]?.decimals ?? 2;
  const scale = 10 ** decimals;
  return Math.ceil((naira / perUnit) * scale - 1e-9) / scale;
}
