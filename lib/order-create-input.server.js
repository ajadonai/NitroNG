 ...
 * @property {number|undefined} catalogueId
...
export function parseCreateOrderInput(body) {
  ...
  const {
    ...
    confirmDuplicate,
    redeemPoints,
    trafficConfig: rawTrafficConfig,
  } = body;

  if (!link || !quantity) {
    return { ok: false, error: 'Link and quantity required' };
  }
  // `catalogueId` is the full list's public service number — the "#2440" the
  // row shows. It is the third way to name a service because it is the only
  // one a customer ever sees; the route resolves it to a serviceId.
  ...
  if (catalogueId !== undefined && (typeof catalogueId !== 'number' || !Number.isInteger(catalogueId) || catalogueId <= 0)) {
    return { ok: false, error: 'Invalid service or tier' };
  }
 ...
 */
// The provider's published targeting list. Two traps ISO validation walks into:
// they use UK not GB, and WW plus 3-letter continent codes are first-class
// options. Kept in lib/traffic-targets.js so the order forms share one source.
export { TRAFFIC_COUNTRIES, TRAFFIC_CONTINENTS } from '@/lib/traffic-targets';
import { TRAFFIC_COUNTRIES as TC, TRAFFIC_CONTINENTS as TCONT } from '@/lib/traffic-targets';

const SUPPORTED_TRAFFIC_TARGETS = new Set([...Object.keys(TC), ...Object.keys(TCONT)]);

// Shared with the admin create-order route: the same traffic order must pass the
// same checks whichever door it comes in through.
export function validateTrafficConfig(raw) {
  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
    return { ok: false, error: 'Invalid traffic config' };
  }
  const { country, device, trafficType, keyword, referrer } = raw;
  if (!country || typeof country !== 'string' || country.trim().length < 2 || country.trim().length > 3) {
    return { ok: false, error: 'Traffic targeting requires a 2-letter country code' };
  }
  const iso = country.trim().toUpperCase();
  if (!SUPPORTED_TRAFFIC_TARGETS.has(iso)) {
    // Caught here rather than by the provider: a rejected dispatch leaves the
    // order stuck in Dispatching with the charge already taken.
    // The provider's list is its own, so a plausible-looking code can still be
    // unsupported — NG is not on it, GB is spelled UK. Say so plainly.
    return {
      ok: false,
      error: `"${iso}" is not a supported target. Use WW for worldwide, a continent (AFR, ASI, EUR, NAM, SAM, MEA), or a country such as US, UK or ZA.`,
    };
  }
  ...
  if (!trafficType || !validTypes.includes(trafficType)) {
    return { ok: false, error: 'Invalid traffic type' };
  }
  if (trafficType === 'keyword' && (!keyword || typeof keyword !== 'string' || !keyword.trim())) {
    return { ok: false, error: 'Google Keyword is required for keyword traffic' };
  }
  if (trafficType === 'referrer' && (!referrer || typeof referrer !== 'string' || !referrer.trim())) {
    return { ok: false, error: 'Referrer URL is required for custom referrer traffic' };
  }
  return {
    ok: true,
    value: {
      ...
      device,
      trafficType,
      ...(trafficType === 'keyword' ? { keyword: keyword.trim().slice(0, 500) } : {}),
      ...(trafficType === 'referrer' ? { referrer: referrer.trim().slice(0, 500) } : {}),
    },
  };
}
...
export function calculateCreateOrderPricing({ tier = null, service, quantity, usdRate }) {
  ...
  const chargeKobo = Math.ceil(Number(sellPer1k) * qty / 100_000) * 100;
  const costKobo = Math.ceil(Number(service.costPer1k) * usdRate * qty / 100_000) * 100;
...
function profileExample(platform) {
  ...
  if (platform.includes('twitch')) return 'https://twitch.tv/yourchannel';
  if (platform.includes('kick')) return 'https://kick.com/yourchannel';
...
/** Lines a provider will accept as a comment: not blank, and carrying at least one letter or digit. Emoji-only lines are rejected upstream. */
...
export function validateCreateOrderOfferInput({ tier = null, service, link, isUrl, comments }) {
  if (tier?.group?.type) {
    ...
    // Twitch and Kick live streams happen AT the channel URL — twitch.tv/name
    // IS the live stream — so live-viewer services need a profile link even
    // though their group type is "views". Without this the validator demanded
    // a /videos/ link and rejected exactly the links the service needs.
    // YouTube/TikTok live are post-style URLs and are not exempted.
    const isChannelLive = /\blive\b/i.test(groupName)
      && (platform.includes('twitch') || platform.includes('kick'));
    const needsProfile = groupType === 'followers' || isMultiPost || isChannelLive;
    const needsPost = ['likes', 'views', 'comments', 'engagement', 'plays'].includes(groupType) && !isMultiPost && !isChannelLive;
  ...
  if (needsCommentText && comments) {
    ...
    if (lineCount < minLines) {
      const typed = comments.split('\n').filter(l => l.trim()).length;
      return {
        ok: false,
        error: `Comments need words: at least ${minLines} lines with words in them, one per line. You have ${lineCount}${typed > lineCount ? ' (lines with only emoji do not count)' : ''}.`,
      };
    }
  }
