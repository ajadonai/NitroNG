...
export async function loadStoredCapiIdentity(db, userId) {
  if (!userId) return {};
  try {
    return await db.user.findUnique({ where: { id: userId }, select: { phone: true, country: true, lastFbp: true, lastFbc: true } }) || {};
  } catch { return {}; }
}
...
export function buildMetaEvent(eventName, opts = {}) {
  ...
  // Meta matches `country` as the hashed lowercase ISO-2 code; sha256 lowercases.
