...
export async function sendEvent(eventName, opts = {}, transportOptions = {}) {
  ...
  try {
    const result = await sendPreparedMetaEvent(event, {
      testEventCode: opts.testEventCode || process.env.META_CAPI_TEST_EVENT_CODE,
      ...transportOptions,
    });
    log.info('MetaCAPI', `${eventName} sent (${opts.eventId})`, { eventsReceived: result.eventsReceived });
    return result;
  } catch (error) {
    const message = safeFailureMessage(error);
    const isTimeout = error?.code === 'timeout';
    log[isTimeout ? 'warn' : 'error']('MetaCAPI', `${eventName} delivery failed (${opts.eventId}): ${message}`);
    reportOperationalFailure('meta_capi_delivery_failed', {
      error,
      data: { eventName, code: error?.code || 'unknown' },
      dedupeKey: `meta_capi_delivery_failed:${eventName.toLowerCase()}`,
...
export async function deliverQueuedMetaEvent(eventId, {
  ...
  try {
    ...
    const isTimeout = error?.code === 'timeout';
    log[isTimeout ? 'warn' : 'error']('MetaCAPI', `${claim.event.eventName} delivery failed (${eventId}): ${message}`, {
      retryAt: nextAttemptAt.toISOString(),
    });
    reportOperationalFailure('meta_capi_delivery_failed', {
      error,
      data: { eventName: claim.event.eventName, attempt: attempts, retryScheduled: updated.count === 1 },
      dedupeKey: `meta_capi_delivery_failed:${claim.event.eventName.toLowerCase()}`,
      level: isTimeout ? 'warning' : 'error',
    });
...
export function scheduleQueuedMetaEventDelivery(eventId, options = {}) {
  ...
  const now = nowInput instanceof Date ? nowInput : new Date(nowInput);
  void Promise.resolve().then(() => db.metaCapiEvent.updateMany({
