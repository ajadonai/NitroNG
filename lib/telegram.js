...
export function tgBonusWithheld(name, email, ip, priorClaims, windowDays, depositKobo, bonusKobo) {
  return send('system', `⚠️ <b>Welcome Bonus Withheld</b>\n${line('User', mask(name))}\n${line('Email', mask(email))}\n${line('IP', ip)}\n${line('Prior claims', `${priorClaims} in ${windowDays}d`)}\n${line('Deposit', naira(depositKobo))}\n${line('Bonus withheld', naira(bonusKobo))}`);
}
