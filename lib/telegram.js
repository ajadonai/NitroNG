...
// ── Revenue ────────────────────────────────────────────
export function tgPayment(name, amountKobo, bonusKobo) {
  send('revenue', `💰 <b>Payment Received</b>\n${line('User', mask(name))}\n${line('Amount', naira(amountKobo))}${bonusKobo ? `\n${line('Bonus', naira(bonusKobo))}` : ''}`);
}
