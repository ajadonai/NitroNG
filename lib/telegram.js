...
export function tgRefund(orderId, amountKobo, reason) {
  return send('revenue', `💸 <b>${naira(amountKobo)}</b> refund\n${id(orderId)}${reason ? `\n${reason}` : ''}`);
}
...
// Cancellations carry a refund, so they belong in the refunds topic — the
// orders topic is new orders and failures only.
export function tgOrderCancelled(orderId, amountKobo, reason) {
  return send('refunds', `❌ <b>${id(orderId)}</b> cancelled\n${naira(amountKobo)} refunded${reason ? `\n${reason}` : ''}`);
}
