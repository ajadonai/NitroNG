...
export function tgFreeOrder({ orderRef, adminName, userName, valueKobo, costKobo, service, quantity, reason, count }) {
  const lines = [
    `🎁 <b>Free order</b> · ${id(orderRef)}${count > 1 ? ` (${count} orders)` : ''}`,
    `${service}${quantity ? ` × ${quantity.toLocaleString()}` : ''}`,
    `Given away: <b>${naira(valueKobo)}</b> of service · costs us ${naira(costKobo)}`,
    `${mask(userName)} · by ${adminName}`,
  ];
...
export function tgDripTimeout(orderId, batch, detail, apiOrderId, provider) {
  ...
  return send('timeout', text);
}

export function tgDispatchFailed(orderId, error) {
  return send('orders', `🔴 <b>${id(orderId)}</b> failed\n${error}`);
}
...
export function tgDigest(date, time, s) {
  return send('pulse', [
    ...
    '',
    '<b>Month to date</b>',
    `  💰 Revenue: <b>${s.monthRevenue}</b> net${s.monthGross ? ` · ${s.monthGross} gross less ${s.monthRefunds} refunded` : ''}`,
 ...
 *
 * The summary topic by default, because that is already where the money
 * questions are answered and a fresh topic cannot be created from code. Point
 * OUTREACH_CONVERSIONS_TOPIC at a dedicated thread once one exists and nothing
 * else has to change.
 */
const CONVERSION_TOPIC = () => Number(process.env.OUTREACH_CONVERSIONS_TOPIC) || OUTREACH_TOPICS.summary;
