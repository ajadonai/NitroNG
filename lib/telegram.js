...
const OUTREACH_TOPICS = { day1: 8, day3: 9, day7: 10, winback: 11, firstDeposit: 13, firstOrder: 14, summary: 400, backlog: 956 };
...
const OUTREACH_MESSAGES = {
  day1: {
    reached: (name) =>
      ...
      + `We spoke briefly today. I saw you signed up yesterday. I help new people get started, so anything you need, just ask me here. You can start with ₦1,000, and your first deposit from ₦2,500 comes with free credit on top.\n\n`
      + `nitro.ng/dashboard`,
    noAnswer: (name) =>
...
export async function tgOutreach(users, touchKey, { label, creditMap } = {}) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const buttons = [
      [{ text: 'Reached', callback_data: `r:${u.id}` }, { text: 'No answer', callback_data: `na:${u.id}` }],
      [{ text: 'DNC', callback_data: `dnc:${u.id}` }, { text: 'Wrong number', callback_data: `wn:${u.id}` }],
    ];
    await sendOutreach(text, topicId, { inline_keyboard: buttons });
