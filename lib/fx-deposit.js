 ...
 *
 *   off  → market, plain: no premium and no subsidy. Until 12 Sep 2026 "off"
 *          meant the legacy crypto rate (`markup_usd_rate`, market PLUS a
 *          ₦200 cushion), which credited every foreign deposit ~13% above
 *          market — the off position of a premium switch was a giveaway.
 *          Legacy is now only the fallback when no market rate exists at all.
...
export function resolveFromSettings(s = {}) {
  ...
  else if (market > 0) { depositRate = round2(market); source = 'market'; }
