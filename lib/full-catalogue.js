...
import { isDripEligible } from '@/lib/drip-feed';
...
/**
 * "Traffic from Instagram" is website traffic. Instagram is where the visitor
 * came from, not the thing being grown — and detectPlatform matches a platform
 * word anywhere in the name, so 31 of these were filing themselves under a
 * social tile. A customer opening Instagram to buy followers found website
 * traffic sitting among them.
 *
 * Tested before the platform words, because the referrer is the only part of
 * these names that is not the product. "LinkedIn Post Clicks [Organic Traffic]"
 * is deliberately untouched: that one really is LinkedIn, and it says traffic
 * about the clicks rather than about a source.
 */
const TRAFFIC_SOURCE = /traffic\s+from\s+|referrer\s+traffic/i;

export function platformOf(name, category) {
  if (TRAFFIC_SOURCE.test(String(name || ''))) return 'webtraffic';
...
/**
 * The same type, from a Service row rather than a label.
 *
 * `buildAll` derives it once for every row it lists, but the order routes only
 * ever hold the Service — they resolve a full-list purchase by its catalogue id
 * and never build the list. Both now read the type from here, so what an order
 * batches by cannot drift from what the row it was bought off said it was.
 */
export function serviceTypeOf(service) {
  if (!service?.name) return '';
  return typeOf(formatResellerService(service.name, service.category).base);
}

/**
 * The platform a full-list row batches under.
 *
 * The stored column, not `category`: drip's per-platform rules key on Nitro
 * tile ids (`telegram`, `discord`), and 2,655 live services file under provider
 * categories that are not platforms at all — "Vip", "Cheapest", a blue circle.
 */
export function servicePlatformOf(service) {
  return String(service?.platform || platformOf(service?.name, service?.category) || '').toLowerCase();
}
...
export function buildAll(services, { usdRate, priceOf = (kobo) => kobo }) {
  ...
  for (const s of services) {
    ...
    rows.push({
      ...
      min: effectiveOrderMinimum(type, s.min, s.max),
      max: s.max,
      unlimited: isUnlimited(s.max),
      refill,
      refillLabel,
      // Whether Nitro will batch this — not `dripfeed`, which is the provider's
      // own drip parameter and a different fact. New Order used to read that
      // one to decide whether to offer the multi-day picker, so the form
      // offered days on rows the server would never schedule.
      drip: isDripEligible({ type, platform }),
      dripfeed: s.dripfeed,
