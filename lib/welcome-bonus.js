...
export async function applyWelcomeBonusDetailed(db, userId, depositAmount, {
  ...
  // Recorded as spend-only credit as well as balance — which is what the
  // top-up bonus and the win-back credit have always done, and what this did
  // not. ₦1,293,050 across 1,349 grants went in as plain money, more than half
  // of every naira held in a wallet, indistinguishable from a real deposit.
  // Nothing paid it out only because no withdrawal path reads user.balance,
  // so the promise that it cannot be withdrawn held by accident.
  //
  // expiresAt null: it never expires. 91.0% of recipients order within 24
  // hours and 94.5% within 30 days, so a deadline would motivate nobody and
  // would put one on 1,277 people's money to recover ₦51,250 from 72 who never
  // ordered at all.
  await db.bonusCredit.create({
    data: { userId, source: 'welcome_bonus', amountGranted: bonus, amountRemaining: bonus, expiresAt: null },
  });
...
const BONUS_PRESETS = [
  ...
  { amount: 5000,  bonus: 600, tag: msg('Best value') },
