...
import { fullListSize, lastKnownFullListSize } from '@/lib/full-list-size';
...
export async function getSiteStats({ includeFullList = false } = {}) {
  ...
  if (includeFullList) {
    ...
    // Out of time, or it came back empty. Fall back to the last size we did
    // measure, which is kept in the database for exactly this. Zero would put
    // "0 more services" inside a sentence boasting about the catalogue.
    if (!full?.services) full = await lastKnownFullListSize();
  }
