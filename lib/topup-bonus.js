...
import { watBounds } from './format.js';
...
// Month boundaries come from the one Lagos-day helper (lib/format.js).
export function lagosMonth(now = new Date()) {
  const { monthStart, nextMonthStart } = watBounds(now);
  const shifted = new Date(now.getTime() + 3_600_000);
