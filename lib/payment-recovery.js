...
export async function recoverStalePendingPayments({ now = new Date() } = {}) {
  ...
  const stats = {
    ...
    verifying: 0,
    retryable: 0,
    // A read we could not trust on a row that is already Expired, Failed or Cancelled. Nothing is owed
  ...
  const TERMINAL = new Set(['Expired', 'Failed', 'Cancelled']);
  const countRetryable = (tx) => { if (TERMINAL.has(tx.status)) stats.retryableTerminal++; else stats.retryable++; };
  ...
  log.info(
    'Payment Recovery',
    `Checked ${stats.checked}: ${stats.recovered} recovered, ${stats.alreadyCredited} already credited, ${stats.pending} pending, ${stats.retryable} retryable (${stats.retryableTerminal} on closed rows), ${stats.failed} failed, ${stats.review} in review, ${stats.refunded} refunded`,
  );
  if (
