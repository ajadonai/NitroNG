...
export async function applyDripRollup(prisma, orderId, dispatches, currentParentStatus) {
  ...
  if (rollup.allDone) {
    return prisma.$transaction(async (tx) => {
      // Lock parent first — shared order-level serialization point.
      // Lock order: parent → children → user (consistent with cancellation).
      ...
      if (!parent || TERMINAL_STATES.includes(parent.status) || parent.deletedAt) return null;
