    ...
    return prisma.$transaction(async (tx) => {
      ...
      const parentRows = await tx.$queryRaw`
        SELECT "id", "userId", "status", "deletedAt",
               "orderId", "charge", "quantity", "nitroPointsRedeemedKobo"
        FROM "orders"
        WHERE "id" = ${orderId}
        FOR UPDATE
      `;
      ...
      // A drip order that finishes Partial owes the customer for what never
      // arrived, exactly as a non-drip one does. This did not happen: the
      // status was set and the points awarded, and the money stayed with us.
      // NTR-8339 was charged ₦23,895 for 500 subscribers, delivered 23, and
      // was never refunded — found because the customer asked.
      //
      // Same arithmetic as the polling cron in app/api/cron/orders/route.js:
      // pro-rata on what is left, floored to whole naira, and netted against
      // anything already refunded so a re-run cannot pay twice.
      if (upd.status === 'Partial') {
        const remains = Number(upd.remains) || 0;
        const want = remains > 0 && parent.charge > 0 && parent.quantity > 0
          ? Math.floor((remains / parent.quantity) * parent.charge / 100) * 100 : 0;
        if (want > 0) {
          const already = await getTotalRefundedKobo(tx, {
            orderId: parent.orderId, orderDbId: orderId, userId: parent.userId,
          });
          const safeRefund = Math.max(0, want - already);
          if (safeRefund > 0) {
            const { walletRefund } = computeRefundSplit(parent.charge, parent.nitroPointsRedeemedKobo, safeRefund);
            if (walletRefund > 0) {
              await tx.$executeRaw`UPDATE users SET balance = balance + ${walletRefund} WHERE id = ${parent.userId}`;
              await tx.transaction.create({
                data: {
                  userId: parent.userId, type: 'refund', amount: walletRefund, method: 'wallet',
                  status: 'Completed', reference: `REF-${parent.orderId}`,
                  note: `Partial delivery refund — ${remains} of ${parent.quantity} not delivered`,
                },
              });
            }
            await reverseOrderPoints(tx, { orderDbId: orderId, refundAmountKobo: safeRefund });
            await tx.order.update({ where: { id: orderId }, data: { refundedAt: new Date() } });
          }
        }
      }
