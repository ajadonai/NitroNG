...
export { STATUS_TIERS, MIN_REDEEM_POINTS, getStatusTiers, getNitroStatus, computeNitroDiscount, computePointsEarnedKobo };
...
export async function awardPointsOnCompletion(orderDbId, tx) {
  ...
  const order = await db.order.findUnique({
    where: { id: orderDbId },
    select: { id: true, orderId: true, userId: true, charge: true, nitroPointsEarnedKobo: true, nitroPointsRedeemedKobo: true, nitroStatusAtPurchase: true, creditUsages: { select: { amount: true } } },
  });
  if (!order || order.nitroPointsEarnedKobo > 0) return 0;
  const bonusUsed = order.creditUsages.reduce((s, u) => s + (u.amount || 0), 0);
  const eligibleCharge = order.charge - (order.nitroPointsRedeemedKobo || 0) - bonusUsed;
