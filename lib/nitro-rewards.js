...
export async function getPointsHistory(userId, limit = 10) {
  const rows = await prisma.nitroPointLedger.findMany({
    ...
    take: limit,
    // The service name makes a row readable: "TikTok video views" beats "Earned".
    include: { order: { select: { orderId: true, tier: { select: { group: { select: { name: true } } } } } } },
  });
  ...
  return rows.map(r => ({
    ...
    // What the points were for, in words the customer recognises.
    detail: r.order?.tier?.group?.name || (r.orderId ? 'An order' : (r.reason || null)),
...
export async function getRewardsPayload(userId) {
  const [spendKobo, balanceKobo, history, taskData] = await Promise.all([
...
async function getTasksSummary(userId) {
  const [activeTasks, userSubs] = await Promise.all([
    prisma.task.findMany({ where: { active: true }, select: { id: true, reward: true, frequency: true, title: true, platform: true, proofType: true } }),
  ...
  const completedOrPending = new Set(userSubs.map(s => s.taskId));
  const available = activeTasks.filter(t => {
    if (t.frequency === 'one_time' && completedOrPending.has(t.id)) return false;
    return true;
  });

  const PROOF_WORD = { link: 'Paste the link', handle: 'Paste your handle', phone: 'Send it on WhatsApp', text: 'Paste what you wrote' };
  const pendingIds = new Set(userSubs.filter(s => s.status === 'pending').map(s => s.taskId));

  return {
    available: available.length,
    topReward: available.length > 0 ? Math.max(...available.map(t => Math.floor(t.reward / 100))) : 0,
    // Enough for the Rewards page to list the first few rather than a bare count.
    list: available.slice(0, 3).map(t => ({
      id: t.id,
      title: t.title,
      platform: t.platform,
      reward: Math.floor(t.reward / 100),
      hint: PROOF_WORD[t.proofType] || 'Send proof',
      waiting: pendingIds.has(t.id),
    })),
  };
}
...
export async function awardOrderPoints(tx, { userId, orderDbId, chargeKobo, tier }) {
...
export function computeRefundSplit(charge, nitroPointsRedeemedKobo, refundAmountKobo) {
  ...
  return {
    walletRefund: Math.floor(walletPaid * fraction / 100) * 100,
    pointsRestore: Math.floor(redeemed * fraction / 100) * 100,
  };
}
