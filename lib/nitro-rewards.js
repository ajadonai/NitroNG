...
export async function reverseOrderPoints(tx, { orderDbId, refundAmountKobo }) {
  ...
  const order = await tx.order.findUnique({
    where: { id: orderDbId },
    select: { charge: true, nitroPointsEarnedKobo: true, userId: true, nitroStatusAtPurchase: true },
  });
  if (!order || order.nitroPointsEarnedKobo <= 0 || order.charge <= 0) return 0;
  ...
  if (maxReversible <= 0) return 0;

  const proportional = Math.floor(order.nitroPointsEarnedKobo * refundAmountKobo / order.charge);
  const reverseKobo = Math.min(maxReversible, Math.max(0, proportional));
  if (reverseKobo <= 0) return 0;
