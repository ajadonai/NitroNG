...
export function createRateLimiter({
  ...
  const reportUnavailable = reason => {
    ...
    try {
      logger.warn?.('[RateLimit] Distributed rate limiting unavailable', {
        reason,
      });
    } catch {}
  ...
  return async function configuredRateLimit(
    ...
    // Fail open onto the in-memory limiter, never closed. Failing closed here
    // blocked every order in production when the Redis quota ran out: rate
    // limiting exists to protect revenue, and a protection that stops the sale
    // is worse than the abuse it guards against. Per-instance memory limits are
    // weaker (each serverless instance counts alone) but still throttle bursts,
    // and the alert below still fires so the outage is seen, not swallowed.
    if (!redisConfigurationAvailable || !redis) {
      if (production) reportUnavailable('redis_not_configured');
