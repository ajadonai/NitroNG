...
if (!redis && process.env.NODE_ENV === 'production' && !globalThis.__rlWarned) {
  globalThis.__rlWarned = true;
  console.warn('[RateLimit] Redis not configured — rate limiting is per-instance only');
}
 ...
 */
export async function rateLimit(req, { maxAttempts = 10, windowMs = 60 * 1000, key: customKey } = {}) {
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() ||
             req.headers.get('x-real-ip') ||
             'unknown';
  const route = new URL(req.url).pathname;
  const key = customKey || `rl:${ip}:${route}`;
