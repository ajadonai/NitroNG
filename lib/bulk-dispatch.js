...
export async function placeWithProvider(order) {
  ...
  // Traffic services take their own parameter names and numeric codes, so the
  // translation lives in one place rather than being open-coded per caller.
  const traffic = trafficProviderParams(order.trafficConfig);
  if (traffic) Object.assign(extra, traffic);
