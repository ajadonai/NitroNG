...
export const getCrewSession = cache(async function getCrewSession() {
  ...
  if (session.member.status !== "approved" || session.member.deletedAt) {
    await prisma.crewSession.deleteMany({ where: { memberId: session.member.id } }).catch(() => {});
    return null;
  }
...
export async function getDashboardData(member) {
  ...
  const [earnings, held, pendingPayouts, recentDirect, recentLead, links, clickCount, totalCommissions, activeReferrals, affSettings] = await Promise.all([
    ...
    isChief ? getAffiliateSettings(['affiliate_pro_rate']) : null,
  ]);
  ...
  return {
    ...
    role: member.role,
    tier: { name: member.tier, rate: isChief ? affSettings.affiliate_pro_rate : member.commissionRate },
