...
export const ORDER_CANCELLATION_LEASE_MS = 10 * 60 * 1000;
export const ACCOUNT_DELETION_ACTIVE_ORDER_STATUSES = Object.freeze([
  'Pending',
  'Processing',
  'Dispatching',
  'In progress',
...
export const ORDER_SETTLEMENT_ACCOUNT_STATUSES = Object.freeze(['Active', 'Suspended']);
...
/**
 * How a customer proves it is really them before the account is scheduled for
 * deletion. A password account re-enters the password. An account that signed
 * up with Google has no password — the callback stores '' — and for as long as
 * the route demanded a password match, nothing those customers typed could
 * satisfy it: they could never delete. They type their email address instead.
 * That is a deliberate act, which is all the password step ever was for
 * someone already holding a signed-in session.
 */
export function accountHasPassword(user) {
  return typeof user?.password === 'string' && user.password.length > 0;
}

export async function verifyAccountDeletionProof(user, { password, confirmEmail } = {}, { compare } = {}) {
  if (accountHasPassword(user)) {
    if (!password) return { ok: false, status: 400, error: 'Password required to delete account' };
    if (typeof compare !== 'function' || !(await compare(password, user.password))) {
      return { ok: false, status: 400, error: 'Incorrect password' };
    }
    return { ok: true, method: 'password' };
  }
  const typed = String(confirmEmail || '').trim().toLowerCase();
  const own = String(user?.email || '').trim().toLowerCase();
  if (!typed || !own || typed !== own) {
    return { ok: false, status: 400, error: 'Type your email address exactly as it appears on the account to confirm' };
  }
  return { ok: true, method: 'email' };
}

export function isOrderSettlementAccountEligible(account) {
  return Boolean(
    account
    && ORDER_SETTLEMENT_ACCOUNT_STATUSES.includes(account.status)
    && account.deletedAt == null
    && account.anonymizedAt == null
  );
}

export function isOrderCancellationLeaseActive(order, now = new Date()) {
  if (!order || !(now instanceof Date) || !Number.isFinite(now.getTime())) return false;
  ...
  return Number.isFinite(updatedAt.getTime())
    && updatedAt.getTime() > now.getTime() - ORDER_CANCELLATION_LEASE_MS;
}

export async function lockOrderSettlementAccount(tx, userId) {
  if (!userId) throw new TypeError('User ID is required');

  const [account] = await tx.$queryRaw`
    SELECT id, status, "deletedAt", "anonymizedAt"
    FROM users
    WHERE id = ${userId}
    FOR UPDATE
  `;
  return isOrderSettlementAccountEligible(account) ? account : null;
}
...
/**
 * Close work that was still active when an account entered deletion.
 *
 * Account deletion is not an order cancellation: the original charge remains
 * settled and no refund is created. Existing completed/superseded drip history
 * is retained, while every remaining live slot is sealed so no later cron can
 * dispatch, roll back, or refund it.
 */
export async function completeActiveOrdersForAccountDeletion(tx, userId, completedAt = new Date()) {
  if (!userId) throw new TypeError('User ID is required');
  ...
  const completedOrders = await tx.order.updateManyAndReturn({
    where: {
      userId,
  ...
  if (orderIds.length === 0) return { orders: 0, dispatches: 0 };

  const dispatches = await tx.dripDispatch.updateMany({
    where: {
      orderId: { in: orderIds },
  ...
  return { orders: orderIds.length, dispatches: dispatches.count };
}
...
export async function withAccountDeletionRetry(db, work) {
...
export async function finalizeAccountDeletion(db, userId, now = new Date()) {
  ...
  return withAccountDeletionRetry(db, async tx => {
    ...
    // Repair legacy deletion requests that reached their deadline with work
    // still open. This uses the same no-refund policy as the request endpoint.
    await completeActiveOrdersForAccountDeletion(tx, user.id, now);
