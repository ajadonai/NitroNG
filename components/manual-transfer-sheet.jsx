        ...
        {manualDone ? (
          <>
            ...
            <div className="flex items-start gap-2.5 py-2.5 px-3 rounded-[11px] text-[12px] leading-relaxed" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.02)", border: `1px solid ${t.cardBorder}`, color: t.textMuted }}>
              ...
              <span>{tr("You can close this. It shows as")} <b className="font-semibold">{tr("Waiting to be confirmed")}</b> {tr("on your wallet until it clears.")}</span>
            </div>
            <div className="flex max-[380px]:flex-col gap-2">
              <button onClick={() => setManualModal(null)} className="flex-1 max-[380px]:flex-none h-11 rounded-xl bg-transparent text-sm font-semibold cursor-pointer text-t-text-muted" style={{ border: `1px solid ${t.cardBorder}` }}>{tr("Done")}</button>
              {onPlaceOrder && <button onClick={() => { setManualModal(null); onPlaceOrder(); }} className="flex-[1.6] max-[380px]:flex-none h-11 rounded-xl border-none bg-gradient-to-br from-[#c47d8e] to-[#8b5e6b] text-white text-sm font-bold cursor-pointer">{tr("Place an order")}</button>}
            </div>
          </>
        ) : manualStep === "details" ? (
          <>
            <div className="flex items-start gap-2.5">
              <div className="min-w-0 flex-1">
                <div className="text-[17px] max-[380px]:text-[15.5px] font-bold leading-tight text-t-text">{tr("Bank transfer")}</div>
                <div className="text-[12.5px] leading-relaxed mt-1 text-t-text-muted">{tr("Send the exact amount, then tell us.")}</div>
              </div>
              <button onClick={() => setManualModal(null)} aria-label={tr("Close")} className="w-8 h-8 rounded-[9px] flex items-center justify-center shrink-0 cursor-pointer text-t-text-muted" style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.03)", border: `1px solid ${t.cardBorder}` }}>
            ...
            <div className="rounded-[13px] overflow-hidden" style={{ border: `1px solid ${t.cardBorder}` }}>
              <div className="py-2.5 px-3.5">
                <div className="text-[10px] font-extrabold uppercase tracking-[.9px] text-t-text-muted">{tr("Bank")}</div>
              ...
              <div className="py-2.5 px-3.5" style={{ borderTop: `1px solid ${t.cardBorder}` }}>
                <div className="text-[10px] font-extrabold uppercase tracking-[.9px] text-t-text-muted">{tr("Account name")}</div>
            ...
            <div className="flex flex-col gap-1.5">
              {[
                [<>{tr("Send that exact amount to the account above.")}</>, "1"],
                [<>{tr("Come back and press")} <b className="font-semibold text-t-text">{tr("I’ve sent it")}</b>.</>, "2"],
            ...
            <div className="flex items-start gap-2.5 py-2.5 px-3 rounded-[11px] text-[12px] leading-relaxed" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.02)", border: `1px solid ${t.cardBorder}`, color: t.textMuted }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="shrink-0 mt-0.5"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4M12 8h.01" /></svg>
              <span>{tr("Take your time — this stays here until you tell us it’s sent.")}</span>
            </div>

            <button onClick={() => setManualStep("confirm")} className="h-11 dash-btn-primary border-none bg-gradient-to-br from-[#c47d8e] to-[#8b5e6b] text-white text-sm font-bold cursor-pointer">{tr("I’ve sent it")}</button>
            <button onClick={async () => { try { await fetch("/api/payments/manual", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reference: manualModal.reference }) }); } catch {} setManualModal(null); onRefresh?.(); }} className="h-9 rounded-[10px] bg-transparent border-none text-[13px] font-semibold cursor-pointer text-t-text-muted">{tr("Cancel this transfer")}</button>
          </>
        ) : (
          <>
            <div className="flex items-start gap-2.5">
              <div className="min-w-0 flex-1">
                <div className="text-[17px] max-[380px]:text-[15.5px] font-bold leading-tight text-t-text">{tr("Who sent it?")}</div>
                <div className="text-[12.5px] leading-relaxed mt-1 text-t-text-muted">{tr("We match your transfer by the name on the sending account.")}</div>
              </div>
              <button onClick={() => setManualModal(null)} aria-label={tr("Close")} className="w-8 h-8 rounded-[9px] flex items-center justify-center shrink-0 cursor-pointer text-t-text-muted" style={{ background: dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.03)", border: `1px solid ${t.cardBorder}` }}>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="manual-sender" className="text-[10px] font-extrabold uppercase tracking-[.9px] text-t-text-muted">{tr("Name on the account you sent from")}</label>
              ...
              <p className="text-[11.5px] leading-snug text-t-text-muted">{tr("Not your Nitro name — the name your bank shows on the transfer. If a friend sent it for you, put their name.")}</p>
            </div>

            <div className="rounded-[13px] overflow-hidden" style={{ border: `1px solid ${t.cardBorder}` }}>
              <div className="flex items-center justify-between gap-2 py-2.5 px-3.5">
                <span className="text-[10px] font-extrabold uppercase tracking-[.9px] text-t-text-muted">{tr("Amount sent")}</span>
              ...
              <div className="flex items-center justify-between gap-2 py-2.5 px-3.5" style={{ borderTop: `1px solid ${t.cardBorder}` }}>
                <span className="text-[10px] font-extrabold uppercase tracking-[.9px] shrink-0 text-t-text-muted">{tr("To")}</span>
            ...
            <div className="flex items-start gap-2.5 py-2.5 px-3 rounded-[11px] text-[12px] leading-relaxed" style={{ background: dark ? "rgba(251,191,36,.1)" : "rgba(217,119,6,.07)", border: `1px solid ${dark ? "rgba(252,211,77,.3)" : "rgba(180,83,9,.24)"}`, color: t.text }}>
              ...
              <span>{tr("Only press this after the money has actually left your account. A transfer we can’t find is rejected.")}</span>
            </div>
