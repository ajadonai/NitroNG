...
function ResellersInner() {
  ...
  const WaButton = ({ label = 'Message us on WhatsApp' }) => (
    <a href={waLink} target="_blank" rel="noopener noreferrer"
      ...
      className="inline-flex items-center gap-2 px-7 py-3 rounded-full text-[14px] font-semibold text-white no-underline transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#c47d8e]/50"
      ...
      <div className="min-h-dvh flex flex-col font-[Plus_Jakarta_Sans,system-ui,sans-serif]" style={{ background: t.bg }}>
        ...
        <main className="flex-1 px-6 pb-20 max-w-[860px] mx-auto w-full">
          ...
          <section className="mb-14 max-md:mb-10">
            <h2 className="text-[22px] font-semibold mb-5 max-md:text-[19px]" style={{ color: t.text, fontFamily: "'Cormorant Garamond',serif" }}>What you get</h2>
            <div className="grid grid-cols-2 max-md:grid-cols-1 gap-4">
              {perks.map(p => (
                <div key={p.title} className="rounded-2xl p-5" style={{ background: cardBg, border: `1px solid ${border}` }}>
                  <div className="text-[14.5px] font-semibold mb-1.5" style={{ color: t.text }}>{p.title}</div>
                  <div className="text-[13.5px] leading-relaxed" style={{ color: t.textSoft }}>{p.body}</div>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* How it works */}
          <section className="mb-14 max-md:mb-10">
            <h2 className="text-[22px] font-semibold mb-5 max-md:text-[19px]" style={{ color: t.text, fontFamily: "'Cormorant Garamond',serif" }}>How it works</h2>
            <div className="flex flex-col gap-3">
              {steps.map(s => (
                <div key={s.n} className="flex items-start gap-4 rounded-2xl p-5" style={{ background: subtleBg, border: `1px solid ${border}` }}>
                  <span className="w-8 h-8 rounded-full flex items-center justify-center text-[14px] font-bold flex-shrink-0" style={{ background: accent, color: '#fff' }}>{s.n}</span>
                  <div>
                    <div className="text-[14.5px] font-semibold mb-1" style={{ color: t.text }}>{s.title}</div>
                    <div className="text-[13.5px] leading-relaxed" style={{ color: t.textSoft }}>{s.body}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Who it's for */}
          <section className="mb-14 max-md:mb-10">
            <h2 className="text-[22px] font-semibold mb-5 max-md:text-[19px]" style={{ color: t.text, fontFamily: "'Cormorant Garamond',serif" }}>Built for</h2>
            <div className="grid grid-cols-3 max-md:grid-cols-1 gap-4">
              {fits.map(f => (
                <div key={f.title} className="rounded-2xl p-5" style={{ background: cardBg, border: `1px solid ${border}` }}>
                  <div className="text-[14px] font-semibold mb-1.5" style={{ color: accent }}>{f.title}</div>
          ...
          <section className="mb-14 max-md:mb-10 rounded-2xl p-6 max-md:p-5" style={{ background: cardBg, border: `1px solid ${border}` }}>
            ...
            <p className="text-[13.5px] leading-relaxed mb-2" style={{ color: t.textSoft }}>
            ...
            <p className="text-[13.5px] leading-relaxed" style={{ color: t.textSoft }}>
          ...
          <section className="text-center rounded-2xl py-10 px-6" style={{ background: subtleBg, border: `1px solid ${border}` }}>
            ...
            <p className="text-[14px] mb-6 max-w-[440px] mx-auto" style={{ color: t.textSoft }}>
