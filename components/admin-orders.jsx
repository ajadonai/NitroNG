      ...
      {redispatchPrompt && (() => {
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ background: "rgba(0,0,0,.5)" }} onClick={() => setRedispatchPrompt(null)}>
          <div className="w-full max-w-[400px] mx-4 rounded-xl p-5" style={{ background: dark ? "#1e1e1e" : "#fff", border: `1px solid ${t.cardBorder}` }} onClick={e => e.stopPropagation()}>
            ...
            <div className="text-[12px] mb-4" style={{ color: t.textMuted }}>Customer: {rd.user} · Charge: {fN(rd.charge)}</div>
            {hasSwap && (
              <div className="text-[12px] mb-4 py-2.5 px-3 rounded-lg flex items-center gap-2" style={{ background: dark ? "rgba(96,165,250,.08)" : "rgba(37,99,235,.05)", border: `1px solid ${dark ? "rgba(96,165,250,.25)" : "rgba(37,99,235,.18)"}`, color: dark ? "#60a5fa" : "#2563eb" }}>
                ...
                <span>Service updated <span className="font-mono font-semibold">{rd.serviceApiId}</span> → <span className="font-mono font-semibold">{rd.tierServiceApiId}</span></span>
              </div>
            )}
            <div className="mb-4">
            ...
            <div className="text-[11px] mb-4 py-2 px-3 rounded-lg" style={{ background: dark ? "rgba(252,211,77,.08)" : "rgba(217,119,6,.05)", color: dark ? "#fcd34d" : "#d97706" }}>Customer will be re-charged {fN(rd.charge)}</div>
            <div className="flex justify-end gap-2">
