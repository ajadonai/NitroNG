...
export default function AdminOrdersPage({ dark, t }) {
  ...
  const doRedispatch = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast.error("Re-dispatch failed", data.error || "Something went wrong"); return; }
      toast.success(redispatchPrompt.id, data.message || "Re-dispatched");
  ...
  return (
    <>
                        ...
                        {expandedBatchOrder === o.id && (
                          <div className="py-3 px-3 desktop:py-3.5 desktop:px-4 pl-4 desktop:pl-5" style={{ background: dark ? "rgba(196,125,142,.05)" : "rgba(196,125,142,.04)", borderTop: `1px solid ${dark ? "rgba(196,125,142,.2)" : "rgba(196,125,142,.15)"}`, borderBottom: `3px solid ${dark ? "rgba(196,125,142,.25)" : "rgba(196,125,142,.2)"}`, borderLeft: `3px solid ${t.accent}` }}>
                            ...
                            <div className="flex items-center gap-2 flex-wrap py-1.5 px-1">
                              ...
                              {(o.status === "Completed" || o.status === "Partial") && <>{o.refundedTotal > 0 && <span className="text-[10px] font-medium rounded-md py-0.5 px-1.5" style={{ background: dark ? "rgba(251,191,36,.1)" : "rgba(217,119,6,.06)", color: dark ? "#fbbf24" : "#d97706" }}>{Math.round(o.refundedTotal / o.charge * 100)}% refunded</span>}<button onClick={() => openRefund(o)} className="m flex items-center gap-1.5 text-[12px] font-semibold cursor-pointer border-none rounded-lg py-1.5 px-2.5" style={{ background: dark ? "rgba(52,211,153,.1)" : "rgba(5,150,105,.06)", color: dark ? "#34d399" : "#059669" }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>{o.refundedTotal >= o.charge ? "Fully Refunded" : "Refund"}</button></>}
              ...
              {expanded === o.id && (
                <div className="py-3.5 px-3.5 desktop:py-4 desktop:px-5" style={{ background: dark ? "rgba(196,125,142,.05)" : "rgba(196,125,142,.04)", borderTop: `1px solid ${dark ? "rgba(196,125,142,.2)" : "rgba(196,125,142,.15)"}`, ...((o.status === "Processing" || o.status === "Pending") ? { borderBottom: `3px solid ${dark ? "rgba(196,125,142,.25)" : "rgba(196,125,142,.2)"}`, borderLeft: `3px solid ${t.accent}` } : {}) }}>
                  ...
                  <div className="flex items-center gap-2 flex-wrap py-1.5 px-1">
                    ...
                    {(o.status === "Completed" || o.status === "Partial") && <>{o.refundedTotal > 0 && <span className="text-[10px] font-medium rounded-md py-0.5 px-1.5" style={{ background: dark ? "rgba(251,191,36,.1)" : "rgba(217,119,6,.06)", color: dark ? "#fbbf24" : "#d97706" }}>{Math.round(o.refundedTotal / o.charge * 100)}% refunded</span>}<button onClick={() => openRefund(o)} className="m flex items-center gap-1.5 text-[12px] font-semibold cursor-pointer border-none rounded-lg py-1.5 px-2.5" style={{ background: dark ? "rgba(52,211,153,.1)" : "rgba(5,150,105,.06)", color: dark ? "#34d399" : "#059669" }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>{o.refundedTotal >= o.charge ? "Fully Refunded" : "Refund"}</button></>}
      ...
      {refundPrompt && (() => {
        ...
        const greenBg = dark ? "rgba(52,211,153," : "rgba(5,150,105,";
        return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ background: "rgba(0,0,0,.5)" }} onClick={() => setRefundPrompt(null)}>
          <div className="w-full max-w-[400px] mx-4 rounded-xl p-5" style={{ background: dark ? "#1e1e1e" : "#fff", border: `1px solid ${t.cardBorder}` }} onClick={e => e.stopPropagation()}>
            ...
            <div className="text-[12px] mb-3" style={{ color: t.textMuted }}>Customer: {refundPrompt.user} · Charged: {fN(refundPrompt.charge)}</div>

            {alreadyRefunded > 0 && (
              <div className="text-[12px] py-2 px-3 rounded-lg mb-3 flex items-center gap-1.5" style={{ background: dark ? "rgba(251,191,36,.08)" : "rgba(217,119,6,.05)", color: dark ? "#fbbf24" : "#d97706" }}>
            ...
            <div className="flex gap-2 mb-4">
              {[25, 50, 100].map(pct => {
                ...
                return (
                  <button key={pct} onClick={() => !disabled && setRefundPercent(pct)} disabled={disabled} className="flex-1 py-2.5 rounded-lg text-center cursor-pointer border transition-all duration-150" style={{ background: active ? `${greenBg}.15)` : "transparent", borderColor: active ? green : (dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"), color: active ? green : disabled ? (dark ? "rgba(255,255,255,.2)" : "rgba(0,0,0,.15)") : t.textMuted, opacity: disabled ? .4 : 1, cursor: disabled ? "not-allowed" : "pointer" }}>
                    <div className="text-sm font-semibold">{pct === 100 ? "Full" : `${pct}%`}</div>
                    <div className="text-[11px] mt-0.5" style={{ opacity: .7 }}>{fN(amt)}</div>
                  </button>
                );
              })}
            </div>

            <div className="py-3 px-3 rounded-lg mb-4 text-center" style={{ background: `${greenBg}.08)`, border: `1px solid ${greenBg}.2)` }}>
              <div className="text-[11px] uppercase tracking-[1px] mb-1" style={{ color: t.textMuted }}>Refund Amount</div>
              <div className="m text-lg font-bold" style={{ color: green }}>{fN(refundPctOf(refundPercent))}</div>
            </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={doRefund} disabled={refundSending || refundPctOf(refundPercent) <= 0} className="py-2 px-4 rounded-lg text-sm font-semibold cursor-pointer border-none transition-all duration-200 hover:-translate-y-px" style={{ background: `${greenBg}.2)`, color: green, opacity: refundSending || refundPctOf(refundPercent) <= 0 ? .5 : 1 }}>{refundSending ? "Processing..." : "Confirm Refund"}</button>
            </div>
          </div>
        </div>
        );
      ...
      {redispatchPrompt && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ background: "rgba(0,0,0,.5)" }} onClick={() => setRedispatchPrompt(null)}>
          <div className="w-full max-w-[400px] mx-4 rounded-xl p-5" style={{ background: dark ? "#1e1e1e" : "#fff", border: `1px solid ${t.cardBorder}` }} onClick={e => e.stopPropagation()}>
            <div className="text-[15px] font-semibold mb-1" style={{ color: t.text }}>Re-dispatch {redispatchPrompt.id}</div>
            <div className="text-[12px] mb-4" style={{ color: t.textMuted }}>Customer: {redispatchPrompt.user} · Charge: {fN(redispatchPrompt.charge)}</div>
            <div className="mb-4">
            ...
            <div className="text-[11px] mb-4 py-2 px-3 rounded-lg" style={{ background: dark ? "rgba(252,211,77,.08)" : "rgba(217,119,6,.05)", color: dark ? "#fcd34d" : "#d97706" }}>Customer will be re-charged {fN(redispatchPrompt.charge)}</div>
            <div className="flex justify-end gap-2">
