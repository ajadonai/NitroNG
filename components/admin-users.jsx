    ...
    <>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-full desktop:min-w-[200px]">
          ...
          <input aria-label="Search users" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name or email..." className="w-full py-[9px] pl-9 pr-8 rounded-lg text-[13px] outline-none font-[inherit]" style={{ border: `1px solid ${t.cardBorder}`, background: dark ? "#131728" : "#fff", color: t.text }} />
          ...
          {loading ? (
            ...
            return (
              <div key={u.id}>
                ...
                {/* Credit panel */}
                {creditId === u.id && (
                  <div className="py-3 px-4 flex gap-2 items-center flex-wrap" style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}`, background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.03)" }}>
                    ...
                    <input type="number" aria-label="Credit amount" placeholder="Amount" value={creditAmt} onChange={e => setCreditAmt(e.target.value)} className="flex-1 min-w-[100px] py-2 px-3 rounded-lg text-[13px] outline-none font-[inherit]" style={{ background: dark ? "#131728" : "#fff", border: `1px solid ${t.cardBorder}`, color: t.text }} />
                ...
                {/* Transactions panel */}
                {txUser?.id === u.id && (
                  <div style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}`, background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.015)" }}>
                    <div className="flex justify-between items-center py-2.5 px-4" style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
                    ...
                    {txLoading ? (
                      ...
                      return (
                        <>
                          ...
                          {txTotalPages > 1 && (
                            <div className="flex justify-between items-center py-2.5 px-4" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
