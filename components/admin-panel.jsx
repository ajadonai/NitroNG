...
export default function AdminPanel(){
  ...
  <nav style={{flex:1,padding:mini?"6px 0 6px 4px":"6px 0 6px 10px",display:"flex",flexDirection:"column",gap:2,overflow:"visible"}}>{NAV.map(([id,ic,lb])=>id.startsWith("---")?<div key={id} style={{height:1,background:t.surfaceBorder,margin:mini?"4px 4px":"4px 14px"}}/>:<button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:11,padding:mini?"8px 0":"8px 14px",width:"100%",textAlign:"left",justifyContent:mini?"center":"flex-start",fontSize:12,fontWeight:pg===id?600:400,marginRight:mini?0:10,overflow:"visible",border:"none",background:pg===id?"linear-gradient(135deg, #c47d8e, #a3586b)":"transparent",color:pg===id?"#fff":t.textSoft,borderLeft:pg===id?"3px solid #8b4a5e":"3px solid transparent",borderRadius:pg===id?(mini?"0 7px 7px 0":"0 10px 10px 0"):"0 9px 9px 0",paddingLeft:pg===id?(mini?"0":"11px"):"14px",...(pg===id?{marginRight:mini?-6:-8,position:"relative",zIndex:3}:{})}}><span style={{fontSize:mini?13:14,width:18,textAlign:"center"}}>{ic}</span><span className="sb-nav-label">{lb}</span></button>)}</nav>
