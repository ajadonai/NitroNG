...
function ExpandedOrderDetails({ o, dark, t, doAction, actionLoading, confirm, compact, toast, onNavigate, waNum }) {
  ...
  const reportIssueButton = waNum ? (
    <a href={`https://wa.me/${waNum}?text=${waMessage}`} target="_blank" rel="noopener noreferrer" className="m flex items-center gap-1.5 text-[13px] font-medium cursor-pointer no-underline border-none bg-transparent p-0" style={{ color: "#25d366" }}>
      ...
      <div className="flex items-center gap-5 py-2 px-1">
        {(o.status === "Processing" || o.status === "Pending") && (
          <>
            ...
            {!o.apiOrderId && <button onClick={async () => { const ok = await confirm({ title: "Cancel Order", message: `Cancel order ${o.id}? Your wallet will be refunded.`, confirmLabel: "Cancel Order", danger: true }); if (ok) doAction(o.id, "cancel"); }} disabled={actionLoading === o.id} className="m flex items-center gap-1.5 text-[13px] font-medium cursor-pointer border-none bg-transparent p-0" style={{ color: dark ? "#fca5a5" : "#dc2626" }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Cancel</button>}
          </>
        )}
        {(o.status === "Completed" || o.status === "Cancelled") && (
          <button onClick={async () => { const ok = await confirm({ title: "Reorder", message: `Reorder ${o.service}? ₦${o.charge?.toLocaleString()} will be charged from your wallet.`, confirmLabel: "Place Reorder" }); if (ok) doAction(o.id, "reorder"); }} disabled={actionLoading === o.id} className="m flex items-center gap-1.5 text-[13px] font-medium cursor-pointer border-none bg-transparent p-0" style={{ color: t.accent }}>{actionLoading === o.id ? <Spinner size={14} color={t.accent} /> : <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>}Reorder</button>
        )}
        {reportIssueButton}
      </div>
    </div>
      ...
      {expanded && (
        <div style={{ background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.02)", borderLeft: `3px solid ${accentColor}`, borderTop: `2px solid ${dark ? "rgba(196,125,142,.28)" : "rgba(196,125,142,.24)"}` }}>
          ...
          <div className="flex items-center gap-5 py-2.5 px-4 desktop:px-5" style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
            ...
            {hasActive && (
              <button onClick={() => doBatchAction(batch.batchId, "check")} disabled={isLoading} className="m flex items-center gap-1.5 text-[12px] font-medium cursor-pointer border-none bg-transparent p-0" style={{ color: dark ? "#a5b4fc" : "#4f46e5", opacity: isLoading ? .5 : 1 }}>
            ...
            {hasCancellable && (
              <button onClick={async () => { const ok = await confirm({ title: "Cancel Batch", message: `Cancel all pending orders in ${batch.batchId} that haven't been sent to providers yet? Your wallet will be refunded.`, confirmLabel: "Cancel All", danger: true }); if (ok) doBatchAction(batch.batchId, "cancel"); }} disabled={isLoading} className="m flex items-center gap-1.5 text-[12px] font-medium cursor-pointer border-none bg-transparent p-0" style={{ color: dark ? "#fca5a5" : "#dc2626", opacity: isLoading ? .5 : 1 }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>Cancel all</button>
            )}
            {hasReorderable && (
              <button onClick={async () => { const ok = await confirm({ title: "Reorder Batch", message: `Reorder all completed/cancelled orders from ${batch.batchId}?`, confirmLabel: "Reorder All" }); if (ok) doBatchAction(batch.batchId, "reorder_completed"); }} disabled={isLoading} className="m flex items-center gap-1.5 text-[12px] font-medium cursor-pointer border-none bg-transparent p-0" style={{ color: t.accent, opacity: isLoading ? .5 : 1 }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0114.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0020.49 15"/></svg>Reorder all</button>
            )}
          </div>
