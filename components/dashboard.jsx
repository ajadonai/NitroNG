        ...
        <div className="fixed inset-0 z-[99998] bg-black/60 flex items-center justify-center p-5">
          <div className="rounded-2xl py-8 px-7 max-w-[420px] w-full shadow-[0_20px_60px_rgba(0,0,0,.3)]" style={{ background: dark ? "#1a1a1a" : "#fff" }}>
            ...
            <button
              ...
              onClick={async () => {
                setPhonePromptError("");
                const checked = validatePhone(phonePromptCc, phonePromptVal);
                if (!checked.ok) { setPhonePromptError(checked.error); return; }
                setPhonePromptSaving(true);
                try {
                  const res = await fetch("/api/auth/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ phone: phonePromptVal, country: phonePromptCc }) });
                  if (res.ok) {
                    const savedPhone = checked.e164;
              ...
              style={{
                cursor: validatePhone(phonePromptCc, phonePromptVal).ok && !phonePromptSaving ? "pointer" : "default",
                background: validatePhone(phonePromptCc, phonePromptVal).ok ? "#25d366" : (dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"),
