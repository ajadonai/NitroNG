            ...
            <div className="w-full max-w-[400px] justify-self-end max-desktop:hidden">
              <form onSubmit={handleHeroAuthSubmit} noValidate className="fu fd2 rounded-[20px] pt-[26px] px-[26px] pb-[22px] relative z-[2]" style={{background:dark?"#160f22":"#fff",border:`1px solid ${dark?"rgba(255,255,255,.14)":"rgba(0,0,0,.09)"}`,boxShadow:"0 30px 80px rgba(0,0,0,.26)"}}>
                ...
                {heroAuth==="signup"&&heroSignupStep===2&&<>
                  ...
                  <label htmlFor="hero-signup-terms" style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:10,cursor:"pointer"}}><input id="hero-signup-terms" name="termsAccepted" type="checkbox" checked={heroAgree} onChange={e=>setHeroAgree(e.target.checked)} style={{marginTop:2,accentColor:t.accent,width:14,height:14,flexShrink:0}}/><span className="text-t-text-soft" style={{fontSize:11,lineHeight:1.4}}>{tr("I agree to the")} <a href="/terms" className="text-accent no-underline">{tr("Terms")}</a> {tr("and")} <a href="/privacy" className="text-accent no-underline">{tr("Privacy Policy")}</a></span></label>
                </>}
                {heroAuth==="login"&&<>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                    ...
                    <button type="button" onClick={()=>{setHeroAuth("forgot");setHeroError("")}} className="text-accent" style={{background:"none",fontSize:13,fontWeight:500}}>{tr(tr("Forgot password?"))}</button>
                  </div>
                </>}
                {heroAuth==="forgot"&&<>
