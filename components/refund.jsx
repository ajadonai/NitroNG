...
function RefundInner(){
  ...
  const sections=[
    ...
    ["Refills","Some services include a refill guarantee. If you lose followers or engagement after delivery, you can request a refill from your dashboard — at no extra cost. Refills are only available on completed orders with an eligible service, and must be requested within the refill window (shown on your order). Refills are not available on services without a refill guarantee. Important: refills only cover drops that occur after your order has been fully delivered above the original start count. If your count falls below the start count — typically caused by platform-wide cleanups, content removal, username changes, or broader engagement reductions — refill coverage does not apply."],
