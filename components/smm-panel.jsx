  ...
  return (
    <div className="root">
      ...
      <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}>
        ...
        <nav style={{flex:1,padding:mini?"6px 0 6px 4px":"6px 0 6px 10px",display:"flex",flexDirection:"column",gap:2,overflow:"visible"}}>{NAV.map(([id,ic,lb])=><button key={id} onClick={()=>go(id)} title={lb} style={{display:"flex",alignItems:"center",gap:11,padding:mini?"9px 0":"9px 14px",width:"100%",textAlign:"left",justifyContent:mini?"center":"flex-start",fontSize:13,fontWeight:pg===id?600:400,marginRight:mini?0:10,overflow:"visible",border:"none",background:pg===id?"linear-gradient(135deg, #c47d8e, #a3586b)":"transparent",color:pg===id?"#fff":t.textSoft,borderLeft:pg===id?"3px solid #8b4a5e":"3px solid transparent",borderRadius:pg===id?(mini?"0 8px 8px 0":"0 12px 12px 0"):"0 9px 9px 0",paddingLeft:pg===id?(mini?"0":"11px"):"14px",...(pg===id?{marginRight:mini?-6:-8,position:"relative",zIndex:3}:{})}}><span style={{fontSize:mini?14:15,width:20,textAlign:"center"}}>{ic}</span><span className="sb-nav-label">{lb}</span></button>)}</nav>
