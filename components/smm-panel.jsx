...
function Dash({user,orders,txs,go,t,dark}){
  ...
  <div style={{display:"flex",flexDirection:"column",gap:16}}><Card d={.25} dark={dark}><h3 style={{fontSize:15,fontWeight:600,color:t.text,marginBottom:12}}>Quick Actions</h3>{[["Place New Order","",t.accent,"new-order"],["Add Funds","",t.green,"funds"],["Share Referral","","#e0a458","referrals"]].map(([l,ic,c,p])=><button key={l} onClick={()=>go(p)} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:10,background:t.btnSecondary,border:`1px solid ${t.btnSecBorder}`,color:t.text,fontSize:13,fontWeight:500,width:"100%",textAlign:"left",marginBottom:6}}>{l}</button>)}</Card>
    ...
    <div className="fg">
      <div>
        ...
        {payState==="idle"&&<Card dark={dark}>
          ...
          {amt>=500&&<div style={{padding:14,borderRadius:12,background:dark?"#0a0d18":"#faf8f5",border:`1px solid ${t.surfaceBorder}`,marginBottom:14}}>
