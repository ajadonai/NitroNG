  ...
  return (
    <div className="root">
      ...
      <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}>
        <div style={{padding:mini?"12px 0":"20px 16px 18px",borderBottom:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between",flexDirection:mini?"column":"row",gap:mini?6:0}}>
        {!mini&&<button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:10,background:"none",padding:0,border:"none",outline:"none",cursor:"pointer"}}><div style={{width:34,height:34,borderRadius:10,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff"}}>B</div><div className="sb-hide"><div className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:10,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Premium SMM</div></div></button>}
        ...
        {!mini&&<div className="sb-balance" style={{padding:"12px 18px",borderBottom:`1px solid ${t.surfaceBorder}`}}><div style={{fontSize:9,color:t.textMuted,fontWeight:600,textTransform:"uppercase",letterSpacing:2}}>Balance</div><div className="m" style={{fontSize:18,fontWeight:700,color:t.green,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fNc(user.balance)}</div></div>}
