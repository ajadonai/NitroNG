...
export default function App() {
  ...
  return (
    <div className="root">
      ...
      <div className="mh"><button onClick={()=>setSb(true)} style={{background:"none",color:t.text,fontSize:22,padding:4}}>☰</button><button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:8,background:"none",padding:0}}><div style={{width:28,height:28,borderRadius:8,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#fff",fontWeight:700}}>B</div><span className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</span></button><div className="m" style={{fontSize:12,fontWeight:600,color:t.green}}>{fN(user.balance)}</div></div>
      {sb&&<div className="ov" onClick={()=>setSb(false)}/>}
      <aside className={`sb${sb?" open":""}${mini?" collapsed":""}`}>
        <div style={{padding:mini?"16px 10px 12px":"22px 18px 14px",borderBottom:`1px solid ${t.surfaceBorder}`,display:"flex",alignItems:"center",justifyContent:mini?"center":"space-between"}}>{!mini&&<button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{display:"flex",alignItems:"center",gap:10,background:"none",padding:0}}><div style={{width:34,height:34,borderRadius:10,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff"}}>B</div><div className="sb-hide"><div className="serif" style={{fontSize:17,fontWeight:600,color:t.text}}>BoostPanel</div><div style={{fontSize:10,color:t.textMuted,letterSpacing:2,textTransform:"uppercase"}}>Premium SMM</div></div></button>}{mini&&<button onClick={()=>window.scrollTo({top:0,behavior:"smooth"})} style={{background:"none",padding:0}}><div style={{width:34,height:34,borderRadius:10,background:t.logoGrad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:"#fff"}}>B</div></button>}<button className="sb-close" onClick={()=>setSb(false)} style={{background:"none",color:t.textMuted,fontSize:20,padding:4,alignItems:"center",justifyContent:"center"}}>✕</button>{!mini&&<button className="sb-collapse" onClick={()=>setMini(true)} style={{background:t.btnSecondary,color:t.textSoft,fontSize:14,padding:"6px 8px",alignItems:"center",justifyContent:"center",borderRadius:6,border:`1px solid ${t.btnSecBorder}`}}>⟨⟨</button>}</div>
