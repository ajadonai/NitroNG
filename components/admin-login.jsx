...
export default function AdminLogin(){
  ...
  const ThemeToggleBtn=()=>(
    <button onClick={toggleTheme} aria-label={dark?"Switch to light":"Switch to dark"} className="theme-toggle w-[52px] h-7 rounded-[14px] relative transition-[background] duration-[0.8s] ease shrink-0" style={{background:dark?"rgba(99,102,241,.25)":"rgba(255,255,255,.2)",border:`1px solid ${dark?"rgba(99,102,241,.2)":"rgba(255,255,255,.25)"}`}}>
  ...
  return (
    <div className="transition-[background,color] duration-[1.2s] ease min-h-dvh flex flex-row relative overflow-hidden">
      ...
      <div className="w-[42%] max-lg:w-[38%] max-md:!hidden py-10 px-10 max-lg:py-8 max-lg:px-7 relative overflow-hidden flex flex-col justify-between" style={{background:t.panelBg,borderRight:`1px solid ${dark?"rgba(255,255,255,.06)":"rgba(255,255,255,.1)"}`}}>
        {/* Orbs + Grid pattern */}
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col" style={{background:t.heroBg}}>
        ...
        {/* Mobile: full nav */}
        <nav className="hidden max-md:!flex items-center justify-between h-[52px] shrink-0 z-10 px-5" style={{borderBottom:`1px solid ${dark?"rgba(255,255,255,.06)":"rgba(255,255,255,.12)"}`}}>
        ...
        <div className="admin-center flex-1 flex items-center justify-center relative z-[1]">
          <div className="fu w-full max-w-[400px] max-lg:max-w-[380px] max-md:max-w-full py-9 px-8 max-lg:py-8 max-lg:px-7 max-md:py-7 max-md:px-[22px] rounded-[20px] max-md:rounded-[18px] backdrop-blur-[20px] [&_input]:transition-[border-color] [&_input]:duration-200 [&_input:focus]:!border-accent" style={{background:t.cardBg,border:`1px solid ${t.cardBorder}`,boxShadow:dark?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(0,0,0,0.1)"}}>

            <div className="flex justify-center mb-5">
              <div className="inline-flex items-center gap-1.5 py-1 px-3.5 rounded-[20px]" style={{background:t.accentLight,border:`1px solid ${dark?"rgba(196,125,142,.15)":"rgba(196,125,142,.1)"}`}}>
            ...
            <p className="text-[15px] text-center mb-6 font-[450]" style={{color:t.textSoft}}>Authorized operators only.</p>

            <div className="min-h-9 mb-1 flex items-center">{error?<div className="w-full py-2 px-3 rounded-lg text-[13px] leading-[1.2]" style={{background:dark?"rgba(220,38,38,0.1)":"#fef2f2",border:`1px solid ${dark?"rgba(220,38,38,0.2)":"#fecaca"}`,color:t.red}}>⚠️ {error}</div>:null}</div>
            ...
            <button onClick={handleLogin} disabled={loading} className="w-full py-3.5 rounded-xl text-white text-base font-semibold mb-4 flex items-center justify-center gap-2 border-none tracking-[0.5px]" style={{background:loading?"#999":t.btnPrimary,opacity:loading?.7:1,boxShadow:loading?"none":"0 6px 24px rgba(196,125,142,.35)"}}>
              {loading&&<span className="w-4 h-4 rounded-full animate-[spin_0.6s_linear_infinite]" style={{border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff"}}/>}
              {loading?"Authenticating...":"Access Dashboard"}
            </button>

            <div className="text-center pt-3" style={{borderTop:`1px solid ${t.cardBorder}`}}>
              <p className="text-xs mb-2" style={{color:t.textMuted}}>🔒 Access attempts are logged.</p>
