      ...
      <div className="sup-split-list w-[280px] shrink-0 overflow-hidden" style={{ borderRight: `1px solid ${t.cardBorder}` }}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {filtered.map(tk => {
            ...
            return (
              <div key={tk.id} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.currentTarget.click()}}} onClick={() => selectTicket(tk)} className="flex items-center gap-2.5 py-2.5 px-3.5 cursor-pointer" style={{ borderBottom: `1px solid ${t.cardBorder}`, background: isSel ? (dark ? "rgba(196,125,142,.1)" : "rgba(196,125,142,.07)") : "transparent", borderLeft: isSel ? `3px solid ${t.accent}` : "3px solid transparent", opacity: tk.status === "Resolved" && !isSel ? 0.45 : 1 }}>
        ...
        {selected ? <>
          <div className="shrink-0 h-[56px] flex items-center gap-2.5 px-[18px]" style={{ borderBottom: `1px solid ${t.cardBorder}`, background: dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)" }}>
            ...
            <div className="flex gap-1.5 shrink-0">
              ...
              <button className="rounded-lg py-1.5 px-2 cursor-pointer shrink-0 transition-transform duration-200 hover:-translate-y-px" onClick={() => setShowInfo(!showInfo)} style={{ background: showInfo ? (dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)") : (dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.04)"), border: `1px solid ${showInfo ? t.accent : t.cardBorder}`, color: showInfo ? t.accent : t.textMuted }}>
          ...
          {selected.status !== "Resolved" && selected.status !== "Archived" ? (
            <>
              ...
              <div className="py-2.5 px-4 flex gap-2 items-end shrink-0" style={{ borderTop: claimedByOther ? "none" : `1px solid ${t.cardBorder}` }}>
                <textarea value={reply} onChange={e => { setReply(e.target.value); e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 120) + "px"; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); doReply(); e.target.style.height = "42px"; } }} placeholder={`Reply to ${selected.user?.split(" ")[0]}...`} rows={1} className="flex-1 py-2.5 px-3.5 rounded-[20px] text-sm outline-none font-[inherit] resize-none leading-[1.5]" style={{ background: dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.04)", border: `1px solid ${t.cardBorder}`, color: t.text, minHeight: "42px", maxHeight: "120px" }} />
                <button onClick={doReply} className="w-[38px] h-[38px] rounded-full border-none flex items-center justify-center shrink-0" style={{ background: reply.trim() ? `linear-gradient(135deg,${t.accent},#a3586b)` : (dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.04)"), cursor: reply.trim() ? "pointer" : "default" }}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-3" style={{ background: dark ? "rgba(255,255,255,.07)" : "rgba(0,0,0,.03)" }}>
      ...
      {selected && showInfo && (
        <div ref={infoPanelRef} className="sup-info-panel w-[240px] py-4 px-3.5 shrink-0 overflow-y-auto" style={{ borderLeft: `1px solid ${t.cardBorder}`, background: t.cardBg || (dark ? '#0e1225' : '#fff') }}>
          ...
          <div className="mb-3 rounded-xl overflow-hidden" style={{ border: `1px solid ${t.cardBorder}` }}>
            <div className="text-[10px] font-semibold uppercase tracking-[1px] py-2 px-3" style={{ color: t.textMuted, background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.02)" }}>Ticket details</div>
          ...
          {selected.activity?.length > 0 && (
            <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${t.cardBorder}` }}>
              <div className="text-[10px] font-semibold uppercase tracking-[1px] py-2 px-3" style={{ color: t.textMuted, background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.02)" }}>Activity</div>
              <div className="px-3 py-2.5 flex flex-col gap-2">
