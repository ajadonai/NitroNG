...
const NAV_ITEMS = [
  { label: "Dashboard", href: "/m", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="9"/><rect x="14" y="3" width="7" height="5"/><rect x="14" y="12" width="7" height="9"/><rect x="3" y="16" width="7" height="5"/></svg> },
  { label: "Tracking Links", href: "/m/links", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>, chiefOnly: true },
  { label: "Team", href: "/m/team", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>, chiefOnly: true },
  { label: "Commissions", href: "/m/commissions", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg> },
  { label: "Payouts", href: "/m/payouts", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg> },
  { label: "Settings", href: "/m/settings", icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> },
];
...
function ShellInner({ children, member }) {
  ...
  const items = NAV_ITEMS.filter(i => !i.chiefOnly || role === "chief");
    ...
    <>
      ...
      <nav className="flex flex-col gap-[2px] mt-[6px]">
        {items.map(item => {
          ...
          return (
            <a key={item.href} href={item.href} onClick={(e) => { e.preventDefault(); router.push(item.href); }} className="flex items-center gap-[11px] py-[9px] px-[11px] rounded-[10px] text-[13.5px] font-medium transition-colors duration-150 nav-link" style={{ color: active ? t.text : t.muted, background: active ? t.accentLight : "transparent", borderLeft: `3px solid ${active ? t.accent : "transparent"}` }}>
      ...
      <div className="mt-auto flex items-center gap-[10px] p-[10px] rounded-xl" style={{ background: t.surface, border: `1px solid ${t.surfaceBrd}` }}>
        ...
        <div className="sidebar-text min-w-0">
          ...
          <div className="text-[10.5px]" style={{ color: t.muted }}>{role === "chief" ? "Crew Chief" : "Crew"}</div>
        </div>
  ...
  return (
    <div className="crew-app" style={{ background: t.bg, color: t.text, minHeight: "100vh" }}>
      ...
      {drawerOpen && (
        <>
          <div className="fixed inset-0 z-40" style={{ background: t.overlay }} onClick={() => setDrawerOpen(false)} />
          <aside className="fixed top-0 left-0 bottom-0 w-[260px] z-50 flex flex-col p-[18px_14px]" style={{ background: t.sidebarBg, backdropFilter: "blur(16px)" }}>
            {navContent}
          </aside>
        </>
      )}
      ...
      <div className="flex flex-col min-w-0">
        ...
        <div className="h-14 flex items-center justify-between px-6 max-md:px-4 sticky top-0 z-30" style={{ background: dark ? "rgba(9,12,21,.85)" : "rgba(240,237,232,.85)", backdropFilter: "blur(16px)", borderBottom: `1px solid ${t.surfaceBrd}` }}>
          <div className="flex items-center gap-3">
            <button className="crew-hamburger bg-transparent border-none flex cursor-pointer" style={{ color: t.text }} onClick={() => setDrawerOpen(true)}>
        ...
        {/* Content */}
        <div className="px-6 max-md:px-4 py-6 max-w-[1120px] w-full mx-auto flex flex-col gap-[26px] max-md:gap-5">
          {children}
        </div>
      </div>
      ...
      <style>{`
        ...
        .crew-hamburger { display: none; }
        .nav-link:hover { color: ${t.soft}; }
        @media (max-width: 1199px) {
          .crew-app { grid-template-columns: 64px 1fr; }
          .sidebar-text { display: none; }
          .crew-sidebar .nav-link { justify-content: center; padding: 11px 0; }
          .crew-sidebar > div:last-child { justify-content: center; padding: 8px; }
          .crew-sidebar > div:last-child button { margin: 0; }
        }
        @media (max-width: 767px) {
          .crew-app { grid-template-columns: 1fr; }
          .crew-sidebar { display: none; }
          .crew-hamburger { display: flex; }
        }
      `}</style>
    </div>
  );
