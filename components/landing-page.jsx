...
function LandingInner(){
  ...
  const initVia=typeof window!=="undefined"?new URLSearchParams(window.location.search).get("via")||"":"";
  const [heroAuth,setHeroAuth]=useState(initVia?"signup":"login");
  ...
  useEffect(()=>{const p=new URLSearchParams(window.location.search);if(p.get("reset")){setResetToken(p.get("reset"));setModal("reset");window.history.replaceState({},"","/");return;}if(p.get("login"))setModal("login");if(p.get("signup"))setModal("signup");if(p.get("ref")){setModal("signup");setHeroRefCode(p.get("ref"));};if(p.get("via")&&!initVia){setHeroVia(p.get("via"));setHeroAuth("signup");}if(p.get("session_expired")){setSessionExpired(true);window.history.replaceState({},"","/");}if(p.get("logout")){setLogoutMsg(true);window.history.replaceState({},"","/");setTimeout(()=>setLogoutMsg(false),4000);}if(p.get("google_error")){setGoogleError(true);window.history.replaceState({},"","/");setTimeout(()=>setGoogleError(false),5000);setModal("login");}if(p.get("error")==="account_pending_deletion"){setHeroError("Account pending deletion. Contact support@nitro.ng.");window.history.replaceState({},"","/");}if(p.get("error")==="disposable_email"){setHeroError("Disposable email addresses aren't allowed. Please sign up with a permanent email.");setModal("signup");window.history.replaceState({},"","/");}if(["google_cancelled","google_state_mismatch","google_token_failed","google_no_email","google_failed","google_missing_params","google_not_configured","google_account_deleted"].includes(p.get("error"))){setGoogleError(true);window.history.replaceState({},"","/");setTimeout(()=>setGoogleError(false),5000);setModal("login");}},[]);
  ...
  return(
    <div className="root h-dvh overflow-hidden flex flex-col" suppressHydrationWarning>
          ...
          <div className={`flex items-center justify-center gap-12 max-desktop:gap-0 pt-14 pb-10 max-desktop:pt-6 max-desktop:pb-4 max-md:pb-2 px-[60px] max-desktop:px-10 max-md:px-5 max-w-[1200px] mx-auto w-full relative z-[1] flex-1 max-desktop:flex-col max-desktop:text-center max-desktop:min-h-0 ${siteAlerts.length > 0 ? "max-md:pt-[50px]" : "max-md:pt-2"}`}>
            ...
            <div className="flex-1 max-w-[540px] text-left relative z-[1] max-desktop:flex-none max-desktop:text-center max-desktop:max-w-full max-desktop:flex max-desktop:flex-col max-desktop:items-center">
              ...
              <div className="fu fd4 hidden max-desktop:!flex max-desktop:flex-col max-desktop:items-center max-desktop:mt-4 max-md:mt-3 w-full max-md:max-w-full relative z-[2]">
                <div className="w-full max-w-[380px] max-md:max-w-full rounded-[18px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.92)":"rgba(255,255,255,0.92)",border:`1px solid ${dark?"rgba(255,255,255,.12)":"rgba(0,0,0,.08)"}`,boxShadow:dark?"0 20px 60px rgba(0,0,0,0.5)":"0 20px 60px rgba(0,0,0,.16)"}}>
                  <div className="px-5 pt-4 pb-3 max-md:px-4">
                    {siteStats.processing>0&&<div className="flex items-center justify-center gap-1.5 mb-2.5" style={{color:dark?"rgba(255,255,255,.45)":"rgba(0,0,0,.4)"}}><span className="relative flex h-[5px] w-[5px]"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#34d399] opacity-75"/><span className="relative inline-flex rounded-full h-[5px] w-[5px] bg-[#34d399]"/></span><span className="text-[11px] font-medium"><span className="m font-semibold" style={{color:dark?"#eae7e2":"#1a1a1a"}}>{siteStats.processing}</span> orders processing now</span></div>}
                    <div className="flex gap-2 mb-3">
                      {[[siteStats.orders||"0","Orders"],[siteStats.users||"0","Creators"],...(siteStats.deliveryRate!=null?[[`${siteStats.deliveryRate}%`,"Delivery"]]:[])].map(([num,label],i)=>
              ...
              <div className="fu fd4 hero-guarantee flex max-md:!hidden items-center gap-[6px] mt-3.5 text-xs flex-wrap" style={{color:dark?"rgba(244,241,237,.3)":"rgba(255,255,255,.6)"}}>
                ...
                Trusted by {siteStats.users||"2,335"}+ Nigerian creators
            ...
            <div className="w-[400px] shrink-0 max-desktop:hidden">
              <div className="rounded-[20px] py-7 px-[26px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.95)":"rgba(255,255,255,0.95)",border:`1px solid ${dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.12)"}`,boxShadow:dark?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(0,0,0,.18)"}}>
      ...
      {modal&&<AuthModal key="auth-modal" dark={dark} t={t} mode={modal} setMode={setModal} onClose={closeModal} prefill={heroSignupData} via={heroVia} resetToken={resetToken}/>}
