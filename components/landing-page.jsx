...
function LandingInner({ initialAuthQuery }){
  ...
  const [heroRemember,setHeroRemember]=useState(false);
    ...
    <div className="root h-dvh overflow-hidden flex flex-col" suppressHydrationWarning>
      ...
      <div ref={scrollRef} className="snap-container flex-1 overflow-y-auto overflow-x-hidden relative">
        ...
        <section id="hero" className="snap-section overflow-hidden relative flex flex-col" style={{background:t.heroBg}}>
          ...
          {/* Flowing paths background */}
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div style={{position:"absolute",top:"-10%",left:"20%",width:500,height:400,borderRadius:"50%",background:dark?"rgba(196,125,142,.06)":"rgba(255,255,255,.1)",filter:"blur(100px)"}}/>
            <div style={{position:"absolute",bottom:"0%",right:"5%",width:300,height:300,borderRadius:"50%",background:dark?"rgba(110,160,230,.04)":"rgba(255,255,255,.07)",filter:"blur(80px)"}}/>
            {[1,-1].map(dir=><svg key={dir} style={{position:"absolute",top:"50%",left:"50%",width:"100%",height:"100%",transform:`translate(-50%,-50%) scale(${dir},1)`}} viewBox="0 0 800 600" fill="none" preserveAspectRatio="xMidYMid slice">
              {[
                {d:"M-50 300 Q100 150 250 280 T550 220 T850 300",w:2,o:dark?.18:.2,del:0,dur:20},
                {d:"M-50 350 Q150 200 300 320 T600 250 T850 350",w:1.8,o:dark?.15:.18,del:2,dur:25},
                {d:"M-50 250 Q80 100 200 230 T500 180 T850 260",w:1.5,o:dark?.12:.15,del:4,dur:22},
                {d:"M-50 400 Q200 280 350 380 T650 300 T850 400",w:2.2,o:dark?.16:.2,del:1,dur:28},
                {d:"M-50 200 Q120 80 280 200 T580 140 T850 220",w:1.2,o:dark?.1:.13,del:6,dur:24},
                {d:"M-50 450 Q100 350 250 420 T550 360 T850 440",w:1.8,o:dark?.13:.16,del:3,dur:26},
                {d:"M-50 150 Q200 50 350 170 T600 100 T850 180",w:1.4,o:dark?.1:.12,del:5,dur:30},
                {d:"M-50 500 Q150 400 300 470 T600 410 T850 490",w:1.6,o:dark?.09:.12,del:7,dur:23},
                {d:"M-50 100 Q100 20 250 120 T500 60 T850 130",w:1,o:dark?.08:.1,del:8,dur:27},
                {d:"M-50 550 Q200 450 380 530 T650 470 T850 540",w:1.5,o:dark?.08:.1,del:9,dur:32},
              ].map(({d,w,o,del,dur},i)=><path key={i} d={d} stroke={dark?"rgba(196,125,142,1)":"rgba(255,255,255,1)"} strokeWidth={w} opacity={o} strokeDasharray="800" strokeDashoffset="800" style={{animation:`hero-path-flow ${dur}s ease-in-out ${del}s infinite`}}/>)}
            </svg>)}
          </div>

          <div className={`flex items-center justify-center gap-12 max-desktop:gap-0 pt-14 pb-10 max-desktop:pt-6 max-desktop:pb-4 max-md:pb-2 px-[60px] max-desktop:px-10 max-md:px-5 max-w-[1200px] mx-auto w-full relative z-[1] flex-1 max-desktop:flex-col max-desktop:text-center max-desktop:min-h-0 ${siteAlerts.length > 0 ? "max-md:pt-[50px]" : "max-md:pt-2"}`}>
            {/* LEFT */}
            <div className="flex-1 max-w-[540px] text-left relative z-[1] max-desktop:flex-none max-desktop:text-center max-desktop:max-w-full max-desktop:flex max-desktop:flex-col max-desktop:items-center">
              ...
              <div className="fu fd4 max-desktop:!hidden flex gap-3 items-center flex-wrap">
                <a href="/signup" onClick={e=>{e.preventDefault();setModal("signup")}} className="hero-cta-btn py-3.5 px-9 rounded-xl text-[15px] font-semibold border-none cursor-pointer transition-transform duration-200 hover:-translate-y-px no-underline" style={{background:"#fff",color:"#1a1a1a",boxShadow:"0 8px 32px rgba(0,0,0,.24)"}}>Create free account <span className="text-lg">→</span></a>
              ...
              <div className="fu fd4 hidden max-desktop:!flex max-desktop:flex-col max-desktop:items-center max-desktop:mt-4 max-md:mt-3 w-full max-md:max-w-full relative z-[2]">
                <div className="w-full max-w-[380px] max-md:max-w-full rounded-[18px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.92)":"rgba(255,255,255,0.92)",border:`1px solid ${dark?"rgba(255,255,255,.12)":"rgba(0,0,0,.08)"}`,boxShadow:dark?"0 20px 60px rgba(0,0,0,0.5)":"0 20px 60px rgba(0,0,0,.16)"}}>
                  <div className="px-5 pt-4 pb-3 max-md:px-4">
                    ...
                    <div className="text-center mb-2.5"><span className="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-[11px] font-semibold" style={{background:dark?"rgba(196,125,142,.15)":"rgba(196,125,142,.1)",color:t.accent}}>🎁 Give your next post a real push — up to ₦3,000 in free promo credit to start</span></div>
                    <a href="/signup" onClick={e=>{e.preventDefault();setModal("signup")}} className="block w-full py-[14px] rounded-xl text-[16px] font-bold border-none cursor-pointer hero-cta-pulse no-underline text-center" style={{background:"linear-gradient(135deg,#c47d8e,#a3586b)",color:"#fff",boxShadow:"0 6px 24px rgba(196,125,142,.4)"}}>Create free account →</a>
                    <a href="/?login=1" onClick={e=>{e.preventDefault();setModal("login")}} className="block w-full py-[11px] rounded-xl text-[14px] font-semibold cursor-pointer mt-2 no-underline text-center" style={{background:"transparent",border:`1.5px solid ${dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.14)"}`,color:dark?"rgba(255,255,255,.7)":"rgba(0,0,0,.55)"}}>Log in to your account</a>
                  </div>
            ...
            <div className="w-[400px] shrink-0 max-desktop:hidden">
              <form onSubmit={handleHeroAuthSubmit} noValidate className="rounded-[20px] py-7 px-[26px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.95)":"rgba(255,255,255,0.95)",border:`1px solid ${dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.12)"}`,boxShadow:dark?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(0,0,0,.18)"}}>
                <h2 className="text-[22px] font-semibold text-center mb-[3px]" style={{color:t.text}}>{heroAuth==="login"?"Let's run it up":heroAuth==="forgot"?"Forgot password?":(heroSignupStep===1?"Create your free account":"Secure Your Account")}</h2>
                <p className="text-[15px] text-center mb-4 font-medium" style={{color:t.textSoft}}>{heroAuth==="login"?"Log in and start boosting":heroAuth==="forgot"?"Enter your email for a reset link":(heroSignupStep===1?"Free to join, no card needed.":"Step 2 of 2. Set your password")}</p>
                {heroAuth==="signup"&&heroSignupStep===1&&<div className="text-center mb-3"><span className="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-[12px] font-semibold" style={{background:dark?"rgba(196,125,142,.15)":"rgba(196,125,142,.1)",color:t.accent}}>🎁 Give your next post a real push — up to ₦3,000 in free promo credit to start</span></div>}
                {heroAuth!=="forgot"&&heroSignupStep===1&&<><button type="button" onClick={()=>{const gp=new URLSearchParams();if(heroRefCode)gp.set('ref',heroRefCode);if(heroVia)gp.set('via',heroVia);window.location.href=`/api/auth/google${gp.size?`?${gp}`:''}`;}} style={{width:"100%",padding:"11px 0",borderRadius:12,background:dark?"rgba(255,255,255,.16)":"#fff",border:`1px solid ${dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.19)"}`,display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:14,fontWeight:600,color:dark?"#eae7e2":"#333",marginBottom:0}}><svg width="16" height="16" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>{heroAuth==="login"?"Continue with Google":"Sign up with Google"}</button><div style={{display:"flex",alignItems:"center",gap:12,margin:"12px 0"}}><div style={{flex:1,height:1,background:dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.14)"}}/><span style={{fontSize:12,fontWeight:500,color:t.textMuted}}>or</span><div style={{flex:1,height:1,background:dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.14)"}}/></div></>}
                {heroAuth==="signup"&&heroSignupStep===1&&<>
                  <div style={{display:"flex",gap:8,marginBottom:12}}>
                    <div style={{flex:1}}>
                      <label htmlFor="hero-signup-first" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>First Name</label>
                      <input id="hero-signup-first" name="firstName" autoComplete="given-name" placeholder="First" value={heroFirstName} onChange={e=>setHeroFirstName(e.target.value.replace(/[^a-zA-Z\u00C0-\u017F]/g,"").slice(0,50))} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:15,outline:"none",fontFamily:"inherit"}}/>
                    </div>
                    <div style={{flex:1}}>
                      <label htmlFor="hero-signup-last" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Last Name</label>
                      <input id="hero-signup-last" name="lastName" autoComplete="family-name" placeholder="Last" value={heroLastName} onChange={e=>setHeroLastName(e.target.value.replace(/[^a-zA-Z\u00C0-\u017F]/g,"").slice(0,50))} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:15,outline:"none",fontFamily:"inherit"}}/>
                    </div>
                  </div>
                  <label htmlFor="hero-signup-email" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Email Address</label>
                  <input id="hero-signup-email" name="email" autoComplete="email" placeholder="you@example.com" value={heroEmail} onChange={e=>setHeroEmail(e.target.value.trim().toLowerCase().slice(0,254))} type="email" style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:15,outline:"none",marginBottom:12,fontFamily:"inherit"}}/>
                  <label htmlFor="hero-signup-phone" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>WhatsApp Number <span style={{color:dark?"#fca5a5":"#dc2626"}}>*</span></label>
                  <div style={{display:"flex",gap:8,marginBottom:12}}>
                    <div aria-hidden="true" style={{padding:"11px 12px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.textSoft,fontSize:15,flexShrink:0,display:"flex",alignItems:"center",gap:6,userSelect:"none"}}><span style={{fontSize:16,lineHeight:1}}>🇳🇬</span> +234</div>
                    <input id="hero-signup-phone" name="phone" autoComplete="tel" placeholder="8012345678" value={heroPhone} onChange={e=>setHeroPhone(e.target.value.replace(/\D/g,"").slice(0,11))} type="tel" style={{flex:1,padding:"11px 14px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,color:t.text,fontSize:15,outline:"none",fontFamily:"inherit"}}/>
                  </div>
                </>}
                {heroAuth==="signup"&&heroSignupStep===2&&<>
                  <label htmlFor="hero-signup-password" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:3,textTransform:"uppercase",letterSpacing:1.5}}>Password</label>
                  <div style={{position:"relative",marginBottom:2}}>
                    <input id="hero-signup-password" name="password" autoComplete="new-password" placeholder="Min. 6 characters" value={heroPw} onChange={e=>setHeroPw(e.target.value.slice(0,128))} type={heroShowPw?"text":"password"} style={{width:"100%",padding:"9px 40px 9px 14px",borderRadius:10,background:t.inputBg,border:"1px solid "+t.inputBorder,color:t.text,fontSize:14,outline:"none",fontFamily:"inherit"}}/>
                    <button onClick={()=>setHeroShowPw(!heroShowPw)} type="button" aria-label={heroShowPw?"Hide password":"Show password"} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",color:t.textMuted,padding:2}}>{heroShowPw?<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19m-6.72-1.07a3 3 0 11-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}</button>
                  </div>
                  <PwStrength pw={heroPw} dark={dark} />
                  {heroPw&&heroPw.length<6&&<div style={{fontSize:11,color:t.textMuted,marginBottom:4,lineHeight:1.5}}>Use 6+ characters with a mix of uppercase, numbers, and symbols for a strong password.</div>}
                  <label htmlFor="hero-signup-confirm" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:3,textTransform:"uppercase",letterSpacing:1.5}}>Confirm Password</label>
                  <input id="hero-signup-confirm" name="passwordConfirmation" autoComplete="new-password" placeholder="Re-enter password" value={heroPw2} onChange={e=>setHeroPw2(e.target.value.slice(0,128))} type="password" style={{width:"100%",padding:"9px 14px",borderRadius:10,background:t.inputBg,border:"1px solid "+(heroPw2&&heroPw!==heroPw2?(dark?"rgba(220,38,38,.4)":"#fecaca"):heroPw2&&heroPw===heroPw2?(dark?"rgba(110,231,183,.4)":"#a7f3d0"):t.inputBorder),color:t.text,fontSize:14,outline:"none",marginBottom:2,fontFamily:"inherit"}}/>
                  ...
                  <label htmlFor="hero-signup-referral" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:3,textTransform:"uppercase",letterSpacing:1.5}}>Referral Code <span style={{fontWeight:400,color:t.textMuted}}>(optional)</span></label>
                  <input id="hero-signup-referral" name="referralCode" placeholder="e.g. NTR-7X92" value={heroRefCode} onChange={e=>setHeroRefCode(e.target.value.replace(/[^a-zA-Z0-9\-]/g,"").toUpperCase().slice(0,20))} style={{width:"100%",padding:"9px 14px",borderRadius:10,background:t.inputBg,border:"1px solid "+t.inputBorder,color:t.text,fontSize:14,outline:"none",marginBottom:8,fontFamily:"inherit"}}/>
                  <label htmlFor="hero-signup-terms" style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:10,cursor:"pointer"}}><input id="hero-signup-terms" name="termsAccepted" type="checkbox" checked={heroAgree} onChange={e=>setHeroAgree(e.target.checked)} style={{marginTop:2,accentColor:t.accent,width:14,height:14,flexShrink:0}}/><span style={{fontSize:12,color:t.textSoft,lineHeight:1.4}}>I agree to the <a href="/terms" style={{color:t.accent,textDecoration:"none"}}>Terms</a> and <a href="/privacy" style={{color:t.accent,textDecoration:"none"}}>Privacy Policy</a></span></label>
                </>}
                {heroAuth==="login"&&<>
                  <label htmlFor="hero-login-email" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Email Address</label>
                  <input id="hero-login-email" name="email" autoComplete="username" placeholder="you@example.com" value={heroEmail} onChange={e=>setHeroEmail(e.target.value.trim().toLowerCase().slice(0,254))} type="email" style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:"1px solid "+t.inputBorder,color:t.text,fontSize:15,outline:"none",marginBottom:12,fontFamily:"inherit"}}/>
                  <label htmlFor="hero-login-password" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Password</label>
                  <input id="hero-login-password" name="password" autoComplete="current-password" placeholder="Enter password" type="password" value={heroPw} onChange={e=>setHeroPw(e.target.value.slice(0,128))} style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:"1px solid "+t.inputBorder,color:t.text,fontSize:15,outline:"none",marginBottom:14,fontFamily:"inherit"}}/>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                    <label htmlFor="hero-login-remember" style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}><input id="hero-login-remember" name="remember" type="checkbox" checked={heroRemember} onChange={e=>setHeroRemember(e.target.checked)} style={{accentColor:t.accent,width:14,height:14}}/><span style={{fontSize:13,color:t.textSoft}}>Remember me</span></label>
                    <button type="button" onClick={()=>{setHeroAuth("forgot");setHeroError("")}} style={{background:"none",color:t.accent,fontSize:13,fontWeight:500}}>Forgot password?</button>
                  </div>
                </>}
                {heroAuth==="forgot"&&<>
                  <label htmlFor="hero-forgot-email" style={{fontSize:11,fontWeight:600,color:t.textSoft,display:"block",marginBottom:5,textTransform:"uppercase",letterSpacing:1.5}}>Email Address</label>
                  <input id="hero-forgot-email" name="email" autoComplete="email" placeholder="you@example.com" value={heroEmail} onChange={e=>setHeroEmail(e.target.value.trim().toLowerCase().slice(0,254))} type="email" style={{width:"100%",padding:"11px 14px",borderRadius:12,background:t.inputBg,border:"1px solid "+t.inputBorder,color:t.text,fontSize:15,outline:"none",marginBottom:16,fontFamily:"inherit"}}/>
                </>}
                ...
                {heroAuth==="signup"&&heroSignupStep===2&&<button type="button" onClick={()=>{setHeroSignupStep(1);setHeroError("")}} className="w-full py-2 px-0 bg-transparent text-sm font-medium mb-2" style={{color:t.textSoft}}>← Back to Step 1</button>}
      ...
      {modal&&<AuthModal key="auth-modal" dark={dark} t={t} mode={modal} setMode={setModal} onClose={closeModal} prefill={heroSignupData} via={heroVia} referralCode={heroRefCode} resetToken={resetToken}/>}
...
export default function Landing({ initialAuthQuery }) {
