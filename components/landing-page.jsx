...
function LandingInner(){
  ...
  useEffect(()=>{const p=new URLSearchParams(window.location.search);if(p.get("login"))setModal("login");if(p.get("signup"))setModal("signup");if(p.get("ref")){setModal("signup");setHeroRefCode(p.get("ref"));};if(p.get("via")&&!initVia){setHeroVia(p.get("via"));setHeroAuth("signup");}if(p.get("session_expired")){setSessionExpired(true);window.history.replaceState({},"","/");}if(p.get("logout")){setLogoutMsg(true);window.history.replaceState({},"","/");setTimeout(()=>setLogoutMsg(false),4000);}if(p.get("google_error")){setGoogleError(true);window.history.replaceState({},"","/");setTimeout(()=>setGoogleError(false),5000);setModal("login");}if(p.get("error")==="account_pending_deletion"){setHeroError("This account is scheduled for deletion. Contact support@nitro.ng to reinstate it.");window.history.replaceState({},"","/");}if(p.get("error")==="disposable_email"){setHeroError("Disposable email addresses aren't allowed. Please sign up with a permanent email.");setModal("signup");window.history.replaceState({},"","/");}if(p.get("reset")){setModal("reset");}if(["google_cancelled","google_state_mismatch","google_token_failed","google_no_email","google_failed","google_missing_params","google_not_configured","google_account_deleted"].includes(p.get("error"))){setGoogleError(true);window.history.replaceState({},"","/");setTimeout(()=>setGoogleError(false),5000);setModal("login");}},[]);
                ...
                <div className="w-full max-w-[380px] max-md:max-w-full rounded-[18px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.92)":"rgba(255,255,255,0.92)",border:`1px solid ${dark?"rgba(255,255,255,.12)":"rgba(0,0,0,.08)"}`,boxShadow:dark?"0 20px 60px rgba(0,0,0,0.5)":"0 20px 60px rgba(0,0,0,.16)"}}>
                  <div className="px-5 pt-4 pb-3 max-md:px-4">
                    <div className="flex gap-2 mb-3">
                      {[[siteStats.orders||"0","Orders"],[siteStats.users||"0","Creators"],["98%","Delivery"]].map(([num,label],i)=>
