              ...
              <div className="fu fd4 hidden max-desktop:!flex max-desktop:flex-col max-desktop:items-center max-desktop:mt-4 max-md:mt-3 w-full max-md:max-w-full relative z-[2]">
                <div className="w-full max-w-[380px] max-md:max-w-full rounded-[18px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.92)":"rgba(255,255,255,0.92)",border:`1px solid ${dark?"rgba(255,255,255,.12)":"rgba(0,0,0,.08)"}`,boxShadow:dark?"0 20px 60px rgba(0,0,0,0.5)":"0 20px 60px rgba(0,0,0,.16)"}}>
                  <div className="px-5 pt-4 pb-3 max-md:px-4">
                    ...
                    <div className="text-center mb-2.5"><span className="inline-flex items-center gap-1.5 py-1.5 px-3 rounded-full text-[11px] font-semibold text-accent" style={{background:dark?"rgba(196,125,142,.15)":"rgba(196,125,142,.1)"}}>🎁 Give your next post a real push — up to ₦3,000 in free promo credit to start</span></div>
            ...
            <div className="w-[400px] shrink-0 max-desktop:hidden">
              <form onSubmit={handleHeroAuthSubmit} noValidate className="rounded-[20px] py-7 px-[26px] backdrop-blur-[20px]" style={{background:dark?"rgba(17,22,40,0.95)":"rgba(255,255,255,0.95)",border:`1px solid ${dark?"rgba(255,255,255,.18)":"rgba(0,0,0,.12)"}`,boxShadow:dark?"0 24px 64px rgba(0,0,0,0.5)":"0 24px 64px rgba(0,0,0,.18)"}}>
                ...
                {heroAuth==="signup"&&heroSignupStep===1&&<>
                  ...
                  <div style={{display:"flex",gap:8,marginBottom:12}}>
                    <div aria-hidden="true" className="text-t-text-soft" style={{padding:"11px 12px",borderRadius:12,background:t.inputBg,border:`1px solid ${t.inputBorder}`,fontSize:15,flexShrink:0,display:"flex",alignItems:"center",gap:6,userSelect:"none"}}><span style={{fontSize:16,lineHeight:1}}>🇳🇬</span> +234</div>
                ...
                {heroAuth==="signup"&&heroSignupStep===2&&<>
                  ...
                  <div style={{position:"relative",marginBottom:2}}>
                    <input id="hero-signup-password" name="password" autoComplete="new-password" placeholder="Min. 6 characters" value={heroPw} onChange={e=>setHeroPw(e.target.value.slice(0,128))} type={heroShowPw?"text":"password"} className="text-t-text font-[inherit]" style={{width:"100%",padding:"9px 40px 9px 14px",borderRadius:10,background:t.inputBg,border:"1px solid "+t.inputBorder,fontSize:14,outline:"none"}}/>
                  ...
                  <input id="hero-signup-referral" name="referralCode" placeholder="e.g. NTR-7X92" value={heroRefCode} onChange={e=>setHeroRefCode(e.target.value.replace(/[^a-zA-Z0-9\-]/g,"").toUpperCase().slice(0,20))} className="text-t-text font-[inherit]" style={{width:"100%",padding:"9px 14px",borderRadius:10,background:t.inputBg,border:"1px solid "+t.inputBorder,fontSize:14,outline:"none",marginBottom:8}}/>
                  <label htmlFor="hero-signup-terms" style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:10,cursor:"pointer"}}><input id="hero-signup-terms" name="termsAccepted" type="checkbox" checked={heroAgree} onChange={e=>setHeroAgree(e.target.checked)} style={{marginTop:2,accentColor:t.accent,width:14,height:14,flexShrink:0}}/><span className="text-t-text-soft" style={{fontSize:12,lineHeight:1.4}}>I agree to the <a href="/terms" className="text-accent no-underline">Terms</a> and <a href="/privacy" className="text-accent no-underline">Privacy Policy</a></span></label>
                </>}
                {heroAuth==="login"&&<>
