...
function AuthModal({ dark, t, mode, setMode, onClose, prefill }) {
  ...
  const verifyInputs = useRef([]);

  useEffect(() => {
    if (step !== 3 || verifyTimer <= 0) return;
    const iv = setInterval(() => setVerifyTimer((p) => p - 1), 1000);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Subheading */}
        <p
          className="text-[15px] text-center mb-7 font-[450]"
        ...
        {mode === 'login' && (
          <>
            ...
            <button
              ...
              className="google-btn w-full py-[13px] rounded-xl flex items-center justify-center gap-2.5 text-[15px] font-semibold mb-0 transition-colors duration-200"
              style={{
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl text-white text-base font-semibold mb-5 flex items-center justify-center gap-2"
              style={{
        ...
        {mode === 'signup' && step === 1 && (
          <>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl text-white text-base font-semibold mb-5"
        ...
        {mode === 'signup' && step === 2 && (
          <>
            ...
            <button
              ...
              disabled={authLoading}
              className="w-full py-3.5 rounded-xl text-white text-base font-semibold mb-2 flex items-center justify-center gap-2"
              style={{
                background: authLoading ? '#999' : t.btnPrimary,
                opacity: authLoading ? 0.7 : 1,
              }}
        ...
        {/* ====== SIGNUP STEP 3 — VERIFY ====== */}
        {mode === 'signup' && step === 3 && !verified && (
          <>
            {/* Email icon */}
            ...
            <div className="flex justify-center gap-2 mb-6">
              {verifyCode.map((d, i) => (
                <input
                  ...
                  maxLength={1}
                  inputMode="numeric"
                  className="w-11 h-[52px] rounded-xl text-xl font-semibold text-center outline-none font-mono"
                  style={{
                    background: t.inputBg,
                  ...
                  onPaste={(e) => {
                    ...
                    if (paste.length > 0) {
                      ...
                      verifyInputs.current[lastIdx]?.focus();
                      if (paste.length === 6) {
                        setAuthLoading(true);
                        setError('');
                  ...
                  onChange={(e) => {
                    ...
                    setError('');
                    if (i < 5 && v)
                      verifyInputs.current[i + 1]?.focus();
                    if (i === 5 && v) {
            ...
            {authLoading && (
              <div className="flex justify-center mb-4">
                <span
                  className="block w-5 h-5 rounded-full animate-[spin_0.6s_linear_infinite]"
                  style={{
                    border: '2px solid rgba(196,125,142,.3)',
            ...
            <button
              onClick={() => {
                ...
                setVerifyTimer(60);
                fetch('/api/auth/verify', { method: 'PUT' }).catch(
                  () => {}
                );
              }}
              disabled={verifyTimer > 0}
              className="w-full py-3 rounded-[10px] bg-transparent text-sm font-medium mb-2"
              style={{
                color: verifyTimer > 0 ? t.textMuted : t.accent,
              }}
            >
              {verifyTimer > 0
                ? `Resend code in ${verifyTimer}s`
                : 'Resend Code'}
            </button>
          </>
        )}

        {/* ====== SIGNUP STEP 3 — VERIFIED ====== */}
        {mode === 'signup' && step === 3 && verified && (
          <>
            {/* Success icon */}
              ...
              <div
                ...
                style={{
                  background: dark
                    ...
                    : 'rgba(5,150,105,0.06)',
                  border: `2px solid ${dark ? '#6ee7b7' : '#059669'}`,
                }}
              >
                <svg
                  width="28"
