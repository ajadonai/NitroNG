...
export function AdminCreateOrderPage({ dark, t }) {
  ...
  const dripNum = Number(dripDays) || 0;
  ...
  const ready = user && !submitting && (
    mode === "single" ? (selectedTier && validQty && !!link && (!dripDays || dripNum >= 2)) :
    batchItems.length > 0
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const body = mode === "bulk" ? {
        ...
        mode: dripNum >= 2 ? "drip" : "single", userId: user.id, tierId: selectedTier.id, quantity: qtyNum, charge, link: fullLink(link),
        ...(dripNum >= 2 ? { dripDays: dripNum } : {}),
      };
      ...
      const label = mode === "bulk" ? `${d.count || batchTotalOrders} orders created` : dripNum >= 2 ? `Drip order created (${dripNum} days)` : "Order created";
      const ids = d.orderIds || [];
      toast.success(label, ids.slice(0, 3).join(" · ") + (ids.length > 3 ? ` · +${ids.length - 3} more` : ""));
      setLink(""); setQty(""); setDripDays(""); setBatchItems([]);
  ...
  const submitBlock = insufficientBal ? (
    ...
    <button onClick={handleSubmit} disabled={!ready} style={{ width: "100%", padding: 13, borderRadius: 12, fontSize: 14.5, fontWeight: 800, background: "linear-gradient(135deg,#c47d8e,#8b5e6b)", color: "#fff", transition: ".15s", border: "none", cursor: ready ? "pointer" : "default", opacity: ready ? 1 : .4 }}>
      {submitting ? "Creating..." : mode === "bulk" ? `Create ${batchTotalOrders} Order${batchTotalOrders !== 1 ? "s" : ""}` : dripNum >= 2 ? `Create Drip Order (${dripNum}d)` : "Create Order"}
    </button>
  );
        ...
        <>
          ...
          {dripNum >= 2 && (
            <div style={{ marginTop: 10, padding: "10px 12px", borderRadius: 10, background: dark ? "rgba(255,255,255,.07)" : "rgba(0,0,0,.03)", border: `1px solid ${dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.06)"}` }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: t.textMuted, marginBottom: 6 }}>Drip schedule · {dripNum} days</div>
              {Array.from({ length: Math.min(dripNum, 3) }, (_, i) => {
                const perDay = Math.ceil(qtyNum / dripNum);
                const dayQty = i < dripNum - 1 ? perDay : qtyNum - perDay * (dripNum - 1);
                return <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: t.soft, padding: "2.5px 0" }}><span>Day {i + 1}</span><span className="m">{Math.max(dayQty, 0).toLocaleString()}</span></div>;
              })}
              {dripNum > 3 && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: t.soft, padding: "2.5px 0" }}><span>… through day {dripNum}</span><span className="m">≈{Math.ceil(qtyNum / dripNum).toLocaleString()}/day</span></div>}
            </div>
          )}
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 18, alignItems: "flex-start" }} className="max-md:flex-col">
        ...
        <div style={{ flex: 1, ...card }} className={mobileReview ? "max-md:hidden" : ""}>
          ...
          {/* quantity + drip days */}
          <div style={{ display: "grid", gridTemplateColumns: mode !== "bulk" && selectedGroup?.tags?.includes("drip") ? "1fr 1fr" : "1fr", gap: 12, marginBottom: 16 }}>
            <div>
            ...
            {mode !== "bulk" && selectedGroup?.tags?.includes("drip") && (
              <div>
                <span style={lab}>Drip days <span style={{ fontWeight: 500, color: t.textMuted }}>(optional)</span></span>
                <input value={dripDays} onChange={e => setDripDays(e.target.value.replace(/[^0-9]/g, ""))} inputMode="numeric" placeholder="—" style={inpM} />
                <div style={{ fontSize: 11.5, color: t.textMuted, marginTop: 5 }}>
                  {dripNum >= 2 && qtyNum ? `≈ ${Math.ceil(qtyNum / dripNum).toLocaleString()} per day` : "Leave empty for auto-drip"}
                </div>
              </div>
            )}
          </div>
