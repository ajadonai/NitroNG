...
function PodiumCard({ entry, rank, dark, tab }) {
  ...
  return (
    <div className="flex-1 rounded-[14px] max-md:rounded-[10px] py-4 px-3.5 max-md:py-2.5 max-md:px-1.5 text-center border-[1.5px] overflow-hidden min-w-0" style={{
      ...
      <div className="m font-bold max-md:text-xs max-md:whitespace-nowrap" style={{ color: dark ? p.valDk : p.valLt, fontSize: isFirst ? 16 : 14 }}>{val}</div>
    </div>
  );
}
...
function ListRow({ entry, dark, t, tab, isLast }) {
  const val = getVal(entry, tab);
  return (
    <div className="flex items-center gap-3 max-md:gap-2.5 py-3 px-4 max-md:py-2.5 max-md:px-3" style={{ borderBottom: isLast ? "none" : `1px solid ${t.cardBorder}`, background: entry.isYou ? (dark ? "rgba(196,125,142,.04)" : "rgba(196,125,142,.03)") : "transparent" }}>
      ...
      <div className="ml-auto text-right">
        <div className="m text-sm max-md:text-[13px] font-bold shrink-0" style={{ color: dark ? "#6ee7b7" : "#059669" }}>{val}</div>
      </div>
    </div>
  );
}
    ...
    <>
      <div className="pb-3.5">
        <div>
          ...
          <div className="text-[15px]" style={{ color: t.textMuted }}>Top Nitro users · {periodLabel}</div>
        </div>
        <div className="page-divider" style={{ background: t.cardBorder }} />
      </div>

      {rewardAnnouncement && (
        <div className="rounded-[10px] py-3 px-4 mb-4 border text-sm font-medium leading-normal" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.04)", borderColor: dark ? "rgba(196,125,142,.15)" : "rgba(196,125,142,.12)", color: t.text }}>
          {rewardAnnouncement}
        </div>
      )}
      ...
      {data?.tiers?.length > 0 && (
        <details className="lb-tiers-mobile mb-4 rounded-xl border" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(255,255,255,.9)", borderColor: dark ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.1)" }}>
          ...
          <div className="px-4 pb-3.5">
            {yourBadge && yourRank && (
              <div className="flex items-center gap-2.5 py-2.5 px-3 rounded-lg mb-3" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.04)" }}>
                <div className="m text-xl font-bold" style={{ color: t.accent }}>#{yourRank}</div>
                <div>
            ...
            {data.tiers.map((tier, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2.5 py-2.5" style={{ borderBottom: i < data.tiers.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.06)"}` : "none" }}>
      ...
      {loading ? (
        <div>
          <div className="flex gap-1 mb-4">{[1,2].map(i => <div key={i} className={`skel-bone ${dark ? "skel-dark" : "skel-light"} h-[30px] w-[90px] rounded-[20px]`} />)}</div>
          <div className={`skel-bone ${dark ? "skel-dark" : "skel-light"} h-[52px] rounded-[10px] mb-4`} />
        ...
        {yourRank && (
          <div className="lb-you-desktop rounded-[10px] py-3 px-4 max-md:py-2.5 max-md:px-3 flex items-center gap-3 mb-4 border" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.04)", borderColor: dark ? "rgba(196,125,142,.15)" : "rgba(196,125,142,.1)" }}>
            <div className="m text-[22px] max-md:text-lg font-bold" style={{ color: t.accent }}>#{yourRank}</div>
            <div>
        ...
        {rest.length > 0 && (
          <div className="rounded-xl overflow-hidden border" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(255,255,255,.9)", borderColor: dark ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.1)" }}>
            {rest.map((entry, i) => (
...
export function LeaderboardCard({ dark, t, onViewAll }) {
  ...
  return (
    <div className="rounded-[14px] p-4 border" style={{ background: dark ? "rgba(255,255,255,.03)" : "rgba(255,255,255,.85)", borderColor: t.cardBorder }}>
      <div className="text-xs font-semibold uppercase tracking-[1.5px] mb-3" style={{ color: t.textMuted }}>Top Spenders · {month}</div>
      {top3.map((entry, i) => (
        <div key={i} className="flex items-center gap-2.5 py-1.5">
          <div className="text-base w-6 text-center">{POD[i + 1].medal}</div>
          <div className="text-sm font-medium flex-1" style={{ color: t.text }}>{entry.name}{entry.isYou ? " (You)" : ""}</div>
          <div className="m text-[13px] font-semibold shrink-0" style={{ color: dark ? "#6ee7b7" : "#059669" }}>
            {entry.orders} order{entry.orders !== 1 ? "s" : ""}
          </div>
        </div>
      ))}
      {onViewAll && <button onClick={onViewAll} className="block text-center mt-3 text-[13px] font-medium cursor-pointer py-1.5 rounded-lg bg-transparent border-none font-[inherit] w-full" style={{ color: t.accent }}>View full leaderboard →</button>}
    </div>
  );
}
...
export function TierPerksCard({ dark, t }) {
  ...
  return (
    <div className="rounded-[14px] p-4 mb-4 border" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(255,255,255,.9)", borderColor: dark ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.1)" }}>
      <div className="text-[11px] font-semibold uppercase tracking-[1px] mb-3" style={{ color: t.textMuted }}>Loyalty tiers</div>
      {yourBadge && (
      ...
      {tiers.map((tier, i) => {
        const isCurrent = yourBadge?.name === tier.name;
        return (
          <div key={i} className="flex items-center gap-2 py-2" style={{ borderBottom: i < tiers.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.06)"}` : "none", opacity: isCurrent ? 1 : 0.65 }}>
