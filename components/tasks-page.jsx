    ...
    <>
      ...
      <div className="flex justify-between items-start gap-3 pb-2 desktop:pb-3">
        ...
        <div ref={sumRef} className="relative shrink-0">
          ...
          {sumOpen && (
            <div className="absolute top-11 z-20 w-[240px] rounded-2xl px-3.5 py-1" style={{ insetInlineEnd: 0, background: dark ? '#1a1329' : '#fff', border: `1px solid ${border}`, boxShadow: '0 18px 44px rgba(0,0,0,.18)' }}>
        ...
        <div className="relative ml-auto" onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget)) setFilterOpen(false); }}>
          ...
          {filterOpen && (
            <>
              ...
              <div className="absolute top-[calc(100%+6px)] z-50 rounded-[10px] overflow-hidden min-w-[150px] py-1" style={{ insetInlineEnd: 0, background: dark ? '#1a1d2e' : '#fff', border: `1px solid ${dark ? 'rgba(255,255,255,.18)' : 'rgba(0,0,0,.14)'}`, boxShadow: '0 8px 32px rgba(0,0,0,.18)' }}>
              {FILTERS.map(f => (
