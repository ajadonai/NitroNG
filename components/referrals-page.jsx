  ...
  return (
    <>
      ...
      {user?.pendingRefBonus && (
        <div className="py-3 px-4 rounded-xl mb-4 flex items-center gap-2.5" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.03)", border: `1px solid ${dark ? "rgba(196,125,142,.12)" : "rgba(196,125,142,.08)"}` }}>
      ...
      {/* Share card */}
      <div className="rounded-xl desktop:rounded-2xl p-3.5 desktop:p-5 mb-3 desktop:mb-4" style={{ background: dark ? "rgba(255,255,255,.03)" : "rgba(255,255,255,.85)", border: `0.5px solid ${t.cardBorder}` }}>
        <div className="text-[13px] font-semibold uppercase tracking-[1.5px] mb-3" style={{ color: t.textMuted }}>Your Referral</div>
        <div className="flex flex-col desktop:flex-row gap-2.5 desktop:gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => copyText(refCode, "code")} className="py-1.5 px-3 rounded-md border text-[13px] font-semibold cursor-pointer bg-transparent" style={{ borderColor: t.cardBorder, color: copied === "code" ? t.green : t.textSoft }}>
          ...
          <div className="flex-1 desktop:flex-[1.5]">
            ...
            <div className="flex items-center gap-2">
              <div className="m flex-1 py-2 px-3 rounded-lg border text-sm overflow-hidden text-ellipsis whitespace-nowrap" style={{ background: dark ? "#0d1020" : "#fff", borderColor: dark ? "rgba(255,255,255,.1)" : "rgba(0,0,0,.12)", color: t.textSoft }}>{refLink}</div>
              <button onClick={() => copyText(`https://${refLink}`, "link")} className="py-2 px-3 desktop:px-3.5 rounded-lg text-[13px] desktop:text-sm font-semibold cursor-pointer whitespace-nowrap shrink-0 border-none text-white" style={{ background: "linear-gradient(135deg, #c47d8e, #8b5e6b)" }}>
      ...
      <div className="grid grid-cols-2 desktop:grid-cols-4 gap-2 desktop:gap-2.5 mb-3 desktop:mb-4">
        {[
          ...
          <div key={label} className="p-3 desktop:p-3.5 rounded-[10px] desktop:rounded-xl" style={{ background: dark ? "rgba(255,255,255,.03)" : "rgba(255,255,255,.85)", border: `0.5px solid ${t.cardBorder}` }}>
      ...
      <div className="rounded-xl desktop:rounded-[14px] overflow-hidden" style={{ background: dark ? "rgba(255,255,255,.03)" : "rgba(255,255,255,.85)", border: `0.5px solid ${t.cardBorder}` }}>
        ...
        {paged.length > 0 ? paged.map((r, i) => (
          <div key={r.id || i} className="flex items-center gap-2.5 desktop:gap-3.5 py-3 px-[13px] desktop:py-3.5 desktop:px-4" style={{ borderBottom: i < paged.length - 1 ? `1px solid ${t.cardBorder}` : "none" }}>
            <div className="w-8 h-8 desktop:w-9 desktop:h-9 rounded-[10px] flex items-center justify-center text-sm desktop:text-base font-semibold shrink-0" style={{ background: dark ? "rgba(196,125,142,.1)" : "rgba(196,125,142,.06)", color: t.accent }}>{(r.name || "?").charAt(0)}</div>
...
export function ReferralsSidebar({ user, dark, t }) {
  ...
  return (
    <>
      {/* How it works */}
      <div className="text-[13px] font-semibold uppercase tracking-[1.5px] mb-3 pl-1" style={{ color: t.textMuted }}>How It Works</div>
      {[
        ["1", "Share your link", "Send your referral link to friends"],
        ["2", "They sign up", "Your friend creates a Nitro account"],
        ["3", "You earn", "Bonus is credited to your wallet"],
      ...
      {/* Current reward */}
      <div className="text-[13px] font-semibold uppercase tracking-[1.5px] mb-3 pl-1" style={{ color: t.textMuted }}>Current Reward</div>
      ...
      {/* Performance */}
      <div className="text-[13px] font-semibold uppercase tracking-[1.5px] mb-3 pl-1" style={{ color: t.textMuted }}>Your Performance</div>
