    ...
    <>
      ...
      <div className="min-h-dvh flex flex-col font-[Outfit,system-ui,sans-serif]" style={{ background: t.bg }}>
        ...
        <main className="flex-1 py-12 px-6 pb-20 max-w-[720px] mx-auto w-full">
          ...
          {/* FAQ list */}
          <div className="rounded-2xl overflow-hidden" style={{ background: dark ? "rgba(255,255,255,.03)" : "rgba(255,255,255,.7)", border: `1px solid ${border}` }}>
            {faqs.map(([q, a], i) => (
              <div key={i} style={{ borderBottom: i < faqs.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.06)"}` : undefined }}>
                ...
                <div className="overflow-hidden" style={{ maxHeight: open === i ? 300 : 0, opacity: open === i ? 1 : 0, transition: "max-height .3s ease, opacity .3s ease" }}>
          ...
          {/* CTA */}
          <div className="mt-10 p-6 rounded-[14px] text-center" style={{ background: dark ? "rgba(196,125,142,.04)" : "rgba(196,125,142,.03)", border: `1px solid ${dark ? "rgba(196,125,142,.1)" : "rgba(196,125,142,.08)"}` }}>
            <p className="text-[15px] mb-3" style={{ color: t.textSoft }}>Still have questions?</p>
            <a href="/dashboard" className="inline-flex items-center gap-2 py-3 px-7 rounded-[10px] bg-gradient-to-br from-[#c47d8e] to-[#8b5e6b] text-white text-sm font-semibold no-underline">Contact support</a>
          </div>
