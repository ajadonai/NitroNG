...
function ShareButtons({ post, dark, size = 34 }) {
  ...
  return (
    <>
      <button onClick={copy} title={copied ? tr("Copied!") : tr("Copy link")} className={cls} style={{ ...dim, ...(copied ? copiedStyle : linkStyle) }}>
        {copied
      ...
      <a href={`https://x.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`} target="_blank" rel="noopener noreferrer" title={tr("Share on X")} className={cls} style={{ ...dim, ...xStyle }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </a>
      <a href={`https://wa.me/?text=${encodeURIComponent(text + " " + url)}`} target="_blank" rel="noopener noreferrer" title={tr("Share on WhatsApp")} className={cls} style={{ ...dim, ...waStyle }}>
    ...
    <div className="rd min-h-screen relative" style={{ ...vars, background: "var(--rbg)", color: "var(--rtx)" }}>
      ...
      <div className="rd-wrap">
        <div className="rd-hero">
          <span className="rd-eye">{post.category} · {rt} {tr("min read")}</span>
        ...
        <div className="rd-cols">
          {toc.length >= 3 ? (
            <>
              <aside className="rd-toc"><span className="rd-eye">{tr("In this article")}</span>{toc.map(h => <a key={h.id} href={'#' + h.id} onClick={e => jump(e, h.id)} className={active === h.id ? "on" : ""}>{h.text}</a>)}</aside>
              <details className="rd-tocph"><summary>{tr("In this article ·")} {toc.length} parts</summary>{toc.map(h => <a key={h.id} href={'#' + h.id} onClick={e => jump(e, h.id)}>{h.text}</a>)}</details>
            </>
          ...
          <article ref={articleRef} className="rd-body">
            ...
            <div className="rd-end"><span>{tr("Found this useful? Pass it on.")}</span><span className="rd-share"><ShareButtons post={post} dark={dark} size={32} /></span></div>
            <div className="rd-author"><Avatar size={44} /><span className="rd-byt"><b>{tr("Written by")} {post.authorName || "the Nitro Team"}</b><i>{tr("Research and growth notes from the team behind Nigeria's fastest content promotion platform.")} <a href="https://x.com/TheNitroNG" target="_blank" rel="noopener noreferrer">{tr("Follow on X")}</a></i></span></div>
          </article>
        </div>

        {(prev || next) && (
          <div className="rd-pn">
            {prev ? <a href={`/blog/${prev.slug}`} className="rd-pnc"><em>{tr("Previous")}</em><b>{prev.title}</b></a> : <span />}
            {next && <a href={`/blog/${next.slug}`} className="rd-pnc right"><em>{tr("Next")}</em><b>{next.title}</b></a>}
          </div>
        )}

        {related?.length > 0 && (
          <div className="rd-rel">
            <span className="rd-eye">{tr("Related articles")}</span>
        ...
        <a href={backHref} className="rd-back">{tr("← Back to")} {backLabel.toLowerCase()}</a>
      </div>
      <SharedFooter />
    </div>
  );
