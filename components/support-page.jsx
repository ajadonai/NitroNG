...
const BOT_RESPONSES = {
  ...
  deposit: { text: "To fund your wallet:\n\n1. Go to **Wallet** in your dashboard\n2. Enter your amount (minimum ₦1,000 — deposits are always charged in naira, whatever currency you are reading prices in)\n3. Choose a payment method — **bank transfer**, **card**, or **crypto**\n\nDeposits are credited instantly. Your wallet balance is used for all orders." },
  refund: { text: "Nitro offers refunds for:\n\n• **Undelivered orders** — auto-refunded after 72 hours\n• **Partial delivery** — refunded for undelivered portion\n• **Wrong service** — full refund to wallet\n\nRefunds go to your Nitro wallet within 5 minutes.\n\n**Important:** Your profile must be set to public before placing an order. There are no refunds for orders placed on private profiles." },
  delivery: { text: "Most orders start processing within **seconds** of payment. Full delivery depends on the service and quantity:\n\n• **Followers/likes** — small orders deliver in minutes, larger orders are spread over several hours for natural growth\n• **Views/streams** — gradual, up to 24 hours\n• **Comments** — usually within 1–4 hours\n\nLarger orders are delivered gradually to keep your account safe. You can track progress in real time on your dashboard.\n\nStandard tier includes a **30-day refill** and Premium tier includes **lifetime refill** — if you lose any, we top them back up for free. Budget has no refill." },
  pricing: { text: "Every service has 3 tiers:\n\n• **Budget** — cheapest, no refill\n• **Standard** — best value, 30-day refill\n• **Premium** — top quality, lifetime refill\n\nPrices start from ₦3 per 1,000. Check **New Order** to see live rates for each platform." },
  platforms: { text: "Nitro offers **140+ service types** across:\n\n• **Instagram** — followers, likes, views, comments, reels\n• **TikTok** — followers, likes, views, shares\n• **YouTube** — subscribers, views, watch hours, likes\n• **Twitter/X** — followers, likes, retweets\n• **Facebook, Telegram, Spotify, Snapchat, LinkedIn, Pinterest, Twitch, Discord** and more\n\nCheck the **New Order** page for the full list." },
...
export default function SupportPage({ dark, t }) {
  ...
  /* ═══ RENDER ═══ */
  return (
    <div className={`sup-split rounded-xl ${mobileView === "chat" ? "sup-view-chat" : "sup-view-list"}`} style={{ border: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"}` }}>

      {/* ═══ LEFT: CONVERSATION LIST ═══ */}
      <div className="sup-split-list w-[280px] shrink-0" style={{ borderRight: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"}` }}>
        <div className="shrink-0 h-[56px] flex justify-between items-center px-4" style={{ background: dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)", borderBottom: `1px solid ${dark ? "rgba(255,255,255,.14)" : "rgba(0,0,0,.1)"}` }}>
        ...
        <div className="shrink-0" style={{ padding: "8px 10px", borderBottom: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {/* Bot chat item */}
          <div role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.currentTarget.click()}}} onClick={() => { setSelected(null); setMobileView("chat"); }} className="flex items-center gap-2.5 py-3 px-3.5 cursor-pointer" style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.06)"}`, background: !selected ? (dark ? "rgba(167,139,250,.1)" : "rgba(124,58,237,.06)") : "transparent", borderLeft: !selected ? `3px solid ${dark ? "#a78bfa" : "#7c3aed"}` : "3px solid transparent" }}>
          ...
          {tickets.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center py-10 px-4 text-center">
              <div className="w-11 h-11 rounded-full flex items-center justify-center mb-3" style={{ background: dark ? "rgba(255,255,255,.07)" : "rgba(0,0,0,.03)" }}>
      ...
      <div className="sup-split-chat">
        {/* Header */}
        <div className="shrink-0 h-[56px] flex items-center gap-2.5 px-[18px]" style={{ borderBottom: `1px solid ${dark ? "rgba(255,255,255,.14)" : "rgba(0,0,0,.1)"}`, background: dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)" }}>
            ...
            {canReply && <button onClick={closeTicket} className="py-[5px] px-3.5 rounded-full text-xs font-medium bg-transparent cursor-pointer shrink-0 transition-transform duration-200 hover:-translate-y-px" style={{ border: `1px solid ${dark ? "rgba(255,255,255,.18)" : "rgba(0,0,0,.14)"}`, color: t.textMuted, fontFamily: "inherit" }}>Close</button>}
        </div>
        ...
        {isNewTicket ? (
          ...
          <>
            ...
            {!selected && showQuick && !isLive && <>
              <div className="py-2 px-4 gap-[5px] flex-wrap shrink-0 hidden desktop:flex" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
                {QUICK_ACTIONS.filter(a => !(a.id === "human" && hasOpenTicket)).map(a => <button key={a.id} onClick={() => handleQuick(a.id)} className="sup-quick-btn py-[7px] px-3 rounded-lg text-xs border border-solid cursor-pointer font-[inherit] flex items-center gap-[5px] transition-transform duration-200 hover:-translate-y-px" style={{ background: a.id === "human" ? (dark ? "rgba(196,125,142,.14)" : "rgba(196,125,142,.1)") : (dark ? "rgba(255,255,255,.09)" : "rgba(0,0,0,.04)"), borderColor: a.id === "human" ? (dark ? "rgba(196,125,142,.28)" : "rgba(196,125,142,.19)") : (dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"), color: a.id === "human" ? t.accent : (t.textSoft || t.textMuted), fontWeight: a.id === "human" ? 600 : 500 }}><QuickIcon id={a.id} size={13} />{a.label}</button>)}
              </div>
              <div className="desktop:hidden relative shrink-0">
                {quickPopup && <div className="absolute bottom-full left-0 right-0 mb-1 py-2.5 px-3 rounded-xl flex gap-[5px] flex-wrap" style={{ background: dark ? "#1a1329" : "#fff", border: `1px solid ${dark ? "rgba(255,255,255,.18)" : "rgba(0,0,0,.14)"}`, boxShadow: dark ? "0 -8px 24px rgba(0,0,0,.4)" : "0 -8px 24px rgba(0,0,0,.1)" }}>
                ...
                <div className="py-1.5 px-4 flex justify-center" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` }}>
                  <button onClick={() => setQuickPopup(!quickPopup)} className="py-[6px] px-4 rounded-full text-xs font-medium cursor-pointer font-[inherit] flex items-center gap-1.5" style={{ background: dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.05)", border: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.1)"}`, color: t.textSoft || t.textMuted }}>
        ...
        {canReply && (() => { const lm = chatMsgs[chatMsgs.length - 1]; return lm?.from === "user"; })() && (
          <div className="py-1.5 px-4 text-center text-[11px] shrink-0 flex items-center justify-center gap-1.5" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}`, color: dark ? "#fcd34d" : "#d97706", background: dark ? "rgba(234,179,8,.04)" : "rgba(234,179,8,.03)" }}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            Sending extra messages may increase your wait time
          </div>
        )}
        {isNewTicket ? (
          <div className="py-2.5 px-4 shrink-0" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"}` }}>
            ...
            <button onClick={createTicket} disabled={!newSubject.trim() || !newMessage.trim() || ticketLoading} className="w-full py-[11px] rounded-[10px] text-sm font-semibold border-none cursor-pointer transition-[transform,box-shadow] duration-200 hover:translate-y-[-1px] hover:shadow-[0_6px_20px_rgba(196,125,142,.31)]" style={{ background: newSubject.trim() && newMessage.trim() ? "linear-gradient(135deg,#c47d8e,#a3586b)" : (dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.06)"), color: newSubject.trim() && newMessage.trim() ? "#fff" : t.textMuted, cursor: newSubject.trim() && newMessage.trim() ? "pointer" : "default", fontFamily: "inherit" }}>{ticketLoading ? "Creating..." : "Create Ticket"}</button>
          </div>
        ) : (!selected || canReply) ? (
          <div className="flex gap-2 items-end shrink-0 py-2.5 px-4" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"}` }}>
            <textarea value={input} onChange={e => { setInput(e.target.value); e.target.style.height = "auto"; e.target.style.height = Math.min(e.target.scrollHeight, 120) + "px"; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMsg(); e.target.style.height = "38px"; } }} placeholder={isTicket ? "Type a message..." : (waitingForAgent ? "Add details while you wait..." : isLive ? "Message support..." : "Ask a question...")} rows={1} className="flex-1 py-2.5 px-4 rounded-[20px] text-sm outline-none resize-none" style={{ background: dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.06)", border: `1px solid ${dark ? "rgba(255,255,255,.16)" : "rgba(0,0,0,.12)"}`, color: t.text, fontFamily: "inherit", minHeight: "38px", maxHeight: "120px", lineHeight: "1.4" }} />
            <button onClick={sendMsg} className="w-[38px] h-[38px] rounded-full border-none flex items-center justify-center shrink-0" style={{ background: input.trim() ? "linear-gradient(135deg,#c47d8e,#a3586b)" : (dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.06)"), cursor: input.trim() ? "pointer" : "default" }}>
          ...
          <div className="py-3 px-[18px] text-center text-[13px] shrink-0" style={{ borderTop: `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}`, color: t.textMuted, background: dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)" }}>This conversation has been resolved</div>
        ) : null}
      </div>
    </div>
  );
