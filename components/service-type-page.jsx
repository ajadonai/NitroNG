    ...
    <>
      ...
      <div className="min-h-dvh flex flex-col font-[Plus Jakarta Sans,system-ui,sans-serif]" style={{ background: t.bg }}>
        ...
        <nav className="max-w-[700px] mx-auto w-full px-6 pt-8 max-md:pt-6" aria-label="Breadcrumb">
          <ol className="flex items-center gap-1.5 text-[11px] list-none p-0 m-0" style={{ color: t.textMuted }}>
        ...
        <main className="flex-1 px-6 pb-20 max-w-[900px] mx-auto w-full">
          ...
          <section className="mb-12">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: cardBg, border: `1.5px solid ${dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)"}` }}>
              {/* Header */}
              <div className="py-3 px-6 max-md:px-4 grid grid-cols-[1fr_90px_70px_100px] max-md:grid-cols-[1fr_80px] gap-2 text-[11px] font-medium" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.03)", borderBottom: `1px solid ${dark ? "rgba(255,255,255,.06)" : "rgba(0,0,0,.04)"}`, color: t.textMuted }}>
                <span>Service</span>
                ...
                {services.map(s => s.tiers.map((tier, ti) => (
                  <div key={`${s.name}-${ti}`} className="grid grid-cols-[1fr_90px_70px_100px] max-md:grid-cols-[1fr_80px] gap-2 items-center py-3.5 px-6 max-md:px-4">
                    <div className="min-w-0">
                      <div className="text-[13px] max-md:text-[13px] font-medium truncate" style={{ color: t.text }}>
                    ...
                    <div className="text-right">
                      {tier.price >= 100000 ? (
                        <>
                          <div className="text-[13px] max-md:text-[13px] font-bold" style={{ color: accent }}>₦{(tier.price / 1000).toLocaleString()}</div>
                          <div className="text-[11px]" style={{ color: t.textMuted }}>per unit</div>
                        </>
                      ) : (
                        <div className="text-[13px] max-md:text-[13px] font-bold" style={{ color: accent }}>₦{tier.price.toLocaleString()}</div>
                      )}
                    </div>
                    <div className="text-center max-md:hidden text-[11px]" style={{ color: tier.refill ? '#22c55e' : t.textMuted }}>
                      {tier.refill ? '✓' : '—'}
                    </div>
                    <div className="text-right max-md:hidden text-[11px]" style={{ color: t.textMuted }}>
                      {tier.speed}
                    </div>
                  </div>
                )))}
            ...
            <div className="mt-3 py-3 px-4 rounded-xl flex items-start gap-3" style={{ background: dark ? "rgba(251,191,36,.06)" : "rgba(251,191,36,.06)", border: `1px solid ${dark ? "rgba(251,191,36,.16)" : "rgba(217,119,6,.12)"}` }}>
              ...
              <div className="text-[11px] leading-[1.55]" style={{ color: t.textSoft }}>Prices are in Naira per 1,000 units and update with the exchange rate. The price you see at checkout is the price you pay.</div>
            </div>
          </section>
          ...
          <section className="mb-12">
            ...
            <div className="flex flex-col gap-2.5">
              <a href={`/services/${platformSlug}`} className="flex items-center gap-3 py-3.5 px-5 rounded-xl no-underline transition-colors duration-150" style={{ background: dark ? "rgba(196,125,142,.06)" : "rgba(196,125,142,.04)", border: `1px solid ${dark ? "rgba(196,125,142,.14)" : "rgba(196,125,142,.1)"}`, color: t.text }}>
                ...
                <span className="text-[13px] font-medium">All {platform} services</span>
              </a>
              {[
