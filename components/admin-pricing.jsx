...
function NumInput({ value, onChange, min = 0, max = 999999, fallback, width = 64, decimal, dark }) {
  ...
  const inpStyle = { background: dark ? "rgba(255,255,255,.12)" : "#fff", borderWidth: "0.5px", borderStyle: "solid", borderColor: dark ? "rgba(255,255,255,.18)" : "rgba(0,0,0,.18)", color: dark ? "#e5e5e5" : "#1a1a1a", fontFamily: "'JetBrains Mono',monospace", width };
  return <input
    value={raw}
    ...
    <div className="pb-10">
      ...
      <div className="adm-card p-5 mb-4" style={cardS}>
        ...
        {brackets.map((b, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 py-2.5" style={{ borderBottom: i < brackets.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` : "none" }}>
      ...
      <div className="adm-card p-5 mb-4" style={cardS}>
        ...
        <div className="grid grid-cols-3 gap-2.5 my-4">
          {[
            ...
            <div key={label} className="py-3 px-2 rounded-[10px] text-center" style={{ background: dark ? "rgba(255,255,255,.05)" : "rgba(0,0,0,.02)" }}>
      ...
      <div className="adm-card p-5 mb-5" style={cardS}>
        ...
        {[["Basic Views", 2], ["Budget Likes", 50], ["Standard Followers", 800], ["Premium Followers", 3000], ["Custom Comments", 12000], ["Premium Reviews", 50000]].map(([name, cost], i, arr) => {
          ...
          return (
            <div key={name} className="flex items-center py-2.5 gap-2" style={{ borderBottom: i < arr.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.12)" : "rgba(0,0,0,.08)"}` : "none" }}>
      ...
      <div className="flex gap-2.5 flex-wrap">
        ...
        <button onClick={reset} className="py-2.5 px-6 rounded-[10px] bg-transparent text-sm font-medium cursor-pointer transition-transform duration-200 hover:-translate-y-px" style={{ border: `1px solid ${dark ? "rgba(255,255,255,.18)" : "rgba(0,0,0,.14)"}`, color: t.textMuted }}>Reset Defaults</button>
      </div>
