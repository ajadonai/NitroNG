...
function NumInput({ value, onChange, min = 0, max = 999999, fallback, width = 64, decimal, dark }) {
  ...
  const inpStyle = { background: dark ? "rgba(255,255,255,.04)" : "#fff", borderWidth: "0.5px", borderStyle: "solid", borderColor: dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.1)", color: dark ? "#e5e5e5" : "#1a1a1a", fontFamily: "'JetBrains Mono',monospace", width };
  return <input
    value={raw}
...
function Tip({ children, green, dark }) {
  return (
    <div className="py-2.5 px-3.5 rounded-lg text-[13px] leading-[1.6] mb-4" style={{ background: green ? (dark ? "rgba(74,222,128,.05)" : "rgba(22,163,74,.03)") : (dark ? "rgba(196,125,142,.05)" : "rgba(196,125,142,.03)"), borderLeft: `3px solid ${green ? (dark ? "#4ade80" : "#16a34a") : "#c47d8e"}`, color: dark ? "#888" : "#666" }}>
      {children}
    </div>
  );
}
    ...
    <div className="pb-10">
      ...
      <div className="adm-card p-5 mb-4" style={cardS}>
        ...
        {brackets.map((b, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 py-2.5" style={{ borderBottom: i < brackets.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)"}` : "none" }}>
      ...
      <div className="adm-card p-5 mb-5" style={cardS}>
        ...
        {[["Basic Views", 2], ["Budget Likes", 50], ["Standard Followers", 800], ["Premium Followers", 3000], ["Custom Comments", 12000], ["Premium Reviews", 50000]].map(([name, cost], i, arr) => {
          ...
          return (
            <div key={name} className="flex items-center py-2.5 gap-2" style={{ borderBottom: i < arr.length - 1 ? `1px solid ${dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)"}` : "none" }}>
      ...
      <div className="flex gap-2.5 flex-wrap">
        <button onClick={save} disabled={saving} className="py-2.5 px-6 rounded-[10px] border-none text-sm font-semibold" style={{ background: "linear-gradient(135deg,#c47d8e,#a3586b)", color: "#fff", cursor: saving ? "wait" : "pointer", opacity: saving ? .5 : 1 }}>{saving ? "Saving..." : "Save Settings"}</button>
        <button onClick={recalc} disabled={recalcing} className="py-2.5 px-6 rounded-[10px] text-sm font-semibold" style={{ border: `1px solid ${dark ? "rgba(252,165,165,.2)" : "rgba(220,38,38,.15)"}`, background: dark ? "rgba(252,165,165,.06)" : "rgba(220,38,38,.04)", color: dark ? "#fca5a5" : "#dc2626", cursor: recalcing ? "wait" : "pointer", opacity: recalcing ? .5 : 1 }}>{recalcing ? "Recalculating..." : "Recalculate All Prices"}</button>
        <button onClick={reset} className="py-2.5 px-6 rounded-[10px] bg-transparent text-sm font-medium cursor-pointer" style={{ border: `1px solid ${dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.08)"}`, color: t.textMuted }}>Reset Defaults</button>
      </div>
