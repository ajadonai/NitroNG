...
const TS = {
  Budget: { bg: "#fef7ed", border: "#e8d5b8", text: "#854F0B", bgD: "#2d2210", borderD: "#5a4020", label: "B" },
  ...
  return (
    <div className="p-5 max-md:p-4">
      <div className="flex justify-between items-center mb-3.5">
        <span className="m text-[13px] uppercase tracking-[1.5px] py-2 px-3 rounded-lg" style={{ color: t.textMuted, background: dark ? "rgba(196,125,142,.18)" : "rgba(196,125,142,.12)" }}>Place order</span>
        {onClose && <button onClick={onClose} className="bg-transparent border border-solid rounded-lg w-7 h-7 flex items-center justify-center text-base cursor-pointer" style={{ borderColor: t.cardBorder, color: t.textSoft }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>}
      </div>
      ...
      {selTier && <>
        ...
        <div className="rounded-[10px] p-3 mb-3.5 border border-solid" style={{ background: dark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.04)", borderColor: t.cardBorder }}>
    ...
    <div ref={mainRef} style={{ paddingBottom: orderMode === "bulk" && cartRows.length > 0 ? 82 : 0 }}>
      ...
      {orderMode === "single" && (orderModal || orderSuccess) && hasOrder && (
        <div className="no-modal-overlay flex fixed inset-0 bg-black/40 z-50 items-center max-md:items-end justify-center p-6 max-md:p-0" onClick={() => { setOrderModal(false); setOrderSuccess(null); }} onKeyDown={e=>{if(e.key==='Escape'){setOrderModal(false);setOrderSuccess(null)}if((e.metaKey||e.ctrlKey)&&e.key==='Enter'&&!orderSuccess&&!orderLoading){submitOrder()}}}>
          <div role="dialog" aria-modal="true" aria-label="Order summary" className="w-full max-w-[420px] max-md:max-w-none rounded-2xl max-md:rounded-b-none max-h-[90vh] max-md:max-h-[80vh] overflow-y-auto shadow-[0_20px_60px_rgba(0,0,0,.38)] border border-solid max-md:border-b-0 max-md:pb-[env(safe-area-inset-bottom)]" onClick={e => e.stopPropagation()} style={{ background: dark ? "#0e1120" : "#ffffff", borderColor: t.cardBorder }}>
            {orderSuccess ? (
...
export function ServicesSidebar({ dark, t }) {
  ...
  return (
    <div style={{ fontSize: "103%" }}>
      ...
      {[
        ["Budget", "B", "Cheapest. No refill if count drops. Good for testing."],
        ["Standard", <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>, "Best value. Free top-up if count drops."],
        ["Premium", <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 4l3 12h14l3-12-6 7-4-7-4 7-6-7z"/><path d="M3 20h18"/></svg>, "Top quality. Won't drop. Lifetime guarantee."],
      ].map(([tier, icon, desc]) => {
        const s = TS[tier];
        return (
