...
export default function NewOrderPage({ openFullList, onOpenedFullList, dark, t, user, onOrderSuccess, onViewOrders, onNavigate, onTopUp, platform, setPlatform, selSvc, setSelSvc, selTier, setSelTier, qty, setQty, link, setLink, comments, setComments, tourActive, activePromotion, rewards, socialLinks, refreshRewards, onSuppressBackToTop }) {
  const tr = useT();
  const money = useMoney();
  ...
  // The dashboard shell owns the back-to-top button now, shared by every tab.
  // This page is the one that has to speak up: its bulk cart bar is a
  // full-width bar in the same corner, so it tells the shell to hide the
  // button rather than let the two collide.
  useEffect(() => {
    onSuppressBackToTop?.(orderMode === "bulk");
    return () => onSuppressBackToTop?.(false);
  }, [orderMode, onSuppressBackToTop]);
            ...
            {orderSuccess ? (
              <div className="rcp" style={{ "--card": successChrome.card, "--ink": successChrome.text, "--mut": successChrome.muted, "--dim": dark ? "#5c6170" : "#a19b93", "--line": successChrome.hair, "--rail": dark ? "rgba(255,255,255,.07)" : "rgba(0,0,0,.06)", "--soft": dark ? "#120c1e" : "#faf9f7", "--ac": t.accent, "--ok": successChrome.money }}>
                ...
                <div className="rcp-acts">
                  <button type="button" onClick={() => { setOrderSuccess(null); setOrderModal(true); }} className="nb sec">{tr("Order again")}</button>
